...
from packages.contracts.event_bus.interfaces import EventBus, EventClient, EventEnvelope, EventPublisher, EventRecorder, EventSubscription, JsonObject
from packages.contracts.event_bus.subjects import STREAM_MAX_AGE_SECONDS, STREAM_MAX_BYTES, STREAM_NAME, STREAM_SUBJECTS, EventSubject
...
CURRENT_CAUSATION_ID: ContextVar[str | None] = ContextVar("current_event_causation_id", default=None)
...
def event_context(evt: EventEnvelope) -> dict[str, str | None]:
    ...
    return {"subject": evt.subject, "event_id": evt.event_id, "correlation_id": evt.correlation_id, "causation_id": evt.causation_id, "source": evt.source}
...
class NatsEventBus(EventBus):
    ...
    async def connect(self) -> None:
        ...
        for attempt in range(DEPENDENCY_RETRY_LIMIT):
            try:
                self.nc = await nats.connect(self.url, name=env(Runtime.SERVICE_NAME_ENV, Runtime.DEFAULT_SERVICE_NAME))
    ...
    async def ensure_stream(self) -> None:
        ...
        try:
            ...
            await self.js.update_stream(name=STREAM_NAME, subjects=subjects, storage="file", max_age=STREAM_MAX_AGE_SECONDS, max_bytes=STREAM_MAX_BYTES)
        except not_found:
            await self.js.add_stream(name=STREAM_NAME, subjects=STREAM_SUBJECTS, storage="file", max_age=STREAM_MAX_AGE_SECONDS, max_bytes=STREAM_MAX_BYTES)
...
class RecordedEventClient:
    ...
    async def emit(self, subject: str, source: str, payload: JsonObject, correlation_id: str | None = None, causation_id: str | None = None) -> EventEnvelope:
        evt = await self.publisher.emit(subject, source, payload, correlation_id, causation_id or CURRENT_CAUSATION_ID.get())
...
class DeadLetterSink:
    ...
    async def capture(self, evt: EventEnvelope, consumer: str, error: Exception, attempts: int) -> EventEnvelope:
        ...
        return await self.events.emit(EventSubject.DEAD_LETTER_CREATED, self.source, dead_letter, evt.correlation_id, evt.event_id)


async def emit_and_record(bus: EventPublisher, db: EventRecorder, subject: str, source: str, payload: JsonObject, correlation_id: str | None = None, causation_id: str | None = None) -> EventEnvelope:
    return await RecordedEventClient(bus, db).emit(subject, source, payload, correlation_id, causation_id)
