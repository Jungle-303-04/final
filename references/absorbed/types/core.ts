...
// Keys in ResourcePermissions that represent optional CRDs Opsia can monitor
// when they're installed in the cluster. A `false` here means "CRD not
// installed (or RBAC denied)" — NOT "Opsia is missing data the user expects",
...
export interface Capabilities {
  ...
  // How / where this Opsia binary is running. Optional on the wire so a
  // newer frontend (e.g. opsia-hub-web bundling a fresher @opsia-io/opsia-app)
...
// DeploymentMode is the closed set of topologies Opsia can run in.
// `local` is a developer's machine with a kubeconfig (most OSS use).
// `in-cluster` is a Opsia pod inside the cluster, no kubeconfig.
// `cloud` is in-cluster + tunneled to Opsia Cloud's hub — UI suppresses
...
export interface HelmRelease {
  ...
  // than helm CLI / Opsia's Helm view, since changes here would get
...
// Library consumers (Opsia Hub) get all GitOps surfaces — the package
