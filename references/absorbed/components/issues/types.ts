...
// host-agnostic: Opsia Hub feeds it fleet-resolved grouped issues, and OSS
// Opsia feeds a single-cluster ("fleet of one") set. Hosts map their wire
// payloads onto these types; the component renders against them.
//
// Mirrors the grouped Issue model opsia emits (internal/issues.GroupIssues →
...
// opsia/pkg/audit.ResourceKey — so Issues and Checks share deep-links rather
 ...
 *  separate axes). Matches opsia's issues.Severity. */
...
export interface Issue {
  ...
  // wire (opsia emits them omitempty).
  cluster_id?: string;
  cluster_name?: string;
  group?: string;
  kind: string;
  ...
  /**
   * Best-effort timing evidence from K8s-native signals. Absent when Opsia has
