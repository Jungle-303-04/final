        ...
        <>
          <Tooltip content={ageTitle(issue)} delay={200} wrapperClassName="shrink-0">
            <time
              ...
              className="flex items-center gap-1 text-xs tabular-nums text-muted-foreground/75"
            >
          ...
          {timing ? (
            <Tooltip content={timing.tooltip} delay={200}>
              <span className="badge-sm text-[10px] text-muted-foreground">{timing.chip}</span>
            </Tooltip>
          ) : null}
        </>
      ) : null}
  ...
  return (
    ...
    >
      ...
      <div
        ...
        className={`group @container/issue flex cursor-pointer items-center gap-3 border-l-[3px] py-3 pl-3 pr-4 transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--primary)]/40 ${open ? ISSUE_SEVERITY_HEADER_BAND_CLASS[severity] : ISSUE_SEVERITY_RAIL_CLASS[severity]}`}
      >
        <SeverityIcon className={`h-[18px] w-[18px] shrink-0 ${ISSUE_SEVERITY_TEXT_CLASS[severity]}`} aria-hidden />

        <div className="flex min-w-0 flex-1 flex-col gap-1">
          <div className="flex min-w-0 items-baseline gap-2">
            <span className="min-w-0 truncate text-sm font-medium text-foreground">{categoryLabel(issue.category)}</span>
            ...
            {issue.reason ? (
              <span
                ...
                className={`min-w-0 flex-1 truncate text-xs text-muted-foreground/75 transition-opacity duration-200 ${open ? 'opacity-0' : 'opacity-100'}`}
              >
                <span className="font-medium text-muted-foreground">{issue.reason}</span>
              ...
              <>
                ...
                <Tooltip content={confidenceTitle(issue.incident_parent.confidence ?? '')} delay={200} wrapperClassName="min-w-0">
                  <span className="truncate text-muted-foreground/75">
                    ...
                    <span className="font-medium text-muted-foreground">{issue.incident_parent.ref.kind} / {issue.incident_parent.ref.name}</span>
                  </span>
                </Tooltip>
              </>
            ) : null}
...
function Diagnosis({ issue }: { issue: Issue }) {
  ...
  return (
    <div className="flex flex-col divide-y divide-border/70 [&>*]:py-4 [&>*:first-child]:pt-0 [&>*:last-child]:pb-0">
      ...
      >
        {issue.cause ? (
          <p className="text-sm leading-relaxed text-foreground">{issue.cause}</p>
        ) : (
        ...
        {meta.length > 0 ? <p className="text-xs leading-relaxed text-muted-foreground/75 tabular-nums">{meta.join(' · ')}</p> : null}
      </CardSection>
  ...
  return (
    ...
    >
      <ul className="flex flex-col gap-2">
        {facts.map((fact, idx) => (
          <li key={`${fact.type}-${idx}`} className="flex flex-col gap-1.5 rounded-md border border-border/70 px-2.5 py-2">
            <div className="flex min-w-0 items-baseline gap-2">
              <span className="shrink-0 text-xs font-medium text-muted-foreground">{diagnosticFactLabel(fact.type)}</span>
              {fact.confidence ? (
                <Tooltip content={confidenceTitle(fact.confidence)} delay={200}>
                  <span className="shrink-0 badge-sm text-[10px] text-muted-foreground/75">
                    {fact.confidence} confidence
                  </span>
                </Tooltip>
              ) : null}
              {fact.message ? <span className="min-w-0 break-words text-xs leading-relaxed text-muted-foreground/75">{fact.message}</span> : null}
            </div>
...
function ResourceLine({
  ...
  const body = (
    <>
      {label ? <span className="shrink-0 text-[11px] font-semibold uppercase tracking-[0.06em] text-muted-foreground/75">{label}</span> : null}
      ...
      <span className={`min-w-0 truncate text-sm ${linkable ? `${compact ? 'font-semibold' : 'font-medium'} text-[var(--primary)]` : 'font-medium text-foreground'}`}>
        {r.namespace ? `${r.namespace} / ` : ''}
        {r.name}
      </span>
      {count && count > 1 ? (
        <Tooltip content={`${count} affected resources grouped under this issue`} delay={200}>
          <span className="shrink-0 text-[10px] text-muted-foreground/75 tabular-nums">{count} affected</span>
        </Tooltip>
      ) : null}
      {linkable && <ResourceLinkIcon className="h-3 w-3 shrink-0 text-muted-foreground/75 opacity-0 transition-opacity group-hover/r:opacity-100" />}
    </>
  );
  ...
  const cls = compact
    ? 'group/r flex w-full items-baseline gap-2 rounded-md px-2 py-1 text-left transition-colors hover:bg-accent/60'
    : 'group/r flex w-full items-baseline gap-2 rounded-md px-2 py-1 text-left text-sm transition-colors hover:bg-accent/60';
  return (
    <li>
      {onResourceClick ? (
