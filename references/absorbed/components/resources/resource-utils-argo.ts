...
export function getArgoApplicationStatus(app: any): StatusBadge {
  ...
  // "opsiahq.io/suspended-prune" annotation and the legacy "opsia.io/..."
  // key still present on Applications suspended by older Opsia builds.
  ...
  const suspendedByOpsia = annotations?.['opsiahq.io/suspended-prune'] || annotations?.['opsia.io/suspended-prune']
  if (health === 'Suspended' || (!hasAutomatedSync && suspendedByOpsia)) {
...
// Opsia suspends an Argo Application by clearing spec.syncPolicy.automated and
...
// opsiahq.io keys and the legacy opsia.io keys (still on apps suspended by
// older builds) count. Shared so the fleet table and the detail page can't
// disagree on whether an app is suspended.
export function isArgoSuspendedByOpsia(app: any): boolean {
  const a = app?.metadata?.annotations
  return Boolean(
    a?.['opsiahq.io/suspended-prune'] ||
      a?.['opsiahq.io/suspended-selfheal'] ||
      a?.['opsia.io/suspended-prune'] ||
      a?.['opsia.io/suspended-selfheal'],
  )
}
