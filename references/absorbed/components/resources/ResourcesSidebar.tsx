        ...
        className={cn(
          ...
          isSelected
            ...
            : isHighlighted
              ...
              : forbidden
                ? 'text-muted-foreground/45 hover:bg-popover hover:text-muted-foreground'
        ...
        <div className="ml-auto flex items-center gap-1 shrink-0">
          {onTogglePin && (
            <span
              ...
              className={cn(
                'p-0.5 rounded transition-all hover:bg-accent',
                isPinned
                  ? 'text-muted-foreground'
                  : 'opacity-0 group-hover/kind:opacity-100 text-muted-foreground/45'
              )}
          ...
          {forbidden ? (
            <Tooltip content="Insufficient permissions" position="left">
              <Shield className="w-3.5 h-3.5 text-warning/60" />
            </Tooltip>
          ) : count === null ? (
            <Tooltip content={COUNT_UNAVAILABLE_MESSAGE} position="left">
              <span
                className={cn(
                  'text-xs py-0.5 rounded text-center font-mono w-8 text-muted-foreground/45',
                  isSelected ? 'bg-primary/30 selection-text' : 'bg-popover',
                )}
...
export function ResourcesSidebar({
  ...
  return (
    <div className={cn('w-56 2xl:w-72 bg-card dark:bg-background border-r border-border overflow-y-auto overflow-x-hidden shrink-0', className)}>
      ...
      <div className="px-2 py-3 border-b border-border">
        <div className="relative">
          ...
          <Input
            ...
            className="w-full pl-7 pr-7 py-2 bg-popover border border-border/60 rounded-lg text-sm text-foreground placeholder-muted-foreground/45 focus:outline-none focus:ring-2 focus:ring-primary"
          />
          {kindFilter && (
            <button
              onClick={() => setKindFilter('')}
              className="absolute right-1.5 top-1/2 -translate-y-1/2 p-0.5 rounded hover:bg-card text-muted-foreground/75 hover:text-muted-foreground"
            >
              <X className="w-3 h-3" />
            </button>
          )}
      ...
      <nav className="p-2">
        ...
        <div className="mb-2">
          <button
            ...
            className="w-full flex items-center gap-2 px-2 py-1.5 text-xs font-medium text-muted-foreground/75 hover:text-muted-foreground uppercase tracking-wide"
          >
            {favoritesExpanded ? (
            ...
            {!favoritesExpanded && pinned.length > 0 && (
              <span className={cn('text-xs py-0.5 rounded bg-popover text-muted-foreground font-normal normal-case text-center font-mono', pinned.length < 1000 ? 'w-8' : 'w-9')}>
                {pinned.length}
              </span>
            )}
          </button>
          <div className={cn(
            ...
            <div className="overflow-hidden">
              <div className="space-y-0.5">
                {pinned.length === 0 ? (
                  <div className="px-3 py-2 text-xs text-muted-foreground/45">
            ...
            return (
              <div key={category.name} className="mb-2">
                <button
                  ...
                  className="w-full flex items-center gap-2 px-2 py-1.5 text-xs font-semibold text-muted-foreground/75 hover:text-muted-foreground uppercase tracking-wide"
                >
                  {isExpanded ? (
                    <ChevronDown className="w-3 h-3" />
                  ) : (
                  ...
                  {!isExpanded && (
                    <span className={cn('text-xs py-0.5 rounded bg-popover text-muted-foreground font-normal normal-case text-center font-mono', category.total < 1000 ? 'w-8' : 'w-9')}>
                      {category.total}
                    </span>
                  )}
                </button>
              ...
              >
                ...
                {count === null ? (
                  <Tooltip content={COUNT_UNAVAILABLE_MESSAGE} position="left">
                    <span
                      className={cn(
                        'badge font-mono text-muted-foreground/45',
                        isSelected ? 'bg-primary/30 selection-text' : 'bg-popover',
                      )}
                      aria-label={COUNT_UNAVAILABLE_MESSAGE}
                    >
        ...
        {hiddenKindsCount > 0 || hiddenGroupsCount > 0 || showEmptyKinds ? (
          <button
            ...
            className="w-full flex items-center gap-2 px-3 py-2 mt-2 text-xs text-muted-foreground/75 hover:text-muted-foreground border-t border-border"
          >
            {showEmptyKinds ? (
              <>
