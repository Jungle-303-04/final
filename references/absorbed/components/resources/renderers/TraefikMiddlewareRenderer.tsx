    ...
    <>
      {type === 'unknown' && (
        <AlertBanner
          ...
          message="This middleware uses a type Opsia doesn't render specially (e.g. a plugin or a newer/commercial middleware). The raw configuration is shown below."
        />
      )}
          ...
          <div className="space-y-1">
            {(config.middlewares || []).map((m: any, i: number) => (
              <div key={i} className="flex items-center gap-2 text-xs">
                <span className="text-muted-foreground/75 w-4 text-right">{i + 1}.</span>
                <ResourceLink
                  name={m.name}
            ...
            {(config.middlewares || []).length === 0 && (
              <span className="text-xs text-muted-foreground/75">No middlewares in chain</span>
            )}
          </div>
        </Section>
      ) : isAuth ? (
        <Section title="Authentication" icon={ShieldCheck} defaultExpanded>
          <PropertyList>
            ...
            {Array.isArray(config.users) && (
              <Property label="Inline users" value={<span className="text-muted-foreground/75 italic">{config.users.length} — hidden</span>} />
            )}
