...
export function AzureManagedControlPlaneCell({ resource, column }: { resource: any; column: string }) {
  switch (column) {
    ...
    default: return <span className="text-sm text-muted-foreground/75">-</span>
  }
}
...
export function AzureManagedMachinePoolCell({ resource, column }: { resource: any; column: string }) {
  switch (column) {
    ...
    case 'priority': {
      ...
      return (
        <span className={cn('badge badge-sm', p === 'Spot' ? CAPACITY_TYPE_BADGE.spot : CAPACITY_TYPE_BADGE.regular)}>{p}</span>
      )
    }
    default: return <span className="text-sm text-muted-foreground/75">-</span>
  }
