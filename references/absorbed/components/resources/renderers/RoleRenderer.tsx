    ...
    <>
      ...
      <Section title={`Rules (${rules.length})`} icon={Shield} defaultExpanded>
        <div className="space-y-3">
          {rules.map((rule: any, i: number) => (
            <div key={i} className="card-inner-lg">
              ...
              {rule.apiGroups && rule.apiGroups.length > 0 && (
                <div className="mb-2">
                  <div className="text-xs text-muted-foreground/75 mb-1">API Groups</div>
              ...
              {rule.resources && rule.resources.length > 0 && (
                <div className="mb-2">
                  <div className="text-xs text-muted-foreground/75 mb-1">Resources</div>
                  <div className="flex flex-wrap gap-1">
              ...
              {rule.verbs && rule.verbs.length > 0 && (
                <div className="mb-2">
                  <div className="text-xs text-muted-foreground/75 mb-1">Verbs</div>
                  <div className="flex flex-wrap gap-1">
                    {rule.verbs.map((verb: string) => (
                      <span
                        key={verb}
                        className={cn('badge', rbacVerbBadgeClass(verb))}
                      >
                        {verb}
                      </span>
                    ))}
              ...
              {rule.resourceNames && rule.resourceNames.length > 0 && (
                <div className="mb-2">
                  <div className="text-xs text-muted-foreground/75 mb-1">Resource Names</div>
                  <div className="flex flex-wrap gap-1">
              ...
              {rule.nonResourceURLs && rule.nonResourceURLs.length > 0 && (
                <div>
                  <div className="text-xs text-muted-foreground/75 mb-1">Non-Resource URLs</div>
                  <div className="flex flex-wrap gap-1">
          ...
          {rules.length === 0 && (
            <div className="text-sm text-muted-foreground/75">No rules defined</div>
          )}
        </div>
      </Section>
        ...
        <Section title="Aggregation Rule" defaultExpanded>
          <div className="space-y-3">
            {aggregationRule.clusterRoleSelectors.map((selector: any, i: number) => (
              <div key={i} className="card-inner-lg">
                <div className="text-xs text-muted-foreground/75 mb-1">Match Labels</div>
                {selector.matchLabels && Object.keys(selector.matchLabels).length > 0 ? (
                  ...
                  <div className="text-xs text-muted-foreground/75">No labels specified</div>
                )}
              </div>
            ))}
          </div>
  ...
  if (loading) {
    return (
      <Section title="Bindings" icon={Users}>
        <div className="text-sm text-muted-foreground/75">Loading bindings…</div>
      </Section>
    )
  }
  if (error) {
            ...
            return (
              <div key={b.binding.kind + '/' + b.binding.namespace + '/' + b.binding.name} className="card-inner">
                ...
                <div className="flex items-center gap-2 flex-wrap text-xs mb-2">
                  <span className={cn('badge', rbacKindBadgeClass(b.binding.kind))}>
                    {b.binding.kind}
                  </span>
                  <ResourceLink
...
function RoleBindingSubjectChip({
  ...
  if (subject.kind === 'ServiceAccount') {
    return (
      <span className="inline-flex items-center gap-1 px-1.5 py-0.5 rounded bg-popover border border-border">
        ...
        <ResourceLink
          ...
          onNavigate={onNavigate}
        />
        {subject.namespace && <span className="text-muted-foreground">({subject.namespace})</span>}
      </span>
    )
  }
  return (
    <span
      className="inline-flex items-center gap-1 px-1.5 py-0.5 rounded bg-popover border border-border text-foreground"
      title={`${subject.kind} — no detail page (external identity)`}
    >
      <span className="text-muted-foreground">{subject.kind.toLowerCase()}:</span>
      {subject.name}
    </span>
  )
}
