...
function RefDisplay({ ref: destRef, ns, onNavigate }: { ref: any; ns: string; onNavigate?: RendererProps['onNavigate'] }) {
  if (!destRef) return <span className="text-muted-foreground/75">-</span>
  ...
  if (destRef.uri) {
    return <span className="text-foreground break-all text-xs">{destRef.uri}</span>
  }
          ...
          <div className="space-y-2">
            {steps.map((step: any, i: number) => (
              <div key={i} className="card-inner flex items-center gap-2">
                <span className="text-xs font-medium text-muted-foreground/75 w-6 shrink-0">#{i + 1}</span>
          ...
          <div className="space-y-3">
            {branches.map((branch: any, i: number) => (
              <div key={i} className="card-inner-lg">
                <div className="text-sm font-medium text-foreground mb-2">Branch {i + 1}</div>
                <PropertyList>
                  {branch.filter && (
