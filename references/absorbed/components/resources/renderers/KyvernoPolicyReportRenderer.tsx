...
const severityColorMap: Record<string, string> = {
  ...
  info: 'bg-accent text-muted-foreground/75',
}
    ...
    <div className="border-b border-border/30 last:border-b-0">
      <div
        className={cn(
          ...
          hasMessage && 'cursor-pointer hover:bg-accent/50',
        )}
        onClick={() => hasMessage && setExpanded(!expanded)}
      >
        {hasMessage ? (
          expanded ? <ChevronDown className="w-3 h-3 text-muted-foreground/75 shrink-0" /> : <ChevronRight className="w-3 h-3 text-muted-foreground/75 shrink-0" />
        ) : (
          <span className="w-3 shrink-0" />
        )}
        {/* Result status */}
        <span className={cn('px-1.5 py-0.5 rounded text-[10px] font-medium shrink-0', resultColorMap[result.result] || 'bg-accent text-muted-foreground/75')}>
          {result.result || '-'}
        </span>
        {/* Severity */}
        {result.severity && (
          <span className={cn('px-1.5 py-0.5 rounded text-[10px] font-medium shrink-0', severityColorMap[result.severity?.toLowerCase()] || 'bg-accent text-muted-foreground/75')}>
            {result.severity}
          </span>
        )}
        {/* Policy / Rule */}
        <span className="text-muted-foreground truncate">{result.policy || '-'}</span>
        {result.rule && (
          <span className="text-muted-foreground/75 truncate">/ {result.rule}</span>
        )}
      </div>
      {expanded && message && (
        <div className="px-2 pb-2 pl-7">
          <div className="text-xs text-muted-foreground break-all card-inner">
            {message}
          </div>
          {result.category && (
            <div className="mt-1 text-[10px] text-muted-foreground/75">Category: {result.category}</div>
          )}
          {result.source && (
            <div className="text-[10px] text-muted-foreground/75">Source: {result.source}</div>
          )}
    ...
    <>
        ...
        {total > 0 && (
          <div className="mt-3">
            <div className="flex rounded overflow-hidden h-5 w-full">
              {summary.pass > 0 && (
                <div
                  className="bg-success/60 flex items-center justify-center text-[10px] font-medium text-white"
              ...
              {summary.fail > 0 && (
                <div
                  className="bg-destructive/60 flex items-center justify-center text-[10px] font-medium text-white"
              ...
              {summary.warn > 0 && (
                <div
                  className="bg-warning/60 flex items-center justify-center text-[10px] font-medium text-white"
            ...
            <div className="flex gap-3 mt-1.5 text-[10px]">
              {summary.pass > 0 && <span className="text-success">Pass: {summary.pass}</span>}
              {summary.fail > 0 && <span className="text-destructive">Fail: {summary.fail}</span>}
              {summary.warn > 0 && <span className="text-warning">Warn: {summary.warn}</span>}
              {summary.error > 0 && <span className="text-destructive">Error: {summary.error}</span>}
      ...
      {results.length > 0 && (
        <Section title={`Results (${results.length})`} icon={ListChecks} defaultExpanded>
          <div className="bg-popover/20 rounded-lg overflow-hidden max-h-[500px] overflow-y-auto">
...
const ruleTypeColorMap: Record<string, string> = {
  ...
  generate: 'bg-success/20 text-success',
      ...
      <Section title="Configuration" icon={Shield}>
        <PropertyList>
          <Property label="Failure Action" value={
            <span className={cn(
              'badge',
              isEnforce ? 'bg-destructive/20 text-destructive' : 'bg-warning/20 text-warning',
            )}>
              {action}
            </span>
          } />
        ...
        <div className="mt-3 flex flex-wrap gap-2">
          ...
          {ruleCountByType.generate > 0 && (
            <span className="px-2 py-0.5 rounded text-[10px] font-medium bg-success/20 text-success">
            ...
            {rules.map((rule, i) => (
              <div key={i} className="card-inner-lg">
                <div className="flex items-center gap-2 mb-1">
                  ...
                  <span className={cn('px-1.5 py-0.5 rounded text-[10px] font-medium', ruleTypeColorMap[rule.type] || 'bg-accent text-muted-foreground/75')}>
                    {rule.type}
                  </span>
                </div>
                <div className="flex gap-3 text-[10px] text-muted-foreground/75">
