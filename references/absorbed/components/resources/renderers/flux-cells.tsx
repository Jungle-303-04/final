...
export function GitRepositoryCell({ resource, column }: { resource: any; column: string }) {
  switch (column) {
    case 'url': {
      ...
      return (
        <Tooltip content={url}>
          <span className="text-sm text-muted-foreground truncate block">{url}</span>
        </Tooltip>
      )
    }
    case 'ref': {
    ...
    case 'revision': {
      ...
      return (
        <span className="text-sm text-muted-foreground/75 font-mono">{revision}</span>
      )
    }
    default:
...
export function KustomizationCell({ resource, column }: { resource: any; column: string }) {
  switch (column) {
    case 'source': {
      ...
      return (
        <Tooltip content={source}>
          <span className="text-sm text-muted-foreground truncate block">{source}</span>
        </Tooltip>
      )
    }
    case 'path': {
      const path = getKustomizationPath(resource)
      return <span className="text-sm text-muted-foreground/75 font-mono">{path}</span>
    }
    case 'status': {
    ...
    case 'revision': {
      ...
      if (!revision) return <span className="text-sm text-muted-foreground/75">-</span>
      ...
      return (
        <Tooltip content={revision}>
          <span className="text-sm text-muted-foreground font-mono truncate block">{formatted}</span>
        </Tooltip>
      )
    }
    case 'inventory': {
    ...
    case 'chart': {
      ...
      return (
        <Tooltip content={chart}>
          <span className="text-sm text-muted-foreground truncate block">{chart}</span>
        </Tooltip>
      )
    }
    case 'version': {
      const version = getFluxHelmReleaseVersion(resource)
      return <span className="text-sm text-muted-foreground">{version}</span>
    }
    case 'status': {
    ...
    case 'message': {
      ...
      return (
        <Tooltip content={message}>
          <span className="text-sm text-muted-foreground truncate block">{message}</span>
        </Tooltip>
      )
    }
    case 'revision': {
