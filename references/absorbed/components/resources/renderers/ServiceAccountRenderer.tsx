      ...
      <Section title="Configuration" icon={UserCog}>
        <PropertyList>
          <Property
            ...
            value={
              <span
                className={automountToken ? 'text-warning' : 'text-success'}
...
function BindingRow({
  ...
  return (
    <div className={cn('card-inner', risky && 'border-orange-500/40')}>
      ...
      <div className="flex items-center gap-2 flex-wrap text-xs">
        <span className={cn('badge', rbacKindBadgeClass(br.binding.kind))}>
          {br.binding.kind}
        </span>
        <ResourceLink
        ...
        <span className={cn('badge', rbacKindBadgeClass(br.role.kind))}>
          {br.role.kind}
        </span>
        <ResourceLink
          kind={roleKindPlural}
        ...
        {br.scopeNamespace && (
          <span
            className={cn('badge', rbacApiGroupBadgeClass)}
        ...
        {risky && (
          <span
            className={cn('badge inline-flex items-center gap-1', BADGE_SEVERITY_COLORS.alert)}
...
function RulesPreview({ rules, max }: { rules: RBACPolicyRule[]; max: number }) {
  ...
  return (
    <div className="mt-2 space-y-1">
      ...
      {hidden > 0 && (
        <div className="text-xs text-muted-foreground/75">+{hidden} more rule{hidden === 1 ? '' : 's'}</div>
      )}
    </div>
  )
}
...
function RuleLine({ rule }: { rule: RBACPolicyRule }) {
  ...
  return (
    <div className="flex items-center gap-1 flex-wrap text-xs">
      {verbs.map((v) => (
        <span key={v} className={cn('badge text-xs', rbacVerbBadgeClass(v))}>{v}</span>
      ))}
      <span className="text-muted-foreground">on</span>
      {isNonResource ? (
        nonResourceURLs.map((u) => (
          <span key={u} className="badge text-xs font-mono bg-popover text-muted-foreground">{u}</span>
        ))
      ) : (
        resources.map((r) => (
          <span key={r} className={cn('badge text-xs', rbacResourceBadgeClass)}>
            {r}
          </span>
        ))
      )}
      {!isNonResource && groups.length > 0 && groups.some((g) => g !== '') && (
        <>
          ...
          {groups.map((g) => (
            <span key={g} className={cn('badge text-xs', rbacApiGroupBadgeClass)}>
              {g === '' ? 'core' : g}
            </span>
          ))}
        </>
      ...
      {tab === 'by-binding' && (
        <div className="space-y-3">
          {allBindings.length === 0 ? (
            <div className="text-sm text-muted-foreground/75">No effective permissions.</div>
          ) : (
            allBindings.map((br, i) => (
              <div key={i} className="card-inner-lg">
                <div className="text-xs text-muted-foreground/75 mb-1">
                  Via {br.binding.kind} <span className="text-muted-foreground">{br.binding.name}</span>
                  {' → '}
                  {br.role.kind} <span className="text-muted-foreground">{br.role.name}</span>
                </div>
                <div className="space-y-1">
                  ...
                  {(!br.rules || br.rules.length === 0) && (
                    <div className="text-xs text-muted-foreground/75">No rules (orphan binding or empty role)</div>
                  )}
                </div>
              </div>
            ))
