              ...
              <table className="w-full text-xs">
                ...
                <tbody>
                  {displayedSecrets.map((secret: any, i: number) => (
                    <tr key={`${secret.ruleID || i}-${i}`} className="border-b border-border/50 hover:bg-accent/50">
                      <td className="py-1.5 px-1 text-muted-foreground font-mono">{secret.ruleID || '-'}</td>
                      <td className="py-1.5 px-1">
                        <span className={cn('px-1.5 py-0.5 rounded text-[10px] font-medium', SEVERITY_BADGE_COLORS[secret.severity] || BADGE_INACTIVE)}>
                          {secret.severity || '-'}
                        </span>
                      </td>
                      <td className="py-1.5 px-1 text-muted-foreground">{secret.category || secret.title || '-'}</td>
                      <td className="py-1.5 px-1 text-muted-foreground max-w-[180px] truncate font-mono" title={secret.target}>{secret.target || '-'}</td>
                      <td className="py-1.5 px-1 text-muted-foreground/75 max-w-[150px] truncate font-mono" title={secret.match}>{secret.match || '-'}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
