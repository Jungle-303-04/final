      ...
      {spec.additionalDisks?.length > 0 && (
        <Section title="Additional Disks" icon={Cpu}>
          <table className="w-full text-xs">
            <thead><tr className="text-muted-foreground/75"><th className="text-left font-medium py-1">Size</th><th className="text-left font-medium py-1">Type</th></tr></thead>
            <tbody>
              {spec.additionalDisks.map((d: any, i: number) => (
                <tr key={i} className="border-t border-border">
                  <td className="py-1 text-muted-foreground">{d.deviceType || '-'}</td>
                  <td className="py-1 text-muted-foreground">{d.size ? `${d.size}GB` : '-'}</td>
                </tr>
              ))}
            </tbody>
          </table>
