      ...
      <Section title="Overview" icon={Server}>
        <PropertyList>
          ...
          <Property label="Mode" value={
            <span className={cn('badge badge-sm', NODEPOOL_MODE_BADGE[mode] || NODEPOOL_MODE_BADGE.User)}>{mode}</span>
          } />
          ...
          <Property label="Priority" value={
            <span className={cn('badge badge-sm', priority === 'Spot' ? CAPACITY_TYPE_BADGE.spot : CAPACITY_TYPE_BADGE.regular)}>{priority}</span>
          } />
      ...
      {zones.length > 0 && (
        <Section title="Availability Zones" icon={Server}>
          <div className="flex flex-wrap gap-1">
            {zones.map((z: string) => (
              <span key={z} className="badge badge-sm bg-popover text-muted-foreground border-border">{z}</span>
            ))}
          </div>
        </Section>
      )}
        ...
        <Section title="Node Labels" icon={Settings}>
          <div className="flex flex-wrap gap-1">
            {Object.entries(labels).map(([k, v]) => (
              <span key={k} className="badge badge-sm bg-popover text-muted-foreground border-border text-[10px]">{k}={v as string}</span>
            ))}
          </div>
        </Section>
      )}
      ...
      {taints.length > 0 && (
        <Section title="Taints" icon={Settings}>
          <table className="w-full text-xs">
            <thead><tr className="text-muted-foreground/75"><th className="text-left font-medium py-1">Key</th><th className="text-left font-medium py-1">Value</th><th className="text-left font-medium py-1">Effect</th></tr></thead>
            <tbody>
              {taints.map((t: any, i: number) => (
                <tr key={i} className="border-t border-border">
                  <td className="py-1 text-muted-foreground">{t.key}</td>
                  <td className="py-1 text-muted-foreground">{t.value}</td>
                  <td className="py-1 text-muted-foreground">{t.effect}</td>
                </tr>
              ))}
            </tbody>
          </table>
