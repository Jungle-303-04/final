  ...
  return (
    <>
      ...
      {connectState === 'error' && connectError && (
        <p className="text-xs text-destructive px-3 pb-2">{connectError}</p>
      )}
      {downloadState === 'error' && downloadError && (
        <p className="text-xs text-destructive px-3 pb-2">{downloadError}</p>
      )}
      ...
      {topology.class && (
        <Section title="Topology" icon={Layers}>
          ...
          {topology.workers?.machineDeployments?.length > 0 && (
            <div className="mt-2">
              <div className="text-xs font-medium text-muted-foreground mb-1">Worker MachineDeployments</div>
                ...
                <tbody>
                  {topology.workers.machineDeployments.map((md: any, i: number) => (
                    <tr key={i} className="border-t border-border">
                      <td className="py-1 text-muted-foreground">{md.class}</td>
                      <td className="py-1 text-muted-foreground">{md.name || '-'}</td>
                      <td className="py-1 text-muted-foreground">{md.replicas ?? '-'}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
