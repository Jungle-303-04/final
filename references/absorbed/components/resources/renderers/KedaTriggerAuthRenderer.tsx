            ...
            {secretRefs.map((ref, i) => (
              <div key={i} className="card-inner">
                <div className="flex flex-wrap gap-x-4 gap-y-1 text-sm">
                  <span className="text-foreground font-medium">{ref.parameter}</span>
        ...
        <Section title={`Environment Variables (${envVars.length})`} defaultExpanded>
          <div className="space-y-2">
            {envVars.map((env, i) => (
              <div key={i} className="card-inner">
                <div className="flex flex-wrap gap-x-4 gap-y-1 text-sm">
                  <span className="text-foreground font-medium">{env.parameter}</span>
                  <span className="text-muted-foreground">Env: {env.name}</span>
                  {env.containerName && (
                    <span className="text-muted-foreground/75">Container: {env.containerName}</span>
                  )}
                </div>
              </div>
            ))}
        ...
        <Section title="HashiCorp Vault" icon={Server}>
          ...
          {vault.secrets && vault.secrets.length > 0 && (
            <div className="mt-2 space-y-1">
              <div className="text-xs font-medium text-muted-foreground uppercase tracking-wider mb-1">Secrets</div>
              {vault.secrets.map((s: any, i: number) => (
                <div key={i} className="card-inner text-sm">
                  <span className="text-foreground font-medium">{s.parameter}</span>
                  <span className="text-muted-foreground ml-2">{s.path} / {s.key}</span>
                </div>
              ))}
            </div>
          )}
          ...
          {gcpSecretManager.secrets && gcpSecretManager.secrets.length > 0 && (
            <div className="mt-2 space-y-1">
              ...
              {gcpSecretManager.secrets.map((s: any, i: number) => (
                <div key={i} className="card-inner text-sm">
                  <span className="text-foreground font-medium">{s.parameter}</span>
                  <span className="text-muted-foreground ml-2">{s.id}{s.version ? ` (v${s.version})` : ''}</span>
                </div>
              ))}
            </div>
          )}
