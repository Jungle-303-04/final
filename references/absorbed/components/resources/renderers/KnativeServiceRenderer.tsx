            ...
            {traffic.map((t: any, i: number) => (
              <div key={i} className="flex items-center gap-2 text-sm">
                <div className="flex items-center gap-1 w-16 shrink-0">
                  ...
                  <span className="text-muted-foreground font-medium text-xs">{t.percent || 0}%</span>
                </div>
                {t.revisionName ? (
                  <ResourceLink
                    name={t.revisionName}
                  ...
                  <span className="text-muted-foreground">{t.latestRevision ? '@latest' : '-'}</span>
                )}
                {t.tag && (
                  <span className="px-1.5 py-0.5 bg-accent rounded text-[10px] text-muted-foreground">
                    tag: {t.tag}
                  </span>
                )}
                {t.url && (
          ...
          <div className="space-y-2">
            {containers.map((c: any, i: number) => (
              <div key={i} className="card-inner text-sm">
                <div className="font-medium text-foreground">{c.name || 'container'}</div>
                <div className="text-xs text-muted-foreground truncate" title={c.image}>{c.image}</div>
