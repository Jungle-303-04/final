    ...
    <>
            ...
            {blockDevices.map((bd: any, i: number) => (
              <div key={i} className="card-inner">
                ...
                <div className="grid grid-cols-2 gap-x-4 gap-y-1 text-xs">
                  {bd.ebs?.volumeType && (
                    <div>
                      <span className="text-muted-foreground/75">Type: </span>
                      <span className="text-muted-foreground">{bd.ebs.volumeType}</span>
                    </div>
                  )}
                  {bd.ebs?.volumeSize && (
                    <div>
                      <span className="text-muted-foreground/75">Size: </span>
                      <span className="text-muted-foreground">{bd.ebs.volumeSize}</span>
                    </div>
                  )}
                  {bd.ebs?.iops && (
                    <div>
                      <span className="text-muted-foreground/75">IOPS: </span>
                      <span className="text-muted-foreground">{bd.ebs.iops}</span>
                    </div>
                  )}
                  {bd.ebs?.throughput && (
                    <div>
                      <span className="text-muted-foreground/75">Throughput: </span>
                      <span className="text-muted-foreground">{bd.ebs.throughput}</span>
                    </div>
                  )}
                  <div>
                    <span className="text-muted-foreground/75">Encrypted: </span>
                    <span className="text-muted-foreground">{bd.ebs?.encrypted ? 'Yes' : 'No'}</span>
                  </div>
                  {bd.ebs?.deleteOnTermination !== undefined && (
                    <div>
                      <span className="text-muted-foreground/75">Delete on Termination: </span>
                      <span className="text-muted-foreground">{bd.ebs.deleteOnTermination ? 'Yes' : 'No'}</span>
                    </div>
                  )}
                </div>
              </div>
      ...
      <Section title="Network" icon={Network}>
        {subnetTerms.length > 0 && (
          <div className="mb-3">
            <div className="text-xs text-muted-foreground/75 font-medium mb-1">Subnet Selector</div>
            {subnetTerms.map((term: any, i: number) => (
              <div key={i} className="card-inner mb-1">
                {term.tags && Object.entries(term.tags).map(([key, val]) => (
                  <span key={key} className="badge-sm mr-1 mb-1 bg-accent text-muted-foreground">
                    {key}: {String(val)}
                  </span>
                ))}
                {term.id && (
        ...
        {sgTerms.length > 0 && (
          <div>
            <div className="text-xs text-muted-foreground/75 font-medium mb-1">Security Group Selector</div>
        ...
        <Section title={`Resolved AMIs (${statusAMIs.length})`} icon={Image} defaultExpanded>
          <div className="space-y-2">
            {statusAMIs.map((ami: any, i: number) => (
              <div key={i} className="card-inner">
                <div className="text-sm font-medium text-foreground mb-1">{ami.id || '(unknown AMI)'}</div>
                {ami.name && (
                  <div className="text-xs text-muted-foreground/75 mb-1">{ami.name}</div>
                )}
                {ami.requirements && ami.requirements.length > 0 && (
                  <div className="flex flex-wrap gap-1">
                    {ami.requirements.map((req: any, ri: number) => (
                      <span key={ri} className="badge-sm bg-accent text-muted-foreground">
        ...
        <Section title={`Resolved Subnets (${statusSubnets.length})`} icon={Network} defaultExpanded>
          <div className="flex flex-wrap gap-1">
            {statusSubnets.map((subnet: any, i: number) => (
              <span key={i} className="badge-sm bg-accent text-muted-foreground">
