...
export function IngressRouteCell({ resource, column }: { resource: any; column: string }) {
  switch (column) {
    case 'entrypoints': {
      ...
      return <span className="text-sm text-muted-foreground">{eps}</span>
    }
    case 'hosts': {
    ...
    case 'targets': {
      ...
      return (
        <Tooltip content={targets}>
          <span className="text-sm text-muted-foreground truncate">{targets}</span>
        </Tooltip>
      )
    }
    default:
...
export function ServersTransportCell({ resource, column }: { resource: any; column: string }) {
  switch (column) {
    case 'insecure': {
      ...
      return (
        <span className={`text-sm ${insecure ? 'text-warning' : 'text-muted-foreground'}`}>
          {insecure ? 'Yes' : 'No'}
        </span>
      )
    }
    case 'serverName': {
      ...
      return <span className="text-sm text-muted-foreground truncate">{name || '-'}</span>
    }
    default:
      return <span className="text-sm text-muted-foreground/75">-</span>
  }
