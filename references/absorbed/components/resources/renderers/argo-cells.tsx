...
export function ArgoApplicationCell({ resource, column }: { resource: any; column: string }) {
  switch (column) {
    case 'project': {
      ...
      return <span className="text-sm text-muted-foreground">{project}</span>
    }
    case 'sync': {
    ...
    case 'repo': {
      ...
      return (
        <Tooltip content={resource.spec?.source?.repoURL || resource.spec?.sources?.[0]?.repoURL || repo}>
          <span className="text-sm text-muted-foreground truncate block">{repo}</span>
        </Tooltip>
      )
    }
    default:
    ...
    case 'generators': {
      ...
      return (
        <Tooltip content={generators}>
          <span className="text-sm text-muted-foreground truncate block">{generators}</span>
        </Tooltip>
      )
    }
    case 'template': {
      const template = getArgoApplicationSetTemplate(resource)
      return <span className="text-sm text-muted-foreground">{template}</span>
    }
    case 'applications': {
