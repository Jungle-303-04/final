      ...
      <Section title="Summary">
        ...
        {total > 0 && (
          <div className="h-3 rounded overflow-hidden flex">
            {passCount > 0 && <div className="h-full bg-success" style={{ width: `${(passCount / total) * 100}%` }} />}
            {failCount > 0 && <div className="h-full bg-destructive" style={{ width: `${(failCount / total) * 100}%` }} />}
          </div>
        )}
      </Section>
      ...
      {sortedChecks.length > 0 && (
        <Section title="Controls">
          ...
          {expandedSection && (
            ...
            <div className="flex items-center gap-2 mb-2">
              <div className="relative flex-1">
                ...
                <Input
                  ...
                  className="w-full pl-6 pr-2 py-1 text-xs bg-background border border-border rounded text-foreground placeholder:text-muted-foreground/75 focus:outline-none focus:ring-1 focus:ring-blue-500/50"
                />
              </div>
              <button
                ...
                className={cn(
                  ...
                  showFailedOnly
                    ...
                    : 'bg-background border-border text-muted-foreground/75 hover:text-muted-foreground'
                )}
              >
            ...
            {hasActiveFilter && (
              <div className="flex items-center gap-1 mb-2 text-xs text-muted-foreground/75">
            ...
            <div className="space-y-0.5">
                ...
                return (
                  <div key={check.id}>
                    ...
                    >
                      ...
                      <span className={cn('px-1.5 py-0.5 rounded text-[10px] font-medium shrink-0', SEVERITY_BADGE_COLORS[check.severity] || BADGE_INACTIVE)}>
                        {check.severity || '-'}
                      </span>
                      <span className="text-xs text-muted-foreground truncate flex-1">{check.name || check.id}</span>
                    ...
                    {isExpanded && controlDef && (
                      <div className="ml-6 mr-1 mb-2 p-2.5 bg-popover/50 rounded border border-border/50 space-y-2">
                        <div className="text-[10px] font-mono text-muted-foreground/75">{check.id}</div>
                        {controlDef.description && (
                          <div>
                            ...
                            <div className="text-xs text-muted-foreground">{controlDef.description}</div>
                          </div>
                        )}
                        {controlDef.checks?.length > 0 && (
                          <div>
                            <div className="text-[10px] font-medium text-muted-foreground/75 uppercase tracking-wider mb-0.5">Check IDs</div>
                            <div className="flex flex-wrap gap-1">
                              {controlDef.checks.map((c: any, i: number) => (
                                <span key={i} className="text-[10px] font-mono text-muted-foreground/75 bg-card px-1.5 py-0.5 rounded">{c.id}</span>
                              ))}
                            </div>
                          </div>
                        )}
              ...
              {filteredChecks.length === 0 && (
                <div className="py-4 text-center text-xs text-muted-foreground/75">
