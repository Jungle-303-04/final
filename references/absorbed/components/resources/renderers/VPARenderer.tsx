      ...
      <Section title="Configuration" icon={Cpu}>
        <PropertyList>
          ...
          <Property label="Update Mode" value={
            <span className={cn(
              updateMode === 'Auto' && 'text-success',
              updateMode === 'Recreate' && 'text-warning',
              updateMode === 'Initial' && 'text-blue-400',
              updateMode === 'Off' && 'text-muted-foreground/75',
            )}>
              {updateMode}
            </span>
          } />
          {isOff && (
            <Property label="" value={
              <span className="text-xs text-muted-foreground/75">Recommendation-only mode — no automatic updates</span>
            } />
          )}
        </PropertyList>
      </Section>
          ...
          <div className="space-y-3">
            {recommendations.map((rec: any) => (
              <div key={rec.containerName} className="card-inner-lg">
                <div className="text-sm font-medium text-foreground mb-2">{rec.containerName}</div>
                <div className="overflow-x-auto">
                  <table className="w-full text-xs">
                    ...
                    <tbody>
                      <tr>
                        <td className="py-1 pr-3 text-muted-foreground">Target</td>
                        <td className="text-right py-1 px-2 text-success font-medium">{formatResource(rec.target?.cpu)}</td>
                        <td className="text-right py-1 px-2 text-success font-medium">{formatResource(rec.target?.memory)}</td>
                      </tr>
                      <tr>
                        <td className="py-1 pr-3 text-muted-foreground">Lower Bound</td>
                        <td className="text-right py-1 px-2 text-muted-foreground">{formatResource(rec.lowerBound?.cpu)}</td>
                        <td className="text-right py-1 px-2 text-muted-foreground">{formatResource(rec.lowerBound?.memory)}</td>
                      </tr>
                      <tr>
                        <td className="py-1 pr-3 text-muted-foreground">Upper Bound</td>
                        <td className="text-right py-1 px-2 text-muted-foreground">{formatResource(rec.upperBound?.cpu)}</td>
                        <td className="text-right py-1 px-2 text-muted-foreground">{formatResource(rec.upperBound?.memory)}</td>
                      </tr>
                      {rec.uncappedTarget && (
                        <tr>
                          <td className="py-1 pr-3 text-muted-foreground/75">Uncapped</td>
                          <td className="text-right py-1 px-2 text-muted-foreground/75">{formatResource(rec.uncappedTarget?.cpu)}</td>
                          <td className="text-right py-1 px-2 text-muted-foreground/75">{formatResource(rec.uncappedTarget?.memory)}</td>
                        </tr>
                      )}
                    </tbody>
                  </table>
            ...
            {containerPolicies.map((policy: any, i: number) => (
              <div key={i} className="card-inner-lg">
                <div className="flex items-center gap-2 mb-2">
                  <span className="text-sm font-medium text-foreground">{policy.containerName || '*'}</span>
                  {policy.mode && (
                    <span className={cn(
                      'px-1.5 py-0.5 rounded text-[10px] font-medium',
                      policy.mode === 'Off' ? 'bg-accent text-muted-foreground/75' : 'bg-blue-500/20 text-blue-400'
                    )}>
                      {policy.mode}
                    </span>
                  )}
