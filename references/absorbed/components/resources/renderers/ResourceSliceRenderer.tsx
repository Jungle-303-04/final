    ...
    <>
      ...
      <Section title={`Devices (${devices.length})`} icon={Cpu} defaultExpanded>
        {devices.length > 0 ? (
            ...
            {visible.map((d: any, i: number) => {
              ...
              return (
                <div key={d?.name || i} className="card-inner">
                  <div className="text-sm text-foreground font-medium">{d?.name || `device-${i}`}</div>
          ...
          <div className="text-sm text-muted-foreground/75">No devices published in this slice</div>
        )}
      </Section>
    </>
  )
