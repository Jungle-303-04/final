...
export function MetricsUnavailableNotice({ rawError, diagnosis }: MetricsUnavailableNoticeProps) {
  return (
    <div className="card-inner-lg text-xs text-muted-foreground/75">
      <div className="flex items-center gap-1.5">
        <span className="min-w-0 leading-5">
          Metrics unavailable. Opsia cannot read metrics.k8s.io.
        </span>
        {rawError && (
          <Tooltip
            content={(
          ...
          >
            <button
              type="button"
              className="inline-flex shrink-0 cursor-help items-center gap-1 text-xs font-medium leading-5 text-muted-foreground/75 hover:text-muted-foreground"
