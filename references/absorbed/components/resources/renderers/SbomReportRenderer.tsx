        ...
        <Section title="Components">
          ...
          {expanded && (
            <div className="overflow-x-auto -mx-1">
              ...
              {hasActiveFilter && (
                <div className="flex items-center gap-1 mb-2 px-1 text-xs text-muted-foreground/75">
              ...
              <table className="w-full text-xs">
                <thead>
                  <tr className="border-b border-border text-muted-foreground/75">
                ...
                <tbody>
                  {displayedComponents.map((comp: any, i: number) => {
                    ...
                    return (
                      <tr key={`${comp.name}-${comp.version}-${i}`} className="border-b border-border/50 hover:bg-accent/50">
                        <td className="py-1.5 px-1 text-muted-foreground max-w-[200px] truncate" title={comp.purl || comp.name}>{comp.name || '-'}</td>
                        <td className="py-1.5 px-1 text-muted-foreground font-mono">{comp.version || '-'}</td>
                        <td className="py-1.5 px-1 text-muted-foreground/75">{comp.type || '-'}</td>
                        <td className="py-1.5 px-1 text-muted-foreground/75 max-w-[120px] truncate" title={license}>{license}</td>
                      </tr>
                    )
                  })}
                </tbody>
