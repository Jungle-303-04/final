      ...
      <Section title="Overview" icon={Settings} defaultExpanded>
        <PropertyList>
          ...
          {imageDigest && <Property label="Image Digest" value={
            <span className="text-xs font-mono text-muted-foreground break-all">{imageDigest}</span>
          } />}
          {routingState && <Property label="Routing" value={
            <span className={cn(
              'badge',
              routingState === 'active' ? 'bg-success/20 text-success' : 'bg-accent text-muted-foreground'
            )}>
              {routingState}
            </span>
          } />}
