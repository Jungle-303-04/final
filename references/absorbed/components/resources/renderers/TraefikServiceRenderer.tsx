      ...
      {weightPct !== undefined && (
        <div className="flex items-center gap-1.5 ml-auto">
          <div className="w-20 h-1.5 bg-accent rounded-full overflow-hidden">
            <div className="h-full bg-primary" style={{ width: `${Math.min(100, weightPct)}%` }} />
          </div>
          <span className="text-muted-foreground/75 tabular-nums w-10 text-right">{weightPct.toFixed(0)}%</span>
        </div>
      )}
    </div>
  )
      ...
      {mirroring && (
        <Section title="Mirroring" icon={Copy} defaultExpanded>
          <div className="space-y-2">
            <div>
              <div className="text-[10px] font-medium text-muted-foreground/75 uppercase tracking-wider mb-1">Primary</div>
              <ServiceRow svc={mirroring} ns={ns} onNavigate={onNavigate} />
            </div>
            {(mirroring.mirrors || []).length > 0 && (
              <div>
                <div className="text-[10px] font-medium text-muted-foreground/75 uppercase tracking-wider mb-1">Mirrors</div>
                <div className="space-y-1">
                  {mirroring.mirrors.map((m: any, i: number) => (
                    <div key={i} className="flex items-center gap-2 text-xs">
                      <ServiceRow svc={m} ns={ns} onNavigate={onNavigate} />
                      {m.percent !== undefined && (
                        <span className="text-muted-foreground/75">{m.percent}%</span>
                      )}
                    </div>
                  ))}
                </div>
