          ...
          {rules.map((rule: any, i: number) => (
            <div key={i} className="card-inner-lg">
              <div className="flex items-center gap-2 mb-2">
                ...
                <span className="text-sm font-medium text-foreground">{rule.host || '*'}</span>
              </div>
              <div className="space-y-1">
                {rule.http?.paths?.map((path: any) => (
                  <div key={path.path || '/'} className="text-xs text-muted-foreground flex items-center gap-2">
                    <span className="text-muted-foreground/75">{path.pathType || 'Prefix'}:</span>
                    <span>{path.path || '/'}</span>
          ...
          <div className="space-y-2">
            {tls.map((t: any) => (
              <div key={t.secretName} className="text-sm">
                <div className="text-muted-foreground">Secret: {t.secretName ? (
                  <ResourceLink name={t.secretName} kind="secrets" namespace={data.metadata?.namespace || ''} onNavigate={onNavigate} />
                ) : '-'}</div>
                <div className="text-xs text-muted-foreground/75">Hosts: {t.hosts?.join(', ') || '*'}</div>
              </div>
            ))}
          </div>
        </Section>
