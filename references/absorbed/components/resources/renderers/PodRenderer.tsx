...
function getRestartRecency(finishedAt: string | undefined, restarts: number): { color: string; label: string | null } {
  ...
  if (ms !== null) {
    let color = 'text-muted-foreground/75'
    if (ms < 10 * 60 * 1000) color = 'text-destructive'
    else if (ms < 60 * 60 * 1000) color = 'text-warning'
    const label = formatMsAgo(finishedAt!)
    return { color, label }
  }

  return { color: restarts > 5 ? 'text-destructive' : 'text-warning', label: null }
}
...
function SecretValueCell({ value }: { value: string }) {
  ...
  return (
    <span className="flex items-center gap-1 min-w-0">
      <span className={cn('break-all text-warning dark:text-warning/80', !revealed && 'blur-[3px] select-none')}>
        {value || '•••'}
      </span>
      <button
        onClick={() => setRevealed(r => !r)}
        className="shrink-0 text-muted-foreground/75 hover:text-warning dark:hover:text-warning transition-colors"
...
function EnvRowShell({ name, children }: { name: string; children: ReactNode }) {
  return (
    <div className="flex items-start gap-1 text-xs font-mono py-0.5 border-b border-border/30 last:border-0">
      <span className="text-muted-foreground shrink-0 break-all">{name}</span>
...
function resolveEnvValueNode(env: any): JSX.Element {
  if (env.valueFrom?.secretKeyRef) {
    ...
    return <span className="text-warning dark:text-warning/60 text-[10px] shrink-0 self-center px-1 py-0.5 bg-warning/10 dark:bg-warning/10 rounded">secret:{name}[{key}]</span>
  }
  ...
  return <span className="text-foreground break-all min-w-0">{env.value ?? ''}</span>
}
        ...
        {allContainers.map((container: any) => {
          ...
          return (
            <div key={container.name}>
              ...
              {multiContainer && (
                <div className="text-xs font-medium text-muted-foreground/75 mb-2 uppercase tracking-wide">
                  {container.name}
                </div>
              )}
                    ...
                    <div key={i} className="mb-1">
                      <div className="flex items-center gap-1.5 text-xs font-mono py-0.5">
                        <span className={cn(
                          ...
                          isSecret ? 'bg-warning/10 dark:bg-warning/10 text-warning dark:text-warning' : 'bg-blue-500/10 text-blue-700 dark:text-blue-400/80'
                        )}>
                          {prefix}
                        </span>
                        <span className="text-muted-foreground">{sourceName}</span>
                        {!resolved && <span className="text-muted-foreground/75">(all keys)</span>}
                      </div>
          ...
          <ul className="text-xs space-y-1">
            {podProblems.map((p, i) => (
              <li key={i} className="flex items-start gap-1.5">
                <span className={cn('w-1.5 h-1.5 rounded-full shrink-0 mt-1', SEVERITY_DOT_COLOR[p.severity])} />
                <span className="min-w-0 break-words text-destructive dark:text-destructive">
                  {p.message}
      ...
      <Section title="Status" icon={Server}>
        <PropertyList>
          {(() => {
            ...
            const node = (
              <span className={cn(healthColors[phaseDisplay.level])}>
                {phaseDisplay.text}
              </span>
            )
            return (
          ...
          <Property
            label={
              <Tooltip
                ...
                position="right"
              >
                <span className="border-b border-dotted border-muted-foreground/75 cursor-help">QoS Class</span>
              </Tooltip>
            }
        ...
        <Section title={`Init Containers (${initContainers.length})`} icon={CirclePlay} defaultExpanded>
          <div className="space-y-3">
            {initContainers.map((container: any, index: number) => {
              ...
              return (
                <div key={container.name} className={cn(
                  'rounded-lg p-3 border-l-2',
                  isCompleted ? 'bg-popover/20 border-success/40' :
                  isFailed ? 'bg-popover/30 border-destructive/50' :
                  isInitRunning ? 'bg-popover/30 border-blue-500/50' :
                  'bg-popover/30 border-warning/40'
                )}>
                  <div className="flex items-center justify-between mb-2">
                    <div className="flex items-center gap-2">
                      <span className="text-[10px] font-mono text-muted-foreground/75 bg-popover rounded px-1.5 py-0.5">
                  ...
                  <div className="text-xs text-muted-foreground space-y-1">
                    ...
                    {command && (
                      <div className="text-muted-foreground/75 font-mono break-all">
                        $ {command}
                      </div>
                    )}
                    {restarts > 0 && (
                      <div className={restarts > 5 ? 'text-destructive' : 'text-warning'}>
                        Restarts: {restarts}
                      </div>
                    )}
                    ...
                    {isWaiting && state?.waiting?.reason && state.waiting.reason !== 'PodInitializing' && (
                      <div className="text-destructive flex items-center gap-1">
                        ...
                        {state.waiting.message && (
                          <span className="text-muted-foreground/75 truncate" title={state.waiting.message}>
                    ...
                    {isFailed && (
                      <div className="text-destructive flex items-center gap-1">
                        ...
                        {state?.terminated?.message && (
                          <span className="text-muted-foreground/75 truncate" title={state.terminated.message}>
              ...
              <div key={container.name} className="card-inner-lg">
                ...
                <div className="text-xs text-muted-foreground space-y-1">
                  ...
                  {currentWaiting?.reason && currentWaiting.reason !== 'ContainerCreating' && (
                    <div className="text-destructive flex items-center gap-1">
                      ...
                      {currentWaiting.message && (
                        <span className="text-muted-foreground/75 truncate" title={currentWaiting.message}>
                  ...
                  {currentTerminated?.reason && (
                    <div className={cn('flex items-center gap-1', terminatedOk ? 'text-sky-500 dark:text-sky-400' : 'text-destructive')}>
                      ...
                      {currentTerminated.exitCode !== undefined && currentTerminated.exitCode !== 0 && (
                        <span className="text-muted-foreground/75">(exit code {currentTerminated.exitCode})</span>
                      )}
                    </div>
                  )}
                  {/* Show last termination info if container restarted */}
                  {lastTermination && restarts > 0 && !currentTerminated && (
                    <div className="text-warning/80 space-y-0.5">
                      <div className="flex items-center gap-1">
                        ...
                        {lastTermination.exitCode !== undefined && lastTermination.exitCode !== 0 && (
                          <span className="text-muted-foreground/75">(code {lastTermination.exitCode})</span>
                        )}
                        {lastTermination.reason === 'OOMKilled' && container.resources?.limits?.memory && (
                          <span className="text-muted-foreground/75">— limit: {container.resources.limits.memory}</span>
                        )}
                      </div>
                      {lastTermination.finishedAt && (() => {
                        ...
                        const runDur = lastTermination.startedAt
                          ...
                          : null
                        return (
                          <div className="text-muted-foreground/75 flex items-center gap-1">
                      ...
                      {container.ports.map((p: any) => (
                        canPortForward && renderPortAction ? (
                          <span key={`${p.name || ''}-${p.containerPort}-${p.protocol || 'TCP'}`} className="inline-flex items-center gap-1">
                            {p.name && <span className="text-muted-foreground/75">{p.name}:</span>}
                            {renderPortAction({
                              namespace,
                              podName,
                          ...
                          <span key={`${p.name || ''}-${p.containerPort}-${p.protocol || 'TCP'}`} className="text-muted-foreground/75">
                ...
                return (
                  <div key={historyContainer.name} className="card-inner-lg">
                    <div className="flex items-center justify-between mb-3">
                      <span className="text-sm font-medium text-foreground">{historyContainer.name}</span>
                    </div>

                    {dataPoints && dataPoints.length > 0 ? (
                      <div className="grid grid-cols-2 gap-6">
                        <div>
                          <div className="text-xs text-muted-foreground/75 mb-2">CPU</div>
                          <MetricsChart
                        ...
                        <div>
                          <div className="text-xs text-muted-foreground/75 mb-2">Memory</div>
                          <MetricsChart
                            dataPoints={dataPoints}
                            type="memory"
                            height={80}
                      ...
                      <div className="grid grid-cols-2 gap-4 text-xs">
                        <div>
                          <div className="text-muted-foreground/75 mb-1">CPU</div>
                          <div className="flex items-baseline gap-1">
                            <span className="text-sm font-medium text-blue-400">{currentContainerMetrics.usage.cpu}</span>
                            {limits?.cpu && (
                              <span className="text-muted-foreground/75">/ {limits.cpu} limit</span>
                            )}
                          </div>
                        </div>
                        <div>
                          <div className="text-muted-foreground/75 mb-1">Memory</div>
                          <div className="flex items-baseline gap-1">
                            <span className="text-sm font-medium text-purple-400">{currentContainerMetrics.usage.memory}</span>
                            {limits?.memory && (
                              <span className="text-muted-foreground/75">/ {limits.memory} limit</span>
                            )}
                          </div>
                        </div>
                      </div>
                    ) : (
                      <div className="text-xs text-muted-foreground/75">Collecting metrics data...</div>
                    )}
                  </div>
                )
              })}
...
function PodPermissionsSection({
  ...
  if (loading) {
    return (
      <Section title={title} icon={Shield}>
        <div className="text-sm text-muted-foreground/75">Loading RBAC graph…</div>
      </Section>
    )
  }
  if (error) {
  ...
  return (
    <Section title={title} icon={Shield} defaultExpanded={hasBlastRadius}>
      ...
      {/* One-line summary */}
      <div className="text-xs text-muted-foreground/75 mb-3">
...
function PodRulePreviewLine({ rule }: { rule: RBACPolicyRule }) {
  ...
  return (
    <div className="flex items-center gap-1 flex-wrap text-xs">
      {verbs.map((v) => (
        <span key={v} className={cn('badge', rbacVerbBadgeClass(v))}>{v}</span>
      ))}
      <span className="text-muted-foreground">on</span>
      {isNonResource ? (
        nonResourceURLs.map((u) => (
          <span key={u} className="badge font-mono bg-popover text-muted-foreground">{u}</span>
        ))
      ) : (
        resources.map((r) => (
          <span key={r} className={cn('badge', rbacResourceBadgeClass)}>
            {r === '*' ? '*' : r}
          </span>
        ))
      )}
      {!isNonResource && groups.length > 0 && groups.some((g) => g !== '') && (
        <>
          ...
          {groups.map((g) => (
            <span key={g} className={cn('badge', rbacApiGroupBadgeClass)}>
              {g === '' ? 'core' : g}
            </span>
          ))}
        </>
