        ...
        <AlertBanner variant="error" title="Issues Detected" items={problems}>
          <div className="flex items-center justify-between mt-2">
            ...
            {onViewPods && (
              <button
                onClick={onViewPods}
                className="flex items-center gap-1 px-2 py-1 text-xs font-medium text-destructive hover:text-destructive bg-destructive/10 hover:bg-destructive/20 border border-destructive/30 rounded transition-colors"
              >
        ...
        <div className="mt-3 pt-3 border-t border-border flex items-center gap-2">
          ...
          {isScalable ? (
            <button
              ...
              className="flex items-center gap-1.5 px-3 py-1.5 text-xs font-medium text-success hover:text-success bg-success/10 hover:bg-success/20 border border-success/30 rounded transition-colors"
            >
            ...
            <Tooltip content={scaleBlockedReason}>
              <button
                ...
                className="flex items-center gap-1.5 px-3 py-1.5 text-xs font-medium text-muted-foreground/75 bg-popover border border-border rounded cursor-not-allowed"
              >
                <Scale className="w-3 h-3" />
                Scale
              </button>
      ...
      <DialogPortal open={showScaleDialog} onClose={() => setShowScaleDialog(false)} className="w-80 p-4">
        ...
        <div className="flex items-center justify-center gap-4 mb-4">
          <button
            ...
            className="p-2 rounded-lg bg-popover hover:bg-accent text-muted-foreground hover:text-foreground transition-colors"
          ...
          <input
            ...
            className="w-20 text-center text-2xl font-semibold bg-popover border border-border rounded-lg py-2 text-foreground focus:outline-none focus:border-blue-500"
            autoFocus
          />
        ...
        <div className="text-xs text-muted-foreground/75 text-center mb-4">
        ...
        <div className="flex gap-2">
          <button
            ...
            className="flex-1 px-3 py-2 text-sm text-muted-foreground hover:text-foreground bg-popover hover:bg-accent border border-border rounded-lg transition-colors"
          >
            Cancel
          </button>
          <button
...
function ScalerDiagnosisRow({ entry }: { entry: ScalerDiagnosis }) {
  if (entry.loading) {
    return <div className="text-xs text-muted-foreground/75">Loading autoscaler diagnosis…</div>
  }
  if (entry.error) {
    return <div className="text-xs text-muted-foreground/75">Autoscaler diagnosis unavailable</div>
  }
  if (!entry.diagnosis) return null
  return (
    <div className="rounded border border-border bg-card px-2 py-1.5 text-xs leading-6 text-muted-foreground whitespace-normal break-normal">
  ...
  if (loading) {
    return (
      <Section title={title} icon={Shield}>
        <div className="text-sm text-muted-foreground">Loading RBAC graph…</div>
      </Section>
    )
  }
  if (error) {
  ...
  return (
    <div className="flex items-center gap-1 flex-wrap text-xs">
      ...
      {isNonResource ? (
        ...
        resources.map((r) => (
          <span key={r} className={cn('badge', rbacResourceBadgeClass)}>{r === '*' ? '*' : r}</span>
        ))
      )}
      {!isNonResource && groups.length > 0 && groups.some((g) => g !== '') && (
        <>
          ...
          {groups.map((g) => (
            <span key={g} className={cn('badge', rbacApiGroupBadgeClass)}>{g === '' ? 'core' : g}</span>
          ))}
        </>
      )}
    </div>
