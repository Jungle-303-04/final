      ...
      <Section title="Status" icon={Clock} defaultExpanded>
        <PropertyList>
          <Property label="Phase" value={
            <span className={cn('badge', bslStatus.color)}>
              {bslStatus.text}
            </span>
          } />
          ...
          {status.lastSyncedRevision && (
            <Property label="Last Synced Revision" value={
              <span className="text-sm font-mono text-muted-foreground break-all">{status.lastSyncedRevision}</span>
            } />
          )}
        </PropertyList>
      </Section>
      ...
      <Section title="Provider" icon={HardDrive} defaultExpanded>
        ...
        {Object.keys(config).length > 0 && (
          <div className="mt-2 pt-2 border-t border-border">
            <div className="text-xs font-medium text-muted-foreground uppercase tracking-wider mb-1">Config</div>
            <div className="flex flex-wrap gap-1">
              {Object.entries(config).map(([k, v]) => (
                <span key={k} className="badge-sm bg-accent text-muted-foreground">
                  {k}: {String(v)}
                </span>
              ))}
            </div>
