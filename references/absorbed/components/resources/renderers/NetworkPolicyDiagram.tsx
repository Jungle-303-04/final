  ...
  return (
    <div className="card-inner-lg space-y-3">
      ...
      {!hasIngress && !hasEgress && (
        <div className="text-xs text-muted-foreground/75 text-center py-2">No policy types specified</div>
      )}
    </div>
  )
}
...
const PEER_STYLES = {
  pod: 'border-success/30 bg-success/8',
  namespace: 'border-sky-500/30 bg-sky-500/8',
  cidr: 'border-warning/30 bg-warning/8',
  combined: 'border-success/30 bg-success/8',
  all: 'border-border bg-popover/50',
  deny: 'border-destructive/30 bg-destructive/8',
} as const
  ...
  const sourceNodes = (
    <div className="min-w-0 max-w-full shrink space-y-1">
      {sources.map((peer, i) => (
        <div key={i}>
          {i > 0 && (
            <div className="text-[9px] text-muted-foreground/75 uppercase tracking-widest text-center my-0.5">or</div>
          )}
          <Tooltip content={peer.sublabel ? `${peer.label}\n${peer.sublabel}` : peer.label} position="top">
            ...
            >
              <div className="flex items-center gap-1.5 min-w-0">
                <span className={cn('w-1.5 h-1.5 rounded-full shrink-0', PEER_DOT[peer.type])} />
                <span className={cn('text-[11px] font-medium truncate min-w-0', denied && 'line-through text-destructive')}>
                  {peer.label}
                </span>
              </div>
              {peer.sublabel && (
                <div className="text-[10px] text-muted-foreground/75 ml-3 truncate">{peer.sublabel}</div>
              )}
            </div>
          </Tooltip>
        </div>
    ...
    <div className="flex flex-col items-center justify-center shrink-0 w-10">
      ...
      {ports.length > 0 && (
        <div className="flex flex-wrap justify-center gap-0.5 mt-0.5">
          {ports.map((p, i) => (
            <span key={i} className="text-[8px] text-muted-foreground/75 font-mono leading-none">{p}</span>
          ))}
        </div>
      )}
    </div>
