      ...
      <Section title="Scaling" icon={Cpu}>
        ...
        {fallback && (
          <div className="mt-2 pt-2 border-t border-border">
            <div className="text-xs font-medium text-muted-foreground uppercase tracking-wider mb-1">Fallback</div>
            <PropertyList>
            ...
            {triggers.map((trigger, i) => (
              <div key={i} className="card-inner">
                <div className="flex items-center gap-2 text-sm">
                  <span className="text-foreground font-medium">{trigger.type}</span>
                  {trigger.name && (
                    <span className="text-muted-foreground/75">({trigger.name})</span>
                  )}
