    ...
    <>
      ...
      <Section title="Namespace Selection" defaultExpanded>
        {explicitNamespaces.length > 0 ? (
          <div>
            <div className="text-xs text-muted-foreground mb-1.5">Explicit Namespaces</div>
            <div className="flex flex-wrap gap-1">
              {explicitNamespaces.map((ns: string) => (
                <span key={ns} className="badge bg-popover text-foreground">
                  {ns}
                </span>
              ))}
            </div>
          ...
          <div>
            <div className="text-xs text-muted-foreground mb-1.5">Label Selector</div>
            <div className="flex flex-wrap gap-1">
              {Object.entries(namespaceSelector).map(([k, v]) => (
                <span key={k} className="badge bg-popover text-muted-foreground">
                  {k}={v}
                </span>
              ))}
            </div>
          ...
          <div className="text-xs text-muted-foreground/75">All namespaces (no selector)</div>
        )}
      </Section>
        ...
        <Section title={`Provisioned Namespaces (${provisionedNamespaces.length})`} defaultExpanded={provisionedNamespaces.length <= 10}>
          <div className="flex flex-wrap gap-1">
            {provisionedNamespaces.map((ns: string) => (
              <span key={ns} className="badge bg-success/10 text-success">
                {ns}
              </span>
            ))}
          </div>
        ...
        <Section title={`Failed Namespaces (${failedNamespaces.length})`} defaultExpanded>
          <div className="space-y-1">
            {failedNamespaces.map((f, i) => (
              <div key={i} className="bg-destructive/10 rounded p-2 flex items-start gap-2">
                <AlertTriangle className="w-3.5 h-3.5 text-destructive mt-0.5 shrink-0" />
                <div className="text-xs">
                  <span className="text-destructive font-medium">{f.namespace}</span>
                  {f.reason && (
                    <span className="text-destructive/80 ml-1 break-all">{f.reason}</span>
                  )}
                </div>
              </div>
            ))}
