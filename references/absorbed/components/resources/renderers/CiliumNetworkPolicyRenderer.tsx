  ...
  return (
    <>
      <Section title="Target" icon={Shield}>
        ...
        <div className="mt-2">
          <div className="text-xs text-muted-foreground/75 mb-1">Endpoint Selector</div>
          <LabelSelectorDisplay selector={endpointSelector} emptyText="All endpoints in namespace" />
        </div>
        {spec.nodeSelector && (
          <div className="mt-2">
            <div className="text-xs text-muted-foreground/75 mb-1">Node Selector</div>
            <LabelSelectorDisplay selector={spec.nodeSelector} emptyText="All nodes" />
          </div>
        )}
        {enableDefaultDeny && (
          <div className="mt-2">
            <div className="text-xs text-muted-foreground/75 mb-1">Default Deny</div>
    ...
    <div className="card-inner-lg">
      {authentication && (
        <div className="mb-2 flex items-center gap-1.5">
          ...
          <span className="text-xs text-warning">Authentication: {authentication.mode || 'required'}</span>
        </div>
      )}

      {fromEndpoints.length > 0 && (
        <div className="mb-2">
          <div className="text-xs text-muted-foreground/75 mb-1">From Endpoints</div>
      ...
      {fromEntities.length > 0 && (
        <div className="mb-2">
          <div className="text-xs text-muted-foreground/75 mb-1">From Entities</div>
          <div className="flex flex-wrap gap-1">
      ...
      {(fromCIDR.length > 0 || fromCIDRSet.length > 0) && (
        <div className="mb-2">
          <div className="text-xs text-muted-foreground/75 mb-1">From CIDR</div>
          <div className="flex flex-wrap gap-1">
            {fromCIDR.map((c, j) => (
              <span key={j} className="badge bg-popover text-muted-foreground">{c}</span>
            ))}
            {fromCIDRSet.map((cs: any, j: number) => (
              <span key={`set-${j}`} className="badge bg-popover text-muted-foreground">{cs.cidr}</span>
            ))}
          </div>
        </div>
      )}
      ...
      {fromFQDNs.length > 0 && (
        <div className="mb-2">
          <div className="text-xs text-muted-foreground/75 mb-1">From FQDNs</div>
          <div className="flex flex-wrap gap-1">
      ...
      {fromNodes.length > 0 && (
        <div className="mb-2">
          <div className="text-xs text-muted-foreground/75 mb-1">From Nodes</div>
          <div className="space-y-1">
      ...
      {fromGroups.length > 0 && (
        <div className="mb-2">
          <div className="text-xs text-muted-foreground/75 mb-1">From Groups</div>
          <div className="flex flex-wrap gap-1">
      ...
      {toEndpoints.length > 0 && (
        <div className="mb-2">
          <div className="text-xs text-muted-foreground/75 mb-1">To Endpoints</div>
          <div className="space-y-1">
      ...
      {toEntities.length > 0 && (
        <div className="mb-2">
          <div className="text-xs text-muted-foreground/75 mb-1">To Entities</div>
          <div className="flex flex-wrap gap-1">
            {toEntities.map((e, j) => (
              <span key={j} className="badge bg-purple-500/20 text-purple-400 border-purple-500/30">{e}</span>
            ))}
      ...
      {(toCIDR.length > 0 || toCIDRSet.length > 0) && (
        <div className="mb-2">
          <div className="text-xs text-muted-foreground/75 mb-1">To CIDR</div>
          <div className="flex flex-wrap gap-1">
            {toCIDR.map((c, j) => (
              <span key={j} className="badge bg-popover text-muted-foreground">{c}</span>
            ))}
      ...
      {toFQDNs.length > 0 && (
        <div className="mb-2">
          <div className="text-xs text-muted-foreground/75 mb-1">To FQDNs</div>
          <div className="flex flex-wrap gap-1">
      ...
      {toServices.length > 0 && (
        <div className="mb-2">
          <div className="text-xs text-muted-foreground/75 mb-1">To Services</div>
          <div className="flex flex-wrap gap-1">
      ...
      {toNodes.length > 0 && (
        <div className="mb-2">
          <div className="text-xs text-muted-foreground/75 mb-1">To Nodes</div>
          <div className="space-y-1">
      ...
      {toGroups.length > 0 && (
        <div className="mb-2">
          <div className="text-xs text-muted-foreground/75 mb-1">To Groups</div>
          <div className="flex flex-wrap gap-1">
          ...
          <div className="flex flex-wrap gap-1">
            {toPorts.map((p: any, j: number) => {
              ...
              return ports.map((port: any, k: number) => (
                <span key={`${j}-${k}`} className="badge bg-popover text-muted-foreground">
      ...
      {icmps.length > 0 && (
        <div className="mb-2">
          <div className="text-xs text-muted-foreground/75 mb-1">ICMP</div>
          <div className="flex flex-wrap gap-1">
      ...
      {!hasRules && (
        <div className="text-xs text-muted-foreground/75">Allow all</div>
      )}
    </div>
  )
}
...
function L7Rules({ rules, ports }: { rules: any; ports?: any[] }) {
  ...
  return (
    <div className="card-inner text-xs space-y-2">
      {portLabel && (
        <div className="text-muted-foreground/75">L7 rules for {portLabel}</div>
      )}
      {httpRules.length > 0 && (
        <div>
          <div className="text-muted-foreground/75 mb-1">HTTP</div>
          <div className="space-y-1">
            {httpRules.map((r: any, i: number) => (
              <div key={i} className="flex flex-wrap items-center gap-1">
                ...
                {r.path && (
                  <span className="text-muted-foreground font-mono">{r.path}</span>
                )}
                {r.headers && r.headers.map((h: any, hi: number) => (
                  <span key={hi} className="badge badge-sm bg-popover text-muted-foreground">
      ...
      {dnsRules.length > 0 && (
        <div>
          <div className="text-muted-foreground/75 mb-1">DNS</div>
          <div className="flex flex-wrap gap-1">
            {dnsRules.map((r: any, i: number) => (
              <span key={i} className="badge badge-sm bg-popover text-muted-foreground">
      ...
      {kafkaRules.length > 0 && (
        <div>
          <div className="text-muted-foreground/75 mb-1">Kafka</div>
          <div className="space-y-1">
            {kafkaRules.map((r: any, i: number) => (
              <div key={i} className="flex flex-wrap gap-1">
                {r.apiKey && (
                  <span className="badge badge-sm bg-popover text-muted-foreground">
                    {r.apiKey}
                  </span>
                )}
                {r.topic && (
