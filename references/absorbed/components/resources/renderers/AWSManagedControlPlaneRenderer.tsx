            ...
            <tbody>
              {subnets.map((s, i) => (
                <tr key={i} className="border-t border-border">
                  <td className="py-1 text-muted-foreground font-mono text-[10px]">{s.id}</td>
                  <td className="py-1 text-muted-foreground">{s.az}</td>
                  <td className="py-1">
                  ...
                  <td className="py-1 text-muted-foreground font-mono text-[10px]">{s.cidrBlock}</td>
                </tr>
              ))}
            </tbody>
          </table>
          ...
          <table className="w-full text-xs">
            ...
            <tbody>
              {securityGroups.map((sg, i) => (
                <tr key={i} className="border-t border-border">
                  <td className="py-1 text-muted-foreground font-medium">{sg.role}</td>
                  <td className="py-1 text-muted-foreground font-mono text-[10px]">{sg.id}</td>
                  <td className="py-1 text-muted-foreground text-[10px] break-all">{sg.name}</td>
                </tr>
              ))}
            </tbody>
          </table>
      ...
      {natIPs.length > 0 && (
        <Section title="NAT Gateways" icon={Network}>
          <div className="flex flex-wrap gap-1">
            {natIPs.map((ip, i) => (
              <span key={i} className="badge badge-sm bg-popover text-muted-foreground border-border font-mono text-[10px]">{ip}</span>
            ))}
          </div>
        </Section>
      )}
        ...
        <Section title="EKS Addons" icon={Package}>
          <table className="w-full text-xs">
            ...
            <tbody>
              {addons.map((a, i) => (
                <tr key={i} className="border-t border-border">
                  <td className="py-1 text-muted-foreground font-medium">{a.name}</td>
                  <td className="py-1 text-muted-foreground font-mono text-[10px]">{a.statusVersion !== '-' ? a.statusVersion : a.specVersion}</td>
                  <td className="py-1">
        ...
        <Section title="Failure Domains" icon={Server}>
          <div className="flex flex-wrap gap-1">
            {failureDomains.map((az) => (
              <span key={az} className="badge badge-sm bg-popover text-muted-foreground border-border">{az}</span>
            ))}
          </div>
        </Section>
      )}
