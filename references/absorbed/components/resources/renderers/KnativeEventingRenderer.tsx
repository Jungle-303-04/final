        ...
        <PropertyList>
          ...
          <Property label="Subscriber" value={
            subscriberRef ? (
              ...
              <span className="text-foreground break-all">{subscriberUri}</span>
            ) : '-'
          } />
        </PropertyList>
      </Section>
        ...
        <Section title={`Filter (${filterEntries.length} attributes)`} defaultExpanded>
          <div className="flex flex-wrap gap-1">
            {filterEntries.map(([key, val]) => (
              <span key={key} className="badge bg-popover text-muted-foreground">
                {key}={String(val)}
              </span>
            ))}
          </div>
          ...
          <div className="space-y-1.5">
            {subscribers.map((sub: any, i: number) => (
              <div key={i} className="text-sm text-muted-foreground">
                ...
                {sub.ready === 'True' && (
                  <span className="ml-2 px-1.5 py-0.5 bg-success/20 text-success rounded text-[10px]">ready</span>
                )}
                {sub.ready === 'False' && (
                  <span className="ml-2 px-1.5 py-0.5 bg-destructive/20 text-destructive rounded text-[10px]">not ready</span>
                )}
              </div>
            ))}
          </div>
        ...
        <PropertyList>
          ...
          <Property label="Reply" value={
            replyRef ? (
              ...
              <span className="text-foreground break-all">{replyUri}</span>
            ) : undefined
          } />
        </PropertyList>
      </Section>
