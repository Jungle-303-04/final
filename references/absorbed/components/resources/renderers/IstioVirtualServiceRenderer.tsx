            ...
            {httpRoutes.map((route: any, i: number) => (
              <div key={i} className="card-inner-lg">
                ...
                {route.match && route.match.length > 0 && (
                  <div className="mb-2">
                    <div className="text-[10px] font-medium text-muted-foreground/75 uppercase tracking-wider mb-1">Match</div>
                ...
                {route.route && route.route.length > 0 && (
                  <div className="mb-2">
                    <div className="text-[10px] font-medium text-muted-foreground/75 uppercase tracking-wider mb-1">Destinations</div>
                      ...
                      {route.route.map((dest: any, di: number) => {
                        ...
                        return (
                          <div key={di} className="flex items-center gap-2 text-xs">
                            ...
                            />
                            {destPort && (
                              <span className="text-muted-foreground/75">:{destPort}</span>
                            )}
                            {destSubset && (
                              <Badge tone="structural" size="sm">
                                subset: {destSubset}
                ...
                {route.retries && (
                  <div className="flex flex-wrap gap-1 text-xs mb-1">
                    <span className="text-muted-foreground/75">Retries:</span>
                ...
                {route.mirror && (
                  <div className="flex items-center gap-1 text-xs mt-1">
                    <span className="text-muted-foreground/75">Mirror:</span>
                    <span className="text-muted-foreground">
                      {route.mirror.host}
                      {route.mirror.port?.number && `:${route.mirror.port.number}`}
                    </span>
                    {route.mirrorPercentage?.value !== undefined && (
                      <span className="text-muted-foreground/75">({route.mirrorPercentage.value}%)</span>
                    )}
                  </div>
                )}
              </div>
