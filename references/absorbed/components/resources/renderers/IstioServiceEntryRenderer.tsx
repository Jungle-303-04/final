          ...
          <div className="space-y-1">
            {ports.map((port, i) => (
              <div key={i} className="flex items-center gap-2 text-sm">
                ...
                {port.name && (
                  <span className="text-muted-foreground/75">{port.name}</span>
                )}
              </div>
            ))}
          </div>
          ...
          <div className="space-y-2">
            {endpoints.map((endpoint, i) => (
              <div key={i} className="card-inner">
                <div className="text-sm text-foreground">{endpoint.address}</div>
                {endpoint.ports && Object.keys(endpoint.ports).length > 0 && (
                  <div className="flex flex-wrap gap-1 mt-1">
                    {Object.entries(endpoint.ports).map(([name, port]) => (
                      <span key={name} className="badge-sm bg-accent text-muted-foreground">
                        {name}: {port}
                      </span>
                    ))}
                  </div>
