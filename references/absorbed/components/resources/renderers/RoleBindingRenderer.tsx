      ...
      <Section title="Role Reference" icon={Shield}>
        <PropertyList>
          <Property
            ...
            value={
              roleRef.kind ? (
                <span className={cn('badge', rbacKindBadgeClass(roleRef.kind))}>
                  {roleRef.kind}
                </span>
              ) : undefined
            }
          ...
          {subjects.map((subject: any, i: number) => (
            <div key={`${subject.kind}-${subject.name}-${i}`} className="card-inner text-sm">
              <div className="flex items-center gap-2">
                <span className={cn('badge', getSubjectKindBadgeClass(subject.kind))}>
                  {subject.kind}
                </span>
                ...
                {subject.kind === 'Group' && WIDE_GROUP_DESCRIPTIONS[subject.name] && (
                  <span className={cn('badge text-xs', BADGE_SEVERITY_COLORS.alert)}>wide</span>
                )}
              </div>
...
function RulesPreviewSection({
  ...
  return (
    <Section title="Rules Granted" icon={Eye} defaultExpanded={false}>
      {loading ? (
        <div className="text-sm text-muted-foreground/75">Loading rules…</div>
      ) : !rules ? (
            ...
            return (
              <div key={i} className="flex items-center gap-1 flex-wrap text-xs">
                ...
                {isNonResource ? (
                  ...
                  resources.map((res) => (
                    <span key={res} className={cn('badge', rbacResourceBadgeClass)}>{res}</span>
                  ))
                )}
                {!isNonResource && (r.apiGroups ?? []).filter((g) => g !== '').map((g) => (
                  <span key={g} className={cn('badge', rbacApiGroupBadgeClass)}>{g}</span>
                ))}
              </div>
            )
          })}
