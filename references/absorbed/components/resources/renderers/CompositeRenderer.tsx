 ...
 * Opsia can't tell the operator what they want to know, and "missing" would
    ...
    <>
      ...
      >
        {resourceRefs.length === 0 ? (
          <div className="text-sm text-muted-foreground/75 py-2">
...
function ComposedRefRow({
  ...
  return (
    <div className="flex items-center gap-2 py-1 px-2 rounded hover:bg-accent text-sm">
      <Box className="w-3.5 h-3.5 text-muted-foreground/75 shrink-0" />
      <span className="text-muted-foreground/75 font-mono shrink-0 w-32 truncate">{ref_.kind}</span>
      ...
      {ref_.namespace && (
        <span className="text-xs text-muted-foreground/75 truncate w-32 text-right">{ref_.namespace}</span>
      )}
...
function StatusCellInline({ statusEntry }: { statusEntry?: ComposedRefStatus }) {
  if (!statusEntry || statusEntry.loading) {
    return <span className="badge-sm bg-popover text-muted-foreground/75 w-20 text-center">…</span>
  }
  if (statusEntry.missing) {
    return <span className="badge-sm bg-popover text-muted-foreground/75 w-20 text-center">missing</span>
  }
  ...
  return (
    <span className={cn('badge-sm w-24 text-center truncate', statusEntry.status.color)}>
