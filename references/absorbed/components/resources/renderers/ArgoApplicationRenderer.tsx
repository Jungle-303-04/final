          ...
          value={
            <div className="flex flex-wrap gap-1">
              {source.helm.parameters.filter(Boolean).map((p: { name: string; value: string }, i: number) => (
                <span key={i} className="badge-sm bg-popover text-muted-foreground font-mono">
                  {p.name ?? '?'}={p.value ?? ''}
                </span>
              ))}
            </div>
          ...
          <div className="flex items-center justify-between">
            ...
            {isSyncing && onTerminate && (
              <button
                ...
                className="flex items-center gap-1.5 px-2 py-1 rounded text-xs bg-destructive/20 text-destructive hover:bg-destructive/30 transition-colors disabled:opacity-50"
          ...
          <div className="space-y-3">
            {sources.map((src: any, idx: number) => (
              <div key={idx} className="card-inner">
                <div className="text-xs font-medium text-muted-foreground/75 mb-1.5">Source {idx + 1}</div>
                <PropertyList>
          ...
          <Property
            ...
            value={
              <span
                className={cn(
                  'badge',
                  syncPolicy.automated ? 'bg-success/20 text-success' : BADGE_INACTIVE
                )}
              >
              ...
              return (
                <div
                  ...
                  className={cn(
                    ...
                    isFailed && 'border-l-2 border-destructive'
                  )}
                >
                  <div className="flex items-center gap-2 text-sm">
                    <span className={cn('badge badge-sm', getSyncResourceBadgeClass(res.status, res.hookPhase))}>
                    ...
                    {res.namespace && (
                      <span className="text-muted-foreground/75 text-xs">({res.namespace})</span>
                    )}
                  </div>
                  {res.message && (
                    <div className={cn(
            ...
            {status.history
              ...
              .map((entry, idx) => (
                ...
                >
                  ...
                  {entry.source?.path && (
                    <div className="text-xs text-muted-foreground mt-0.5 truncate" title={entry.source.path}>
                      {entry.source.path}
                    </div>
                  )}
                </div>
