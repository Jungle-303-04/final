...
function CopyButton({ text }: { text: string }) {
  ...
  return (
    <button
      ...
      className="p-0.5 text-muted-foreground/75 hover:text-foreground opacity-0 group-hover/row:opacity-100 transition-opacity shrink-0"
      title="Copy value"
    >
      ...
      <Section title={`Data (${dataEntries.length + binaryDataKeys.length} keys)`} defaultExpanded>
        <div className="space-y-3">
          ...
          {shortEntries.length > 0 && (
            <table className="w-full bg-background rounded overflow-hidden text-xs border-separate border-spacing-0 table-fixed">
              <colgroup>
              ...
              <tbody>
                {shortEntries.map(([key, value], i) => (
                  <tr key={key} className={`group/row ${i % 2 === 0 ? '' : 'bg-popover/30'}`}>
                    <td className="px-2.5 py-1.5 text-muted-foreground font-medium truncate align-top">
                      <TruncatedKey text={key} />
                    </td>
                    <td className="px-2.5 py-1.5 text-foreground font-mono break-all align-top">
                      {value}
                    </td>
          ...
          {longEntries.map(([key, value]) => {
            ...
            return (
              <ExpandableSection key={key} title={`${key} (${formatBytes(value.length)})`} defaultExpanded={!isLarge}>
                <pre className="bg-background rounded p-2 text-xs text-muted-foreground overflow-x-auto max-h-60 whitespace-pre-wrap">
                  {value}
                </pre>
              </ExpandableSection>
            )
