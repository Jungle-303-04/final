...
function StepStatusIcon({ phase, nodeType }: { phase: string; nodeType?: string }) {
  if (nodeType === 'Skipped' || phase === 'Skipped') {
    return <SkipForward className="w-4 h-4 text-muted-foreground/75 shrink-0" />
  }
  if (nodeType === 'Suspend' && phase !== 'Succeeded') {
    return <PauseCircle className={cn('h-4 w-4 shrink-0', SEVERITY_TEXT.warning)} />
  }
  switch (phase) {
    case 'Succeeded':
      return <CheckCircle className={cn('h-4 w-4 shrink-0', SEVERITY_TEXT.success)} />
    case 'Failed':
    case 'Error':
      return <XCircle className={cn('h-4 w-4 shrink-0', SEVERITY_TEXT.error)} />
    case 'Running':
      return <Loader2 className={cn('h-4 w-4 shrink-0 animate-spin', SEVERITY_TEXT.warning)} />
    default:
      return <Clock className="w-4 h-4 text-muted-foreground/75 shrink-0" />
  }
}
      ...
      <Section title="Status" icon={Play}>
        <PropertyList>
          <Property label="Phase" value={
            <span className={cn('badge', getPhaseBadgeClass(phase))}>
              {phase}
            </span>
          } />
              ...
              return (
                <div key={step.id} className={cn(
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <span className={cn(
                      'flex-1',
                      isSkipped ? 'text-muted-foreground/75' : isSuspend ? SEVERITY_TEXT.warning : 'text-foreground'
                    )}>
                      {step.displayLabel}
                      {isSkipped && <span className="ml-1 text-xs text-muted-foreground/75">(skipped)</span>}
                      {isSuspend && <span className={cn('ml-1 text-xs opacity-70', SEVERITY_TEXT.warning)}>(suspend)</span>}
                    </span>
                    <span className="text-xs text-muted-foreground/75">{step.displayType}</span>
                    <span className="text-xs text-muted-foreground">{getStepDuration(step) || '-'}</span>
                  </div>
                  {isFailed && step.message && (
                    <div className={cn('mt-1 ml-6 line-clamp-2 break-all text-xs', SEVERITY_TEXT.error)} title={step.message}>{step.message}</div>
                  )}
                  {step.templateRef && (
                    <div className="mt-1 ml-6 text-xs text-muted-foreground/75">
