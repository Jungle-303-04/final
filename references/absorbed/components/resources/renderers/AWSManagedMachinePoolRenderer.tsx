      ...
      <Section title="Overview" icon={Server}>
        <PropertyList>
          ...
          <Property label="Capacity Type" value={
            <span className={cn('badge badge-sm', capacityType === 'spot' ? CAPACITY_TYPE_BADGE.spot : CAPACITY_TYPE_BADGE.onDemand)}>{capacityType === 'onDemand' ? 'On-Demand' : capacityType}</span>
          } />
        ...
        <Section title="Subnets" icon={Server}>
          <div className="flex flex-wrap gap-1">
            {subnetIDs.map((id: string) => (
              <span key={id} className="badge badge-sm bg-popover text-muted-foreground border-border font-mono text-[10px]">{id}</span>
            ))}
          </div>
        </Section>
      )}
        ...
        <Section title="Node Labels" icon={Settings}>
          <div className="flex flex-wrap gap-1">
            {Object.entries(labels).map(([k, v]) => (
              <span key={k} className="badge badge-sm bg-popover text-muted-foreground border-border text-[10px]">
                {k}={v as string}
              </span>
            ))}
          </div>
