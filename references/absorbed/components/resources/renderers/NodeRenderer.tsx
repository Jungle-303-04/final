        ...
        <div className="space-y-2">
          {[
            ...
            <div key={row.label} className="card-inner">
              <div className="text-xs font-medium text-muted-foreground mb-1">{row.label}</div>
              <div className="space-y-0.5 text-xs">
                <div className="flex items-baseline justify-between gap-2">
                  <span className="text-muted-foreground/75">Capacity</span>
                  <span className="text-foreground tabular-nums">{row.capacity ?? '-'}</span>
                </div>
                <div className="flex items-baseline justify-between gap-2">
                  <span className="text-muted-foreground/75">Allocatable</span>
                  <span className="text-foreground tabular-nums">{row.allocatable ?? '-'}</span>
                </div>
                <div className="flex items-baseline justify-between gap-2">
                  <span className="text-muted-foreground/75">In Use</span>
                  <span className="text-foreground font-medium tabular-nums">{row.inUse ?? '-'}</span>
                </div>
              </div>
            </div>
          ))}
          ...
          {hasMetricsHistory && metricsHistory?.dataPoints ? (
            <div className="space-y-4">
              ...
              <div className="card-inner-lg">
                <div className="text-xs text-muted-foreground/75 mb-1 flex items-center justify-between">
                  <span>CPU</span>
          ...
          <div className="space-y-1">
            {taints.map((taint: any, i: number) => (
              <div key={`${taint.key}-${taint.effect}-${i}`} className="text-sm">
                <span className={cn(
                  'badge',
                  taint.effect === 'NoSchedule' ? 'bg-warning/20 text-warning' :
                  taint.effect === 'NoExecute' ? 'bg-destructive/20 text-destructive' :
