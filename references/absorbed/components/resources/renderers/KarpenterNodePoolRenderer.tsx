      ...
      <Section title="Disruption" icon={Shield}>
        ...
        {disruption.budgets && disruption.budgets.length > 0 && (
          <div className="mt-2 space-y-1">
            <div className="text-xs text-muted-foreground/75 font-medium mb-1">Budgets</div>
            {disruption.budgets.map((budget: any, i: number) => (
              <div key={i} className="card-inner text-sm text-muted-foreground">
        ...
        <Section title="Template Labels" icon={Tag}>
          <div className="flex flex-wrap gap-1">
            {Object.entries(templateLabels).map(([key, val]) => (
              <span
                key={key}
                className="badge-sm bg-accent text-muted-foreground"
              >
                {key}: {String(val)}
              </span>
            ))}
            ...
            {requirements.map((req: any, i: number) => (
              <div key={i} className="card-inner">
                <div className="flex items-center gap-2 text-sm">
                  <span className="text-foreground font-medium">{req.key}</span>
                  <span className="text-muted-foreground/75">{req.operator}</span>
                </div>
                {req.values && req.values.length > 0 && (
                  <div className="mt-1 flex flex-wrap gap-1">
                    {req.values.map((v: string, vi: number) => (
                      <span key={vi} className="badge-sm bg-accent text-muted-foreground">
                        {v}
                      </span>
                    ))}
                  </div>
