  ...
  return (
    <>
      ...
      <Section title="Cluster Overview" icon={Database} defaultExpanded>
        <PropertyList>
          ...
          {data.spec?.enableSuperuserAccess !== undefined && (
            <Property label="Superuser Access" value={
              <span className={`badge-sm ${data.spec.enableSuperuserAccess ? 'bg-success/20 text-success' : 'bg-accent text-muted-foreground'}`}>
        ...
        {instanceNames.length > 0 && (
          <div className="mt-2 pt-2 border-t border-border">
            <div className="text-xs font-medium text-muted-foreground uppercase tracking-wider mb-1">Instance Nodes</div>
            <div className="flex flex-wrap gap-1">
              {instanceNames.map((name: string) => (
                <span
                  key={name}
                  className="badge-sm bg-accent text-muted-foreground font-mono"
                >
                  {name}
                </span>
              ))}
          ...
          <div className="space-y-1.5">
            {reportedState.map((instance) => (
              <div key={instance.podName} className="flex items-center gap-2 text-sm">
                <span className="text-foreground font-mono flex-1 break-all">{instance.podName}</span>
                <span className={
                  instance.isPrimary
                    ? 'badge-sm bg-blue-100 text-blue-700 dark:bg-blue-900/40 dark:text-blue-300'
                    : 'badge-sm bg-popover text-muted-foreground'
                }>
                  {instance.isPrimary ? 'Primary' : 'Replica'}
                </span>
                {instance.timelineID != null && (
                  <span className="text-xs text-muted-foreground/75">TL {instance.timelineID}</span>
                )}
              </div>
            ))}
          </div>
      ...
      <Section title="Storage" icon={HardDrive} defaultExpanded>
        ...
        {walStorage && (
          <div className="mt-2 pt-2 border-t border-border">
            <div className="text-xs font-medium text-muted-foreground uppercase tracking-wider mb-1">WAL Storage</div>
            <PropertyList>
          ...
          <div className="space-y-1.5">
            {certExpirations.map((cert) => (
              <div key={cert.secretName} className="flex items-center gap-2 text-sm">
                <span className="text-foreground font-mono flex-1 break-all">{cert.secretName}</span>
                <span className="text-xs text-muted-foreground/75">{cert.expiryDate}</span>
                {cert.daysUntilExpiry <= 7 && (
                  <span className="badge-sm bg-destructive text-destructive dark:bg-destructive/40 dark:text-destructive">
                    {cert.daysUntilExpiry}d
                  </span>
                )}
                {cert.daysUntilExpiry > 7 && cert.daysUntilExpiry <= 30 && (
                  <span className="badge-sm bg-warning text-warning dark:bg-warning/40 dark:text-warning">
                    {cert.daysUntilExpiry}d
                  </span>
                )}
                {cert.daysUntilExpiry > 30 && (
                  <span className="badge-sm bg-popover text-muted-foreground">
                    {cert.daysUntilExpiry}d
                  </span>
                )}
              </div>
      ...
      {(monitoring.podMonitorEnabled || (monitoring.customQueriesConfigMap && monitoring.customQueriesConfigMap.length > 0)) && (
        <Section title="Monitoring" icon={Activity} defaultExpanded>
          ...
          {monitoring.customQueriesConfigMap && monitoring.customQueriesConfigMap.length > 0 && (
            <div className="mt-2 pt-2 border-t border-border">
              <div className="text-xs font-medium text-muted-foreground uppercase tracking-wider mb-1">Custom Queries</div>
              <div className="flex flex-wrap gap-1">
