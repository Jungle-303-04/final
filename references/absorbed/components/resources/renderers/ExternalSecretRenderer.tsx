              ...
              <tbody>
                {dataMappings.map((mapping, i) => (
                  <tr key={i} className="border-t border-border/50">
                    <td className="py-1.5 pr-3 text-foreground font-mono text-xs">{mapping.secretKey || '-'}</td>
                    <td className="py-1.5 pr-3 text-muted-foreground font-mono text-xs break-all">{mapping.remoteKey || '-'}</td>
                    <td className="py-1.5 text-muted-foreground text-xs">
                      {mapping.remoteProperty || '-'}
                      {mapping.remoteVersion && (
                        <span className="ml-1 text-muted-foreground/75">v{mapping.remoteVersion}</span>
                      )}
                    </td>
                  </tr>
                ))}
            ...
            {dataFromSources.map((source, i) => (
              <div key={i} className="card-inner">
                <div className="flex items-center gap-2 text-sm">
                  ...
                  {source.details && (
                    <span className="text-muted-foreground/75 text-xs font-mono break-all">{source.details}</span>
                  )}
                </div>
              </div>
            ))}
