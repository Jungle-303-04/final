        ...
        <Section title="Cert SANs" icon={Settings}>
          <div className="flex flex-wrap gap-1">
            {certSANs.map((san: string, i: number) => (
              <span key={i} className="badge badge-sm bg-card text-muted-foreground border-border">{san}</span>
            ))}
          </div>
        </Section>
      )}
            ...
            <tbody>
              {files.map((f: any, i: number) => (
                <tr key={i} className="border-t border-border">
                  <td className="py-1 text-muted-foreground font-mono text-[10px]">{f.path}</td>
                  <td className="py-1 text-muted-foreground">{f.owner || '-'}</td>
                  <td className="py-1 text-muted-foreground">{f.permissions || '-'}</td>
                </tr>
              ))}
            </tbody>
          </table>
