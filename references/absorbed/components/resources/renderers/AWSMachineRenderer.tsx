      ...
      <Section title="Instance" icon={Cloud}>
        <PropertyList>
          ...
          <Property label="State" value={
            <span className={cn('badge badge-sm', instanceState === 'running'
              ? 'status-healthy'
            ...
            <tbody>
              {addresses.map((addr: any, i: number) => (
                <tr key={i} className="border-t border-border">
                  <td className="py-1 text-muted-foreground">{addr.type}</td>
                  <td className="py-1 text-muted-foreground font-mono">{addr.address}</td>
                </tr>
              ))}
            </tbody>
          </table>
