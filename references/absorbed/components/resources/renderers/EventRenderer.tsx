        ...
        <div className="flex items-start gap-3">
          ...
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-2 mb-1">
              <span className={cn(
                'badge',
                isWarning ? 'bg-warning/20 text-warning' : 'bg-blue-500/20 text-blue-400'
              )}>
                {eventType}
              </span>
              {reason && (
                <span className={cn(
                  ...
                  isWarning ? 'text-warning' : 'text-blue-300'
                )}>
                  {reason}
                </span>
              )}
            ...
            {message && (
              <p className={cn(
                'text-sm break-all',
                isWarning ? 'text-warning/90' : 'text-blue-200/90'
              )}>
                {message}
              </p>
            )}
        ...
        <Section title="Involved Object" icon={Target} defaultExpanded>
          <div className="space-y-2">
            <div className="flex items-center gap-2">
              <span className="text-muted-foreground/75 text-sm w-24">Kind</span>
              <span className="text-foreground text-sm font-medium">{involvedObject.kind}</span>
            </div>
            <div className="flex items-center gap-2">
              <span className="text-muted-foreground/75 text-sm w-24">Name</span>
              {onNavigate ? (
                <button
                  onClick={() => {
                ...
                <span className="text-foreground text-sm">{involvedObject.name}</span>
              )}
            </div>
            {involvedObject.namespace && (
              <div className="flex items-center gap-2">
                <span className="text-muted-foreground/75 text-sm w-24">Namespace</span>
                <span className="text-muted-foreground text-sm">{involvedObject.namespace}</span>
              </div>
            )}
            {involvedObject.uid && (
              <div className="flex items-center gap-2">
                <span className="text-muted-foreground/75 text-sm w-24">UID</span>
                <span className="text-muted-foreground/75 text-xs font-mono">{involvedObject.uid}</span>
              </div>
            )}
            {involvedObject.fieldPath && (
              <div className="flex items-center gap-2">
                <span className="text-muted-foreground/75 text-sm w-24">Field</span>
                <span className="text-muted-foreground text-sm font-mono">{involvedObject.fieldPath}</span>
              </div>
            )}
          </div>
        </Section>
