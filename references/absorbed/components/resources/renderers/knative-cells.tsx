...
function TextCell({ value }: { value: string }) {
  return <span className="text-sm text-muted-foreground truncate block">{value}</span>
}
...
export function RevisionCell({ resource, column }: { resource: any; column: string }) {
  switch (column) {
    ...
    case 'routing': {
      ...
      if (state === 'active') return <span className="text-success text-sm">Active</span>
      if (state === 'reserve') return <span className="text-muted-foreground/75 text-sm">Reserve</span>
  ...
  switch (column) {
    ...
    case 'mode': {
      ...
      return (
        <span className={cn(
          'badge',
          mode === 'Proxy' ? 'bg-warning/20 text-warning' : 'bg-success/20 text-success'
        )}>
          {mode}
        </span>
      )
