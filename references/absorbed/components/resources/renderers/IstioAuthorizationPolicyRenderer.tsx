          ...
          <div className="space-y-3">
            {rules.map((rule: any, i: number) => (
              <div key={i} className="card-inner-lg">
                <div className="text-sm font-medium text-foreground mb-2">Rule {i + 1}</div>

                {/* From (sources) */}
                {rule.from && rule.from.length > 0 && (
                  <div className="mb-2">
                    <div className="text-[10px] font-medium text-muted-foreground/75 uppercase tracking-wider mb-1">From (Sources)</div>
                ...
                {rule.to && rule.to.length > 0 && (
                  <div className="mb-2">
                    <div className="text-[10px] font-medium text-muted-foreground/75 uppercase tracking-wider mb-1">To (Operations)</div>
                ...
                {rule.when && rule.when.length > 0 && (
                  <div>
                    <div className="text-[10px] font-medium text-muted-foreground/75 uppercase tracking-wider mb-1">When (Conditions)</div>
                    {rule.when.map((w: any, wi: number) => (
                      <div key={wi} className="flex flex-wrap gap-1 mb-1 text-xs">
                        <span className="text-muted-foreground font-medium">{w.key}</span>
                        {w.values && (
