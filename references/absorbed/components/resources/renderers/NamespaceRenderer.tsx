      ...
      <Section title="Status" icon={Box}>
        <PropertyList>
          <Property label="Phase" value={
            phase ? (
              <span className={cn(
                phase === 'Active' && 'text-success dark:text-success',
        ...
        {quotas.map((q: any, qi: number) => {
          ...
          return (
            <div key={name} className="card-inner">
              <div className="text-xs font-medium text-foreground mb-1.5">{name}</div>
              {resourceNames.length === 0 ? (
                <div className="text-xs text-muted-foreground">No hard limits set.</div>
              ) : (
                    ...
                    return (
                      <div key={res} className="text-xs">
                        <div className="flex items-center justify-between gap-2">
                          <span className="text-muted-foreground truncate">{res}</span>
                          <span className={cn('shrink-0 tabular-nums', tone)}>
                            {used[res] ?? '0'} / {hard[res]}{pct !== null && ` (${pct}%)`}
                          </span>
                        </div>
                        {pct !== null && (
                          <div className="mt-0.5 h-1 rounded-full bg-background overflow-hidden">
  ...
  if (loading) {
    return (
      <Section title="RBAC" icon={Shield}>
        <div className="text-sm text-muted-foreground">Loading RBAC summary…</div>
      </Section>
    )
  }
  if (error) {
  ...
  return (
    <div className="card-inner">
      <div className="flex items-center gap-2 flex-wrap text-xs mb-1.5">
        <span className={cn('badge', rbacKindBadgeClass(entry.binding.kind))}>
          {entry.binding.kind}
        </span>
        <ResourceLink
        ...
        <span className={cn('badge', rbacKindBadgeClass(entry.binding.roleRef.kind))}>
    ...
    <span className="inline-flex items-center gap-1.5 flex-wrap">
      {shown.map((s, i) => (
        <span key={i} className="inline-flex items-center gap-1">
          <span className="text-muted-foreground/75">{s.kind.toLowerCase()}:</span>
          ...
          {i < shown.length - 1 && <span className="text-muted-foreground/75">,</span>}
        </span>
      ))}
      {hidden > 0 && <span className="text-muted-foreground/75">+{hidden} more</span>}
    </span>
  )
}
