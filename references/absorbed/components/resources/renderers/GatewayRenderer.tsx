          ...
          <div className="space-y-2">
            {allAddresses.map((addr: any, i: number) => (
              <div key={`${addr.type}-${addr.value}-${i}`} className="flex items-center gap-2 text-sm">
                <span className="text-muted-foreground/75 w-28 shrink-0">{addr.type || 'Unknown'}</span>
                <span className="text-foreground break-all">{addr.value}</span>
              </div>
            ))}
          </div>
        </Section>
              ...
              <div key={listener.name} className="card-inner-lg">
                <div className="flex items-center justify-between mb-2">
                  <div className="flex items-center gap-2">
                    {isHTTPS && <Lock className="w-3.5 h-3.5 text-success" />}
                    <span className="text-sm font-medium text-foreground">{listener.name}</span>
                ...
                <div className="space-y-1 text-xs text-muted-foreground">
                  {listener.hostname && (
                    <div>
                      <span className="text-muted-foreground/75">Hostname: </span>
                  ...
                  {isHTTPS && listener.tls && (
                    <div>
                      <span className="text-muted-foreground/75">TLS Mode: </span>
                      <span>{listener.tls.mode || 'Terminate'}</span>
                      {listener.tls.certificateRefs?.length > 0 && (
                        <span className="ml-2">
                          <span className="text-muted-foreground/75">Certs: </span>
                  ...
                  {listener.allowedRoutes && (
                    <div>
                      <span className="text-muted-foreground/75">Allowed Routes: </span>
                  ...
                  {listenerStatus && (
                    <div>
                      <span className="text-muted-foreground/75">Attached Routes: </span>
                      <span className="text-foreground font-medium">{listenerStatus.attachedRoutes ?? 0}</span>
                    </div>
                  )}
                </div>
              </div>
