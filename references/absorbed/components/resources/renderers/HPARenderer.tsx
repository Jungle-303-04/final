          ...
          <div className="card-inner space-y-3">
            <div className="flex flex-wrap items-start justify-between gap-2">
              <div className="min-w-0">
                <div className="text-sm font-medium text-foreground">{hpaDiagnosis.summary}</div>
            ...
            {hpaDiagnosis.reasons && hpaDiagnosis.reasons.length > 0 && (
              <div className="space-y-2">
                {hpaDiagnosis.reasons.map((reason) => (
                  <div key={`${reason.id}-${reason.message}`} className="rounded border border-border bg-card p-2">
                    <div className="flex flex-wrap items-center gap-2 text-xs">
                      <span className="font-medium text-foreground">Evidence</span>
                      <span className="text-muted-foreground/75">{formatReasonID(reason.id)}</span>
                      {reason.conditionType && <span className="text-muted-foreground/75">{reason.conditionType}</span>}
                      {reason.conditionReason && <span className="text-muted-foreground/75">{reason.conditionReason}</span>}
                      {reason.detail && <span className="text-muted-foreground/75">{reason.detail}</span>}
                    </div>
                    {!isReasonMessageRedundant(hpaDiagnosis.state, reason) && (
                      <div className="mt-1 text-xs text-muted-foreground">{reason.message}</div>
                    )}
                  </div>
                ))}
              </div>
              ...
              <div key={`${metric.type}-${metric.name}`} className="card-inner">
                <div className="flex flex-wrap items-center justify-between gap-2 text-sm">
                  <div>
                    ...
                    <div className="text-xs text-muted-foreground/75">{metric.type}</div>
                  </div>
              ...
              return (
                <div key={i} className="card-inner">
                  <div className="flex items-center justify-between text-sm">
                    <span className="text-foreground">{metric.resource?.name || metric.type}</span>
                    <span className="text-muted-foreground">{current}{typeof current === 'number' ? '%' : ''} / {target}{typeof target === 'number' ? '%' : ''}</span>
                  </div>
                    ...
                    <div className="mt-2 h-2 bg-accent rounded overflow-hidden">
                      <div
                        className={cn(
                          ...
                          current > target ? 'bg-destructive' : current > target * 0.8 ? 'bg-warning' : 'bg-success'
                        )}
