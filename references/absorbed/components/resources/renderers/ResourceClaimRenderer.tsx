    ...
    <>
              ...
              return (
                <div key={req?.name || i} className="card-inner">
                  <div className="flex flex-wrap gap-x-4 gap-y-1 text-sm">
                    <span className="text-foreground font-medium">{req?.name || `request-${i}`}</span>
                    <span className="text-muted-foreground">{requestDeviceClass(req)}</span>
                    {detail?.count != null && (
                      <span className="text-muted-foreground/75">count: {String(detail.count)}</span>
                    )}
                    {detail?.allocationMode && (
                      <span className="text-muted-foreground/75">{detail.allocationMode}</span>
                    )}
                  </div>
                </div>
              )
      ...
      <Section title="Allocation" icon={Layers} defaultExpanded>
        {allocation.length > 0 ? (
          ...
          <div className="text-sm text-muted-foreground/75">Not allocated — waiting for a driver to satisfy this claim</div>
        )}
      </Section>
            ...
            {reservedFor.map((r, i) => (
              <div key={i} className="card-inner text-sm">
                {r.resource === 'pods' ? (
                  ...
                  <span className="text-muted-foreground">{r.resource}/{r.name}</span>
                )}
              </div>
            ))}
          </div>
        ...
        <Section title={`Device Status (${deviceStatuses.length})`}>
          <div className="space-y-2">
            {deviceStatuses.map((d: any, i: number) => (
              <div key={i} className="card-inner text-sm">
                <div className="text-foreground font-medium">{d.device || '-'}</div>
                {(d.conditions || []).map((c: any, j: number) => (
                  <div key={j} className="text-xs text-muted-foreground mt-0.5">
