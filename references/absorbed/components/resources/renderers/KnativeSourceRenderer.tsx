...
function SinkProperty({ sink, ns, onNavigate }: { sink: any; ns: string; onNavigate?: RendererProps['onNavigate'] }) {
  ...
  if (sink.uri) {
    return <Property label="Sink URI" value={
      <span className="text-foreground break-all">{sink.uri}</span>
    } />
  }
          ...
          <div className="space-y-1.5">
            {resources.map((r: any, i: number) => (
              <div key={i} className="card-inner text-sm">
                <span className="text-foreground">{r.apiVersion}/{r.kind}</span>
                {r.selector && (
            ...
            {containers.map((c: any, i: number) => (
              <div key={i} className="card-inner text-sm">
                ...
                {c.args && c.args.length > 0 && (
                  <div className="text-xs text-muted-foreground/75 mt-1 font-mono truncate">
                    $ {c.args.join(' ')}
                  </div>
                )}
              </div>
