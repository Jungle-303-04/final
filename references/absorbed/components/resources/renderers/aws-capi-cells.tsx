...
function MonoCell({ value }: { value: string }) {
  return (
    <Tooltip content={value}>
      <span className="text-xs text-muted-foreground font-mono truncate max-w-[180px] inline-block">{value}</span>
    </Tooltip>
  )
}
...
export function AWSManagedMachinePoolCell({ resource, column }: { resource: any; column: string }) {
  switch (column) {
    ...
    case 'capacityType': {
      ...
      return (
        <span className={cn('badge badge-sm', ct === 'spot' ? CAPACITY_TYPE_BADGE.spot : CAPACITY_TYPE_BADGE.onDemand)}>{ct === 'onDemand' ? 'On-Demand' : ct}</span>
      )
    }
    default:
      return <span className="text-sm text-muted-foreground/75">-</span>
...
export function AWSMachineCell({ resource, column }: { resource: any; column: string }) {
  switch (column) {
    ...
    case 'instanceState': {
      ...
      return (
        <span className={cn('badge badge-sm', state === 'running'
          ? 'status-healthy'
