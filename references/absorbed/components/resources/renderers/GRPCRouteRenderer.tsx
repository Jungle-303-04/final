          ...
          <Property
            ...
            value={
              hostnames.length > 0 ? (
                ...
                <span className="text-xs text-muted-foreground/75">Any</span>
              )
            }
          />
          <Property
          ...
          {rules.map((rule: any, ruleIdx: number) => {
            ...
            return (
              <div key={ruleIdx} className="card-inner">
                <div className="text-xs font-medium text-muted-foreground/75 mb-1.5">
                  Rule {ruleIdx + 1}
                </div>
                ...
                <div className="space-y-1">
                  {matches.length === 0 && backendRefs.length === 0 && (
                    <div className="text-xs text-muted-foreground italic">Match all</div>
                  )}
                  {matches.length === 0 && backendRefs.length > 0 && !multiBackend && (
                    <div className="text-xs text-muted-foreground flex flex-wrap items-center gap-1.5">
                      <span className="italic text-muted-foreground/75">all</span>
                      <ArrowRight className="w-3 h-3 text-muted-foreground/75 shrink-0" />
                      <ResourceRefBadge resourceRef={toServiceRef(backendRefs[0])} onClick={onNavigate} />
                      {backendRefs[0].port && <span className="text-muted-foreground/75">:{backendRefs[0].port}</span>}
                    </div>
                  )}
                  {matches.length === 0 && multiBackend && (
                    <>
                      <div className="text-xs text-muted-foreground italic">all</div>
                      {backendRefs.map((b: any, bi: number) => {
                        const pct = hasWeights ? Math.round(((b.weight ?? 1) / totalWeight) * 100) : null
                        return (
                          <div key={bi} className="text-xs text-muted-foreground flex items-center gap-1.5 pl-1">
                            <ArrowRight className="w-3 h-3 text-muted-foreground/75 shrink-0" />
                            <ResourceRefBadge resourceRef={toServiceRef(b)} onClick={onNavigate} />
                            {b.port && <span className="text-muted-foreground/75">:{b.port}</span>}
                            {pct !== null && <span className="text-muted-foreground/75 text-[10px]">{pct}%</span>}
                          </div>
                        )
                      })}
                    </>
                  ...
                  {matches.map((match: any, matchIdx: number) => (
                    <div key={matchIdx} className="text-xs text-muted-foreground flex flex-wrap items-center gap-1.5">
                      {match.method && (
                        <>
                          ...
                          {match.method.service && (
                            <span>
                              <span className="text-foreground font-medium">{match.method.service}</span>
                              {match.method.method && <span className="text-muted-foreground/75">/{match.method.method}</span>}
                            </span>
                          )}
                          {!match.method.service && match.method.method && (
                            <span><span className="text-muted-foreground/75">*/</span><span className="text-foreground">{match.method.method}</span></span>
                          )}
                        </>
                      )}
                ...
                {filters.length > 0 && (
                  <div className="mt-1.5">
                    <div className="text-xs text-muted-foreground flex flex-wrap items-center gap-1.5">
                      <Filter className="w-3 h-3 text-muted-foreground/75" />
              ...
              return (
                <div key={`${ref.namespace || ''}-${ref.name}-${idx}`} className="card-inner">
                  ...
                  {accepted?.message && accepted.status === 'False' && (
                    <div className="text-xs text-muted-foreground/75 mt-1">{accepted.message}</div>
                  )}
                </div>
              )
            })}
