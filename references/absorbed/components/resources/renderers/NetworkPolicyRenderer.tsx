    ...
    <>
      ...
      <Section title="Target" icon={Shield}>
        ...
        {policyTypes.length > 0 && (
          <div className="mt-2">
            <div className="text-xs text-muted-foreground/75 mb-1">Policy Types</div>
      ...
      {hasIngress && (
        <Section title="Ingress Rules" icon={ArrowDownToLine} defaultExpanded>
          {ingress && ingress.length > 0 ? (
            ...
            <div className="text-sm text-destructive">Deny all ingress</div>
          )}
        </Section>
      )}
      ...
      {hasEgress && (
        <Section title="Egress Rules" icon={ArrowUpFromLine} defaultExpanded>
          {egress && egress.length > 0 ? (
            ...
            <div className="text-sm text-destructive">Deny all egress</div>
          )}
        </Section>
      )}
    </>
...
function PeerEntry({ peer }: { peer: any }) {
  if (peer.podSelector) {
    return (
      <div className="text-sm">
        <span className="text-muted-foreground text-xs">podSelector: </span>
  ...
  if (peer.namespaceSelector) {
    return (
      <div className="text-sm">
        <span className="text-muted-foreground text-xs">namespaceSelector: </span>
  ...
  if (peer.ipBlock) {
    return (
      <div className="text-sm">
        <span className="text-muted-foreground text-xs">ipBlock: </span>
        <span className="text-xs text-foreground">{peer.ipBlock.cidr}</span>
