              ...
              <div key={`${port.name || 'port'}-${port.port || index}-${port.protocol || 'TCP'}`} className="card-inner text-sm">
                <div className="flex items-center justify-between gap-3">
                  <div className="min-w-0">
                    <div className="font-medium text-foreground truncate">{port.name || `port-${index + 1}`}</div>
                    {port.appProtocol && (
                      <div className="text-xs text-muted-foreground/75 mt-0.5">{port.appProtocol}</div>
                    )}
                  </div>
                ...
                <div key={`${endpoint.addresses?.join(',') || 'endpoint'}-${index}`} className="card-inner text-sm space-y-3">
                  <div className="flex items-start justify-between gap-3">
                    ...
                    <div className="flex flex-wrap justify-end gap-1.5 shrink-0">
                      <span className={cn('badge-sm', ready ? 'status-healthy' : 'status-unhealthy')}>
                  ...
                  {(endpoint.nodeName || endpoint.zone) && (
                    <div className="flex flex-wrap gap-x-4 gap-y-1 text-xs text-muted-foreground/75">
