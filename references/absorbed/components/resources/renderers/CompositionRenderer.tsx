      ...
      <Section title="Composition" icon={Layers} defaultExpanded>
        <PropertyList>
          ...
          <Property label="Mode" value={
            <span className={cn('badge-sm', mode === 'Pipeline' ? 'status-violet' : 'status-neutral')}>
              {mode}
            </span>
          } />
          {xrdKind && (
              ...
              return (
                <div key={i} className="card-inner-lg">
                  <div className="flex items-center gap-2 mb-2">
                    ...
                    <span className="font-medium text-foreground">{step.step || '(unnamed step)'}</span>
                  </div>
                  <PropertyList>
                    {fnName && (
                      <Property
                    ...
                    {inputApiVersion && (
                      <Property
                        label="Input API"
                        value={<span className="font-mono text-xs text-muted-foreground/75">{inputApiVersion}</span>}
                      />
                    )}
                  </PropertyList>
                  {step.input && (
                    <details className="mt-2">
                      <summary className="text-xs text-muted-foreground/75 cursor-pointer hover:text-muted-foreground">
                        Show input
                      </summary>
                      <div className="mt-1">
            ...
            {resources.map((res: any, i: number) => {
              ...
              return (
                <div key={i} className="card-inner text-sm flex items-center gap-2 flex-wrap">
                  ...
                  {baseApiVersion && (
                    <span className="text-muted-foreground/75 text-xs">{baseApiVersion}</span>
                  )}
                  {patchCount > 0 && (
                    <span className="ml-auto text-muted-foreground/75 text-xs">
