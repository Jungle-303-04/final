        ...
        <PropertyList>
          ...
          {scope && (
            <Property
              label="Scope"
              value={
                <span className={cn('badge-sm', scope === 'Cluster' ? 'status-violet' : 'status-neutral')}>
                  {scope}
                </span>
              }
            />
            ...
            {versions.map((v: any, i: number) => (
              <div key={i} className="card-inner text-sm flex items-center gap-2 flex-wrap">
                ...
                {v.deprecationWarning && (
                  <span className="text-muted-foreground/75 text-xs break-all">{v.deprecationWarning}</span>
                )}
              </div>
            ))}
          </div>
