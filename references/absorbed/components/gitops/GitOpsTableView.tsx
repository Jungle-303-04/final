...
import { isArgoOperationInProgress, isArgoSuspendedByOpsia } from '../resources/resource-utils-argo'
...
//
// Originally inline in opsia/web/src/components/gitops/GitOpsView.tsx as the
// per-cluster Opsia UI. Extracted here so Opsia Hub's cross-cluster fleet
...
//     when the caller passes them (Hub-only — single-cluster Opsia omits)
...
export interface GitOpsTableViewProps {
  ...
  /**
   * Which side the filter rail sits on. Default 'left' (OSS Opsia, which
...
export function GitOpsTableView({
  ...
  if (totalGitOps === 0 && allRowsInput.length === 0 && !loading && !hasGlobalNamespaceFilter && !hasUnavailableGitOpsCounts) {
    return (
      <div className="flex h-full min-h-0 flex-1 items-start justify-center bg-background px-4 pb-4 pt-[22vh]">
        <div className="rounded-lg border border-border bg-card p-8 text-center">
          <GitBranch className="mx-auto h-8 w-8 text-muted-foreground/75" />
    ...
    <div className="flex h-full min-w-0 flex-1 flex-col overflow-hidden bg-background">
      ...
      <div className="flex min-w-0 flex-1 flex-col overflow-hidden">
        {forbiddenSecretsClusters && forbiddenSecretsClusters.length > 0 && mode === 'applications' && (
          ...
          <div className="shrink-0 border-b border-warning/30 bg-warning/10 px-4 py-2 text-xs text-warning dark:text-warning">
          ...
          <div className="mt-3 flex flex-wrap items-center gap-2">
            <div className="relative w-full max-w-md">
              ...
              <Input
                ...
                className="h-8 w-full rounded-md border border-border bg-background pl-8 pr-3 text-sm text-foreground placeholder:text-muted-foreground/75 focus:outline-none focus:ring-1 focus:ring-blue-500/50"
              />
            </div>
            ...
            {onDestinationFilterChange && mode === 'applications' && (
              <div className="ml-auto flex items-center gap-1 text-[11px]">
                <span className="text-muted-foreground/75">Destination:</span>
            ...
            {hasAnyFilter && (
              <Tooltip content={hasGlobalNamespaceFilter ? 'Reset all filters and the active namespace' : 'Reset all filters'}>
                <button
                  ...
                  className="inline-flex h-8 items-center gap-1.5 rounded-md border border-border bg-background px-2.5 text-xs text-muted-foreground hover:bg-accent hover:text-foreground"
                >
        ...
        <div className={cn('min-h-0 min-w-0 flex-1 overflow-auto bg-background', onRowAction && 'pb-20')}>
          {mode !== 'applications' ? (
            ...
            <div className="p-4 text-sm text-destructive">Failed to load GitOps applications: {error.message}</div>
            ...
            <div className="flex h-full flex-col items-center justify-center gap-3 text-sm text-muted-foreground">
              ...
              {(hasGlobalNamespaceFilter || (allRows.length > 0 && hasLocalFilters)) && (
                <button
                  type="button"
                  onClick={clearAllFilters}
                  className="inline-flex items-center gap-1.5 rounded-md bg-popover px-3 py-1.5 text-sm text-muted-foreground transition-colors hover:bg-border hover:text-foreground"
                >
                  <RotateCcw className="h-3.5 w-3.5" />
                  Clear filters
                </button>
...
function GitOpsFilterSidebar({
  ...
  return (
    <aside
      className={`flex w-72 shrink-0 flex-col overflow-hidden border-border bg-card/90 max-[899px]:max-h-72 max-[899px]:w-full max-[899px]:border-b ${
          ...
          <div className="grid grid-cols-2 gap-1">
            {AVAILABLE_MODES.map((item) => (
              ...
              >
                ...
                <div className={mode === item ? 'text-white/70' : 'text-muted-foreground/75'}>{modeCounts[item]}</div>
              </button>
            ))}
          </div>
        </GitOpsFilterSection>
    ...
    <Tooltip content={label}>
      <button
        ...
        className={`inline-flex h-8 w-8 items-center justify-center transition-colors ${
          active ? 'bg-primary text-white' : 'bg-background text-muted-foreground hover:bg-accent hover:text-foreground'
        }`}
      >
...
function GitOpsSortMenu({ sortKey, onChange }: { sortKey: SortKey; onChange: (key: SortKey) => void }) {
  ...
  return (
    <div ref={ref} className="relative">
      ...
      {open && (
        <div className="absolute right-0 top-full z-50 mt-1 w-40 overflow-hidden rounded-lg border border-border bg-card py-1 shadow-xl">
          {SORT_OPTIONS.map(([key, label]) => (
            <button
              ...
              className={cn(
                ...
                key === sortKey ? 'font-medium text-foreground' : 'text-muted-foreground',
              )}
            >
              <Check className={cn('h-3.5 w-3.5 shrink-0', key === sortKey ? 'opacity-100 text-primary' : 'opacity-0')} />
              {label}
            </button>
          ))}
        </div>
  ...
  return (
    <div ref={containerRef} className="relative">
      ...
      >
        ...
        Labels
        {activeLabels.size > 0 && (
          <span className="rounded bg-success/20 px-1 text-[10px] tabular-nums">{activeLabels.size}</span>
        )}
      </button>
      {open && (
        <div className="absolute right-0 top-full z-50 mt-1 w-80 overflow-hidden rounded-lg border border-border bg-card shadow-xl">
          <div className="border-b border-border p-2">
            <div className="mb-2 text-xs text-muted-foreground">
              Selected labels are combined with <span className="font-semibold text-foreground">AND</span>.
            </div>
            <div className="flex items-center gap-2">
              <div className="relative flex-1">
                ...
                <Input
                  ...
                  autoFocus
                  className="h-7 w-full rounded border border-border bg-popover pl-7 pr-2 text-xs text-foreground placeholder:text-muted-foreground/75 focus:outline-none focus:ring-1 focus:ring-blue-500/50"
                />
              </div>
              {activeLabels.size > 0 && (
                <button
                  ...
                  className="shrink-0 rounded px-1 py-0.5 text-xs text-muted-foreground/75 hover:text-foreground"
                >
                  Clear
                </button>
              )}
            ...
            {filtered.map((label) => {
              ...
              return (
                ...
                >
                  ...
                  <span className="shrink-0 tabular-nums text-muted-foreground/75">({label.count})</span>
                </button>
              )
            })}
            {filtered.length === 0 && (
              <div className="px-3 py-2 text-xs text-muted-foreground/75">No labels match.</div>
            )}
          </div>
        </div>
      )}
...
function StatusDistribution({ rows }: { rows: GitOpsRow[] }) {
  ...
  return (
    <DistributionBar
      ...
      segments={[
        { key: 'degraded', count: summary.degraded, fillClass: 'bg-destructive' },
        { key: 'progressing', count: summary.progressing, fillClass: 'bg-sky-500' },
        { key: 'healthy', count: summary.healthy, fillClass: 'bg-success' },
        { key: 'unknown', count: Math.max(0, rows.length - summary.healthy - summary.progressing - summary.degraded), fillClass: 'bg-muted-foreground/40' },
      ]}
    />
  )
}
    ...
    <table className="w-full min-w-[1040px] table-fixed border-separate border-spacing-0 text-sm">
      <thead className="sticky top-0 z-10 bg-background">
        <tr>
          ...
          <th className={cn(TH_CLASS, showDestination ? 'w-[20%]' : 'w-[28%]')}>Source</th>
          {showDestination && <th className={cn(TH_CLASS, 'w-[14%]')}>Destination</th>}
          <SortableTh label="Last Sync" sortKey="lastSync" activeKey={sort?.key ?? null} direction={sort?.dir ?? 'asc'} onSort={onSort} className="w-[10%]" />
          {showActions && (
            <th className={cn(TH_CLASS, 'w-[6%] text-right')}>
        ...
        {rows.map((row) => {
          ...
          return (
            <tr
              ...
              className={cn(
                'border-b border-border bg-background hover:bg-accent',
                !href && 'cursor-pointer',
            ...
            >
                  ...
                  <div className="min-w-0">
                    {href ? (
                      <a
                        ...
                        className="block truncate font-medium text-foreground hover:underline focus-visible:underline focus-visible:outline-none rounded-sm"
                      >
                        {row.name}
                      </a>
                    ) : (
              ...
              <TableCell>
                <div className="truncate text-foreground">{row.repository || row.chart || '-'}</div>
                <div className="truncate text-xs text-muted-foreground/75">{[row.targetRevision, row.path || row.chart].filter(Boolean).join(' · ') || '-'}</div>
              </TableCell>
              {showDestination && (
                <TableCell>
                  <DestinationCell row={row} onDestinationClick={onDestinationClick} destinationHrefFor={destinationHrefFor} />
                  <div className="truncate text-xs text-muted-foreground/75">{row.destinationNamespace || row.namespace || '-'}</div>
                </TableCell>
              )}
              <TableCell>
                {row.terminating
              ...
              {showActions && onRowAction && (
                <td
                  className="overflow-visible border-b border-border px-2 py-2 text-right align-middle"
...
function GitOpsTile({
  ...
  const tileClass = cn(
    'group relative flex min-w-0 flex-col overflow-hidden rounded-md border border-border bg-card text-left shadow-sm transition-all hover:border-muted-foreground/40 hover:shadow-md focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-foreground/20',
    row.terminating && 'opacity-80',
  )
  const body = (
    <>
      <div className={cn('h-1 w-full', statusStripe(row))} />
      <div className="flex flex-1 flex-col gap-3 px-4 pb-4 pt-3">
        <div className="line-clamp-2 break-all text-[15px] font-semibold leading-tight text-foreground">
          {row.name}
        </div>
        ...
        <div className="flex flex-col gap-1 text-[12px]">
          {source && (
            <div className="truncate text-muted-foreground">{source}</div>
          )}
          {revision && (
            <div className="truncate font-mono text-[11px] text-muted-foreground/75">{shortRevision(revision)}</div>
          )}
          {row.terminating ? (
            <div className="font-medium text-orange-400/80">Pending {formatRelativeAge(row.terminationStartedAt ?? '') || 'now'}</div>
          ) : (
            lastSyncRaw && <div className={cn('font-medium', recencyClass)}>{formatRelativeAge(lastSyncRaw)}</div>
          )}
        </div>
        {(dest || ns || row.project) && (
          <div className="mt-auto flex flex-wrap items-center gap-x-1.5 gap-y-1 border-t border-border/60 pt-3 text-[11px] text-muted-foreground/75">
...
//     destination cluster's Opsia-known name. Click stops row propagation
...
//     Opsia-connected cluster (onboarding hook).
function DestinationCell({
  ...
  if (!dest) {
    return <div className="block truncate text-foreground">{row.destination || '-'}</div>
  }
  if (dest.match === 'in_cluster') {
    return <span className="block truncate text-muted-foreground/75">same cluster</span>
  }
  ...
  return (
    <span
      className="flex min-w-0 items-center gap-1 text-xs text-warning dark:text-warning"
      title={`Not a Opsia-connected cluster. Connect ${rawLabel} to see workloads.`}
    >
...
function TableCell({ children }: { children: ReactNode }) {
  ...
  return <td className="overflow-hidden border-b border-border px-3 py-2 align-middle text-muted-foreground">{children}</td>
}
...
function statusStripe(row: GitOpsRow) {
  ...
  if (row.health === 'Degraded') return 'bg-destructive'
  if (row.health === 'Progressing' || row.sync === 'Reconciling') return 'bg-sky-500'
  if (row.sync === 'OutOfSync') return 'bg-warning'
  if (row.health === 'Healthy' && row.sync === 'Synced') return 'bg-success'
...
function recencyTone(value: string): string {
  if (!value) return 'text-muted-foreground/75'
  const time = Date.parse(value)
  if (!Number.isFinite(time)) return 'text-muted-foreground/75'
  const diffMs = Date.now() - time
  if (diffMs < 10 * 60_000) return 'text-success dark:text-success'
  if (diffMs > 7 * 24 * 60 * 60_000) return 'text-warning dark:text-warning'
...
// fetch+normalize pipeline (OSS fetches per-kind from the opsia API;
...
export function normalizeArgoApplication(resource: any): GitOpsRow {
  ...
  return {
    ...
    suspended: (status?.suspended ?? false) || isArgoSuspendedByOpsia(resource),
