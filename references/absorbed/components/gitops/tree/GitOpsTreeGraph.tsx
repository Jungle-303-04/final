...
function EdgeLegend({ edges }: { edges: Edge[] }) {
  ...
  return (
    <div className="absolute bottom-4 left-4 z-10 rounded-md border border-border bg-card/90 px-2.5 py-1.5 text-[10px] backdrop-blur">
      <div className="mb-1 font-medium uppercase tracking-wide text-muted-foreground/75">Edges</div>
...
function GitOpsTreeToolbar({
  ...
  return (
    <div className="absolute right-4 top-4 z-10 flex flex-wrap items-center justify-end gap-2">
      ...
      <div className="flex items-center gap-1 rounded-lg border border-border bg-card/90 px-2 py-1.5 backdrop-blur">
        <Search className="h-3.5 w-3.5 text-muted-foreground/75" />
        <Input
          value={query}
          onChange={(event) => onQueryChange(event.target.value)}
          placeholder="Find node..."
          className="w-36 bg-transparent text-xs text-foreground outline-none placeholder:text-muted-foreground/75"
        />
        {query && (
          <button type="button" onClick={() => onQueryChange('')} className="text-muted-foreground/75 hover:text-foreground">
      ...
      <Tooltip content="Fit tree" delay={120}>
        <button
          type="button"
          onClick={onFit}
          className="flex items-center gap-1.5 rounded-lg border border-border bg-card/90 px-2.5 py-1.5 text-xs text-muted-foreground backdrop-blur transition-colors hover:text-foreground"
        >
...
const GitOpsResourceNode = memo(function GitOpsResourceNode({ data }: NodeProps<Node<{ node: GitOpsTreeNode; highlighted?: boolean }>>) {
  ...
  return (
    <>
      ...
      <div
        className={cn(
          'relative overflow-hidden rounded-lg border bg-card shadow-md transition-colors',
          data.highlighted ? 'border-primary ring-2 ring-primary/40' : 'border-border',
          ...
          gitopsTool && !data.highlighted && 'ring-1 ring-primary/30 hover:ring-primary/60',
          ...
          node.role === 'group' && 'cursor-pointer hover:border-muted-foreground/50 hover:bg-accent',
        )}
        style={{ width: dim.width, minHeight: dim.height, borderLeftWidth: 4 }}
      >
        <div className="px-3 py-2.5">
          <div className="mb-1 flex items-center gap-1.5">
            <KindIcon className="h-3.5 w-3.5 shrink-0 text-muted-foreground/75" aria-hidden />
            <span className="truncate text-[10px] font-medium uppercase tracking-wide text-muted-foreground/75">
            ...
            {gitopsTool && (
              <span className="rounded-sm border border-primary/30 bg-primary/10 px-1 py-px text-[9px] font-semibold uppercase tracking-wide text-primary">
                {gitopsTool === 'argocd' ? 'Argo' : 'Flux'}
              </span>
            )}
            <span className={cn('ml-auto h-1.5 w-1.5 rounded-full', getStatusDotColor(status))} />
          </div>
          <div className="truncate pr-1 text-sm font-medium text-foreground">{node.ref.name}</div>
          ...
          <div className="mt-0.5 flex items-center gap-1 text-xs text-muted-foreground">
            ...
            {(node.role === 'group' || gitopsTool) && <ChevronRight className="ml-auto h-3 w-3 shrink-0 text-muted-foreground/75" />}
          </div>
          {chips.length > 0 && (
                  ...
                  className={cn(
                    ...
                    chip.tone === 'warning'
                      ...
                      : chip.tone === 'danger'
                        ...
                        : 'border-border bg-popover/70 text-muted-foreground'
                  )}
                >
