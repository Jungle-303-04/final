...
export function SyncStatusBadge({ sync, suspended }: { sync: SyncStatus; suspended?: boolean }) {
  if (suspended) {
    ...
    return (
      <span className={cn('badge', SEVERITY_BADGE_BORDERED.info)}>
  ...
  return (
    <span className={cn('badge', config.color)}>
      <Icon className={cn('w-3 h-3', sync === 'Reconciling' && 'animate-spin')} />
      {config.label}
    </span>
  )
}
...
export function HealthStatusBadge({ health }: { health: GitOpsHealthStatus }) {
  ...
  return (
    <span className={cn('badge', color)}>
      <Icon className={cn('w-3 h-3', health === 'Progressing' && 'animate-spin')} />
      {label}
    </span>
  )
}
