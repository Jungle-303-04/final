...
export interface GitOpsDetailStatus {
  ...
  // suspended-by-Opsia via annotations). Caller resolves the OR.
  suspended: boolean
}
...
export interface GitOpsDetailLayoutProps {
  ...
  // lineage; OSS leaves it false so single-cluster Opsia's UX is unchanged.
  isFleetContext?: boolean
  destinationCluster?: { id: string; name: string }

  // Tab-title side effect — sets document.title to "<name> · Opsia" while
  // mounted, restores on unmount. Opt-in so hub-web fleet detail can pick
  // its own title format ("<name> — Fleet GitOps").
  manageDocumentTitle?: boolean
  documentTitleSuffix?: string                // defaults to " · Opsia"
...
export function GitOpsDetailLayout(props: GitOpsDetailLayoutProps) {
  ...
  useEffect(() => {
    ...
    const suffix = documentTitleSuffix ?? ' · Opsia'
  ...
  const bodyShellClass = fullscreen
    ? 'fixed inset-0 z-[80] flex min-h-0 min-w-0 flex-col bg-background'
  ...
  return (
    <div className="flex h-full min-h-0 min-w-0 flex-1 flex-col overflow-hidden bg-background">
          ...
          <div className="flex flex-wrap items-start justify-between gap-3">
            <div className="min-w-0">
              ...
              <div className="flex flex-wrap items-center gap-2">
                ...
                <span className="shrink-0 text-xs text-muted-foreground/75">/</span>
                ...
                <span className="inline-flex shrink-0 items-center rounded border border-border bg-accent/50 px-1.5 py-0.5 text-[11px] font-medium text-muted-foreground">
              ...
              {/* Spec/config row */}
              <div className="mt-2 flex flex-wrap gap-x-5 gap-y-0.5 text-[11px] text-muted-foreground/75">
          ...
          {helmValues && onToggleHelmValues && (
            <div className="shrink-0 border-b border-border bg-background">
              <button
                ...
                className="flex w-full items-center gap-2 px-4 py-2 text-left text-xs text-muted-foreground hover:bg-accent"
              ...
              >
                ...
                <Settings className="h-3.5 w-3.5 shrink-0 text-muted-foreground/75" />
                <span className="font-medium text-foreground">Helm values</span>
                ...
                {helmValues.source === 'argo-parameters' && (
                  <span className="text-[10px] uppercase tracking-wider text-muted-foreground/75">parameters</span>
                )}
              </button>
      ...
      {resourceLoading ? (
        ...
        <div className="p-4 text-sm text-destructive">Failed to load resource: {resourceError.message}</div>
      ) : (
