...
function ResourceKindGroup({ kind, resources, onResourceClick, showHealth }: ResourceKindGroupProps) {
  ...
  return (
    <div>
      <button
        onClick={() => setExpanded(!expanded)}
        className="flex items-center gap-1.5 text-xs text-muted-foreground/75 hover:text-muted-foreground transition-colors mb-1"
      >
        <CollapseChevron open={expanded} className="w-3 h-3" />
        <span className="font-medium">{kind}</span>
        <span className="text-muted-foreground/75">({resources.length})</span>
      </button>
...
function ResourceItem({ resource, onClick, showHealth }: ResourceItemProps) {
  ...
  const content = (
    <div className="flex items-center gap-2 py-0.5">
      ...
      {onClick && (
        <ExternalLink className="w-3 h-3 text-muted-foreground/75 opacity-0 group-hover:opacity-100 transition-opacity" />
      )}
    </div>
  )
  ...
  if (onClick) {
    return (
      <button
        ...
        className="group w-full text-left hover:bg-popover/30 rounded px-1 -mx-1 transition-colors"
      >
        {content}
      </button>
    )
  ...
  if (healthy !== undefined && unhealthy !== undefined) {
    return (
      <span className="text-xs">
        ...
        <span className={unhealthy > 0 ? 'text-destructive' : 'text-muted-foreground'}>{count}</span>
      </span>
    )
  }
