      ...
      {shown.map((m, i) => {
        ...
        return (
          <div key={i} className="flex items-start gap-1.5">
            <Icon className={cn('w-3 h-3 shrink-0 mt-0.5', isDanger ? SEVERITY_TEXT.error : SEVERITY_TEXT.warning)} />
            <span>{m.message}</span>
          </div>
        )
      })}
      {overflow > 0 && (
        <div className="text-muted-foreground/75">{`+${overflow} more`}</div>
      )}
      {clickHint && (
        <div className="text-muted-foreground/75 mt-0.5">Click to open →</div>
      )}
    </div>
  )
}
