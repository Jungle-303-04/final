  ...
  return (
    <section className="rounded-lg border border-border bg-card p-4 shadow-sm">
      <button
        ...
        className="flex w-full items-center gap-2 rounded-md text-left transition-colors hover:text-foreground"
      >
        ...
        <span className="text-sm font-semibold text-foreground">Audit Findings</span>
        <div className="flex items-center gap-2 ml-1">
          {dangers > 0 && (
            <span className={cn('text-xs font-medium tabular-nums', SEVERITY_TEXT.error)}>{dangers} critical</span>
          )}
          {warnings > 0 && (
            <span className={cn('text-xs font-medium tabular-nums', SEVERITY_TEXT.warning)}>{warnings} warning</span>
          )}
        </div>
      </button>
            ...
            {findings.map((f, i) => {
              ...
              return (
                <div key={`${f.checkID}-${i}`} className="flex items-start gap-2 py-1">
                  {isDanger ? (
                    <ShieldAlert className={cn('w-3.5 h-3.5 shrink-0 mt-0.5', SEVERITY_TEXT.error)} />
                  ) : (
                    <AlertTriangle className={cn('w-3.5 h-3.5 shrink-0 mt-0.5', SEVERITY_TEXT.warning)} />
                  )}
                  <div className="flex min-w-0 flex-1 flex-wrap items-center gap-x-2 gap-y-1">
                    <span className="text-xs text-muted-foreground">{f.message}</span>
                    <span className={cn('badge-sm text-[10px] shrink-0', BP_CATEGORY_BADGE[f.category] || DEFAULT_BADGE_COLOR)}>
                      {f.category}
                    </span>
                  </div>
                </div>
