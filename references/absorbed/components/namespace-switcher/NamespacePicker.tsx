...
/**
 * Backend-reported namespace scope. Mirrors Opsia's `/cluster/namespace-scope`
 * response shape; the host supplies it however it fetches it (OSS via its API
 * hooks, Opsia Hub via the per-cluster apiBase).
 */
 ...
 * Opsia OSS and Opsia Hub (mirrors the ClusterSwitcher pattern — pure UI, data
  ...
  const tooltipContent = disabled && disabledTooltip
    ...
    : scope.cacheScoped
      ? scope.namespaceRescope
        ? `Opsia is watching only ${scope.cacheScopeNamespace || triggerLabel} to stay fast on large clusters. Pick another namespace to re-point it (takes a moment; closes open terminals).`
        : `Opsia is watching only ${scope.cacheScopeNamespace || triggerLabel} on this cluster.`
      : restrictedHint
    ...
    <>
      ...
      >
        <button
          ...
          className={
            variant === 'segment'
              ? `flex items-center justify-center gap-1.5 px-3 py-1.5 h-full min-w-[110px] max-w-[200px] text-[13px] text-foreground hover:bg-accent disabled:opacity-60 transition-colors ${className}`
              : `flex items-center gap-1.5 px-2 py-1 rounded text-sm bg-popover hover:bg-accent text-foreground disabled:opacity-60 transition-colors ${className}`
          }
        ...
        >
          ...
          {isClusterWide ? (
            <Globe className="w-3.5 h-3.5 shrink-0 text-muted-foreground/75" />
          ) : restrictedHint ? (
            <AlertTriangle className="w-3.5 h-3.5 shrink-0 text-muted-foreground/75" />
          ) : null}
      ...
      {isOpen &&
        createPortal(
          ...
          >
              ...
              <>
                {items.length > 6 && (
                  <div className="flex items-center gap-2 px-2 py-1.5 border-b border-border">
                    ...
                    <input
                      ...
                      className="flex-1 bg-transparent text-sm outline-none text-foreground placeholder:text-muted-foreground/75"
                    />
                  </div>
                )}

                <div className="px-3 py-1.5 border-b border-border text-[11px] leading-snug text-muted-foreground">
                  Opsia is watching one namespace to stay fast on large clusters.
                  {scope.namespaceRescope
                ...
                <ul className="max-h-80 overflow-y-auto py-1">
                  ...
                  {filteredItems.map(ns => {
                    ...
                    return (
                      <li key={ns}>
                        <label className="w-full flex items-center justify-between px-3 py-1.5 text-sm hover:bg-accent text-left text-foreground cursor-pointer">
                          <span className="flex items-center gap-2 min-w-0">
                            ...
                            {isContextDefault && (
                              <span className="text-[10px] uppercase tracking-wide text-muted-foreground/75 shrink-0">
                                kubeconfig
                              </span>
                            )}
                          </span>
                ...
                <div className="flex items-center justify-between px-3 py-1.5 border-t border-border text-[11px] text-muted-foreground/75">
                  <span>{draft.size === 1 ? Array.from(draft)[0] : 'Select a namespace'}</span>
                  <button
                    onClick={closeAndApply}
                    className="px-2 py-0.5 rounded bg-popover hover:bg-accent text-foreground"
                  >
                    Done
                  </button>
                </div>
            ...
            {!scope.authoritative && (
              <div className="px-3 py-2 border-t border-border text-[11px] status-degraded">
