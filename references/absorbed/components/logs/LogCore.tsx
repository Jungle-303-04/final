...
export function LogCore({
  ...
  // Seed isDark: forceDark prop wins; else localStorage['opsia-logs-dark'];
  ...
  const [isDark, setIsDark] = useState<boolean>(() => {
    ...
    try {
      const v = localStorage.getItem('opsia-logs-dark')
  ...
  const toggleDark = useCallback(() => {
    ...
    setIsDark(prev => {
      const next = !prev
      try { localStorage.setItem('opsia-logs-dark', String(next)) } catch {}
      return next
    })
  }, [themeLocked])
  const [wordWrap, setWordWrap] = useState(() => {
    try { return localStorage.getItem('opsia-logs-wrap') !== 'false' } catch { return true }
  })
  const [showTimestamps, setShowTimestamps] = useState(() => {
    try { return localStorage.getItem('opsia-logs-timestamps') !== 'false' } catch { return true }
  })
  const [tsFormat, setTsFormat] = useState<TimestampFormat>(() => {
    try {
      const v = localStorage.getItem('opsia-logs-ts-format') as TimestampFormat | null
      return v && TIMESTAMP_FORMAT_ORDER.includes(v) ? v : 'time-local'
    } catch { return 'time-local' }
  })
  const [ansiEnabled, setAnsiEnabled] = useState(() => {
    try { return localStorage.getItem('opsia-logs-ansi') !== 'false' } catch { return true }
  })
  const [collapseStacks, setCollapseStacks] = useState(() => {
    try { return localStorage.getItem('opsia-logs-collapse-stacks') !== 'false' } catch { return true }
  })
  ...
  const [structuredMode, setStructuredMode] = useState<StructuredMode>(() => {
    try {
      const v = localStorage.getItem('opsia-logs-structured-mode') as StructuredMode | null
  ...
  const pickStructuredMode = useCallback((mode: StructuredMode) => {
    ...
    try { localStorage.setItem('opsia-logs-structured-mode', mode) } catch {}
    setShowStructuredMenu(false)
  }, [])
  ...
  const cycleStructuredMode = useCallback(() => {
    setStructuredMode(prev => {
      ...
      try { localStorage.setItem('opsia-logs-structured-mode', next) } catch {}
      return next
    })
  }, [])
  ...
  const toggleWrap = useCallback(() => {
    setWordWrap(prev => {
      const next = !prev
      try { localStorage.setItem('opsia-logs-wrap', String(next)) } catch {}
      return next
    })
  }, [])
  ...
  const toggleTimestamps = useCallback(() => {
    setShowTimestamps(prev => {
      const next = !prev
      try { localStorage.setItem('opsia-logs-timestamps', String(next)) } catch {}
      return next
    })
  }, [])
  ...
  const pickTsFormat = useCallback((fmt: TimestampFormat) => {
    setTsFormat(fmt)
    try { localStorage.setItem('opsia-logs-ts-format', fmt) } catch {}
    // Auto-show timestamps when user picks a format — otherwise the change isn't visible.
    setShowTimestamps(true)
    try { localStorage.setItem('opsia-logs-timestamps', 'true') } catch {}
    setShowTsMenu(false)
  }, [])
  ...
  const toggleAnsi = useCallback(() => {
    setAnsiEnabled(prev => {
      const next = !prev
      try { localStorage.setItem('opsia-logs-ansi', String(next)) } catch {}
      return next
    })
  }, [])
  ...
  const toggleCollapseStacks = useCallback(() => {
    setCollapseStacks(prev => {
      const next = !prev
      try { localStorage.setItem('opsia-logs-collapse-stacks', String(next)) } catch {}
      return next
    })
  }, [])
