...
function eventColorClass(event: TimelineEvent): string {
  if (isProblematicEvent(event)) return 'text-warning dark:text-warning'
  if (isHistoricalEvent(event)) return 'text-muted-foreground/75'
...
function MarkerGlyph({ shape, size, className }: { shape: MarkerShape; size: number; className?: string }) {
  ...
  if (shape === 'diamond') {
    const d = size * 0.8
    return <span className={cn(className, 'rounded-[2px]')} style={{ display: 'block', width: d, height: d, background: 'currentColor', transform: 'rotate(45deg)' }} />
  }
  if (shape === 'ring') {
    return <span className={cn(className, 'rounded-full')} style={{ display: 'block', width: size, height: size, border: '2px solid currentColor' }} />
  }
  return <span className={cn(className, 'rounded-full')} style={{ display: 'block', width: size, height: size, background: 'currentColor' }} />
}
...
export interface TimelineSwimlanesProps {
  ...
  topology?: Topology
  namespaces?: string[]
  // RBAC capability flag (was a opsia/web context); host passes it. Default false.
  hasLimitedAccess?: boolean
  // GitOps lane labels deep-link to a controller path; the host decides how to
  // navigate (opsia router push, or cross-route-tree href). When omitted, GitOps lanes
...
export function TimelineSwimlanes({ events, isLoading, onResourceClick, viewMode, onViewModeChange, topology, namespaces, hasLimitedAccess = false, onNavigatePath, showDeleted: showDeletedProp, onShowDeletedChange, pinnedOnly: pinnedOnlyProp, onPinnedOnlyChange, viewWindow, onViewWindowChange, bounds, onExtendRequest, nowMs, gaps, onAppClick, search: searchProp, onSearchChange, activityFilter: activityFilterProp, onActivityFilterChange, kindFilter: kindFilterProp, onKindFilterChange, appIndex, grouping: groupingProp, onGroupingChange, sort: sortProp, onSortChange, pinnedLanes, onTogglePin, selectedEventId, onSelectedEventChange, isLive, compact = false }: TimelineSwimlanesProps) {
  ...
  const renderLane = (lane: ResourceLane, keyPrefix = '', depth = 0, isLast = false): React.ReactNode => {
    ...
    const track = (heightClass: string, small?: boolean) => (
      <LaneTrack
        className={cn('flex-1 mr-8', heightClass)}
        events={trackEvents}
    ...
    if (depth === 0) {
      return (
        ...
        <div key={keyPrefix + lane.id} className={cn(lane.isAppGroup && 'border-y border-border')}>
          {/* Parent lane */}
          <div className={lane.isAppGroup && !(isExpanded && hasVisibleChildren) ? undefined : 'border-b-subtle'}>
            <div className="flex items-center">
              {/* Lane label */}
              <div className={cn('relative shrink-0 border-r border-border px-3 flex items-center gap-1 group/pin', laneLabelWidthClass, compact ? 'py-1' : 'py-2')}>
                ...
                {hasVisibleChildren && isExpanded && (
                  <span className="absolute bottom-0 top-1/2 w-px bg-border" style={{ left: TREE_ROOT_RAIL_PX }} />
                )}
                ...
                {hasVisibleChildren ? (
                  <button
                    ...
                    className="p-1 -m-0.5 text-muted-foreground/75 hover:text-foreground hover:bg-popover rounded"
                  >
                    ...
                    <div className="flex items-center gap-1.5 min-w-0">
                      <Tooltip content={lane.name} wrapperClassName="min-w-0 flex-1">
                        <span
                          ...
                          className={cn('min-w-0 w-full text-sm text-foreground hover:text-accent-text hover:underline cursor-pointer', compact ? 'font-medium' : 'font-semibold font-mono')}
                        >
                          <MiddleEllipsis text={lane.name} className="block" />
                        </span>
                      </Tooltip>
                      {hasVisibleChildren && (
                        <span className="shrink-0 text-[11.5px] font-semibold text-muted-foreground/75">
    ...
    return (
      <div key={keyPrefix + lane.id} className={cn('border-b-subtle', isLast && !(isExpanded && hasVisibleChildren) && 'border-b-0')}>
        <div className="flex">
          <ChildLaneLabel
            kind={lane.kind}
            group={lane.group}
  ...
  const emptyState = (
    <div className="flex flex-col items-center justify-center h-64 text-muted-foreground/75">
      ...
      {windowInsideGap(viewWindow, gaps) ? (
        ...
        <>
          ...
          {namespaces && namespaces.length > 0 && <p className="text-sm mt-1 text-muted-foreground/45">Searching in: {namespaces.length === 1 ? namespaces[0] : `${namespaces.length} namespaces`}</p>}
          <button
            type="button"
            onClick={clearAllFilters}
            className="mt-4 flex items-center gap-2 px-3 py-1.5 text-sm bg-popover border border-border rounded-lg text-muted-foreground hover:bg-accent hover:text-foreground transition-colors"
          >
        ...
        <>
          ...
          {namespaces && namespaces.length > 0 && (
            <p className="text-sm mt-2 text-muted-foreground">
              Filtering by namespace: <span className="font-medium text-foreground">{namespaces.length === 1 ? namespaces[0] : `${namespaces.length} namespaces`}</span>
            </p>
          )}
          {hasLimitedAccess && (
            <p className="flex items-center gap-1 text-sm mt-2 text-warning/80">
  ...
  return (
    ...
    <div className={cn('flex flex-col w-full', compact ? 'min-h-0' : 'h-full')}>
      ...
      {!compact && (
      <div className="border-b border-border bg-card/30 relative z-40">
        <TimelineToolbar
          search={searchTerm}
          ...
          <div className="flex items-center gap-2 px-4 py-1.5">
            <Tooltip content="Zoom in (Ctrl+scroll)">
              <button
                ...
                aria-label="Zoom in"
                className="p-1.5 text-muted-foreground hover:text-foreground hover:bg-popover rounded"
              >
            ...
            {showJumpToNow && (
              <Tooltip content="Jump to current time">
                <button
                  ...
                  className="px-2 py-1 text-xs text-accent-text hover:underline hover:bg-popover rounded"
                >
                  → Now
                </button>
              </Tooltip>
        ...
        {showLegend && (
          <>
            ...
            <div className="absolute right-4 top-full z-50 mt-1 w-[280px] rounded-xl border border-border bg-card p-4 shadow-lg">
              <div className="text-[10px] font-bold uppercase leading-none tracking-[0.07em] text-muted-foreground/75">Events</div>
              <div className="mt-2.5 grid grid-cols-2 gap-x-4 gap-y-2 text-xs text-muted-foreground">
                ...
                <MarkerLegendItem shape="diamond" colorClass="text-warning dark:text-warning" label="warning" description="Warning event (CrashLoopBackOff, Failed, etc.)" />
                <MarkerLegendItem shape="ring" colorClass="text-muted-foreground/75" label="historical" description="Inferred from resource metadata (creation time, etc.)" />
                <ClusterLegendItem description="Nearby events collapsed into one pill" />
              </div>
              <div className="mt-3.5 text-[10px] font-bold uppercase leading-none tracking-[0.07em] text-muted-foreground/75">Health</div>
              <div className="mt-2.5 grid grid-cols-2 gap-x-4 gap-y-2 text-xs text-muted-foreground">
      ...
      <div className={cn('overflow-y-auto overflow-x-hidden', compact ? 'max-h-[240px]' : 'flex-1')}>
        <div
          ref={containerRef}
        ...
        >
          ...
          <div className="sticky top-0 z-30 bg-card border-b border-border">
            <div className="flex">
              <div className={cn('shrink-0 border-r border-border px-3 h-8 flex items-center', laneLabelWidthClass)}>
                <span className="text-[11px] font-semibold uppercase tracking-[0.1em] text-muted-foreground/75">Resource</span>
                ...
                {!compact && (
                  <Tooltip content={mostlyExpanded ? 'Collapse all (E)' : 'Expand all (E)'} wrapperClassName="ml-auto">
                    <button
                      ...
                      className="rounded p-1 text-muted-foreground/75 hover:bg-popover hover:text-foreground"
                    ...
                    <>
                      {axisTicks.map((tick) => {
                        ...
                        return (
                          ...
                          >
                            ...
                            <span className="mt-0.5 whitespace-nowrap text-xs text-muted-foreground/75">{tick.label}</span>
                          </div>
                        )
                      })}
                      ...
                      {nowVisible && (
                        <div
                          ...
                          style={{ left: `${nowX}%` }}
                        >
                          <div className={cn('h-2 w-0.5 bg-purple-500', nowX > 97 && 'translate-x-0.5')} />
                ...
                {atPastEdge && (
                  <Tooltip content="Start of loaded history — load earlier" position="bottom">
                    <button
                      ...
                      className="absolute left-0 top-1/2 z-30 flex h-5 w-5 -translate-y-1/2 items-center justify-center rounded border border-border bg-popover text-muted-foreground shadow-sm transition-colors hover:bg-accent hover:text-foreground"
                ...
                {atFutureEdge && (
                  <Tooltip content="End of loaded history — load more recent" position="bottom">
                    <button
                      ...
                      className="absolute right-0 top-1/2 z-30 flex h-5 w-5 -translate-y-1/2 items-center justify-center rounded border border-border bg-popover text-muted-foreground shadow-sm transition-colors hover:bg-accent hover:text-foreground"
          ...
          <div className="relative">
              ...
              return (
                ...
                >
                  {widthPx > GAP_LABEL_MIN_PX && (
                    <span className="whitespace-nowrap rounded bg-card/70 px-1 text-[10px] uppercase tracking-wide text-muted-foreground/75">
                      connector offline
                    </span>
                  )}
                </div>
            ...
            {pinnedLaneRows.length > 0 && (
              <div data-testid="timeline-pinned-section">
                <div className="flex">
                  <div className={cn('shrink-0 border-r border-border px-3 py-1.5', laneLabelWidthClass)}>
                    <span className="text-[11px] font-semibold uppercase tracking-[0.1em] text-muted-foreground/75">Pinned</span>
                  </div>
                ...
                : (
                  <>
                    ...
                    {/* End-of-list caption: absence here means "nothing happened
                        ...
                        as the empty states. */}
                    <div className="flex items-center justify-center gap-2 py-6 text-sm text-muted-foreground/75">
...
const HEALTH_STRIP_COLORS: Record<string, string> = {
  ...
  degraded: 'bg-warning dark:bg-[#b8861e]',
...
function ClusterLegendItem({ description }: { description: string }) {
  return (
    <Tooltip content={description} position="top">
      <span className="flex items-center gap-1.5 cursor-help">
        <span className="inline-flex h-4 items-center gap-1 rounded-full border border-border bg-card px-1.5">
          <MarkerGlyph shape="circle" size={8} className="text-blue-600 dark:text-blue-400" />
          <span className="text-[11px] font-semibold leading-none tabular-nums text-foreground">×5</span>
        </span>
...
function HealthLegendItem({ color, label, description, swatchStyle }: { color?: string; label: string; description: string; swatchStyle?: React.CSSProperties }) {
  return (
    <Tooltip content={description} position="top">
      <span className="flex items-center gap-1.5 cursor-help">
        <span className={cn('h-[7px] rounded-[2px]', color)} style={{ width: '18px', ...swatchStyle }} />
        <span className="leading-none">{label}</span>
      </span>
    </Tooltip>
  )
...
function LaneWarnChip({ events }: { events: TimelineEvent[] }) {
  ...
  return (
    <Tooltip content={`${pluralize(issueCount, 'problem')} (OOMKilled, CrashLoopBackOff, etc.)`} position="top">
      <span className="flex items-center gap-0.5 text-[11px] font-semibold px-1.5 py-0.5 rounded bg-warning text-warning dark:bg-warning/50 dark:text-warning">
...
function KindChip({ kind, title }: { kind: string; title?: string }) {
  ...
  return (
    <Tooltip content={tip} disabled={!tip} wrapperClassName="shrink-0">
      <span
        className="rounded border border-border/60 bg-popover px-1.5 py-0.5 font-mono text-[10px] font-semibold tracking-wide text-muted-foreground"
        aria-label={tip}
      >
        {label}
      </span>
...
function GroupChip({ group }: { group: string }) {
  return (
    <Tooltip content={`API group ${group}`} wrapperClassName="shrink-0">
      <span
        className="rounded bg-accent px-1 py-px text-[10px] font-medium text-muted-foreground/75 ring-1 ring-inset ring-border"
...
function ChildLaneLabel({ kind, group, showGroupChip, kindTitle, name, labelWidthClass = 'w-[360px]', isLast, onClick, pinButton, title, depth = 1, hasChildren, expanded, onToggle }: { kind: string; group?: string; showGroupChip?: boolean; kindTitle?: string; name: string; labelWidthClass?: string; isLast: boolean; onClick: () => void; pinButton?: React.ReactNode; title?: string; depth?: number; hasChildren?: boolean; expanded?: boolean; onToggle?: () => void }) {
  ...
  return (
    <div
      ...
      className={cn('relative shrink-0 border-r border-border/50 pr-3 flex items-center gap-1.5 group/pin bg-card/40', labelWidthClass)}
    ...
    >
      ...
      <span className={cn('absolute top-0 w-px bg-border', isLast ? 'h-1/2' : 'h-full')} style={{ left: parentRailPx }} />
      {/* Horizontal branch from the parent rail into this row. */}
      <span className="absolute top-1/2 h-px bg-border" style={{ left: parentRailPx, width: selfRailPx - parentRailPx }} />
      ...
      {hasChildren && expanded && (
        <span className="absolute bottom-0 top-1/2 w-px bg-border" style={{ left: selfRailPx }} />
      )}
      {hasChildren && onToggle && (
        <button
          onClick={(e) => { e.stopPropagation(); onToggle() }}
          // Centered ON this row's rail. bg keeps the line from striking the glyph.
          className="absolute top-1/2 z-10 -translate-x-1/2 -translate-y-1/2 rounded bg-card p-1 text-muted-foreground/75 hover:bg-popover hover:text-foreground"
      ...
      <Tooltip content={title ?? name} wrapperClassName="min-w-0 flex-1">
        <span
          onClick={onClick}
          className="min-w-0 w-full text-[13px] font-mono text-muted-foreground hover:text-accent-text hover:underline cursor-pointer"
        >
  ...
  return (
    <Tooltip content={title} wrapperClassName="shrink-0">
      <button
        ...
        onClick={(e) => { e.stopPropagation(); onToggle() }}
        className={cn(
          'p-0.5 rounded hover:bg-popover transition-opacity',
          pinned ? 'text-accent-text opacity-100' : 'text-muted-foreground/75 hover:text-foreground',
...
function AppGroupLaneLabel({ lane, memberCount, onToggle, onAppClick }: { lane: ResourceLane; memberCount: number; onToggle: () => void; onAppClick?: (appKey: string) => void }) {
  ...
  return (
    <div
      ...
      className="flex-1 min-w-0 flex items-center gap-1.5 cursor-pointer rounded px-1 -mx-1 hover:bg-card/30"
      onClick={onToggle}
    >
      <Tooltip content={tooltip} disabled={!tooltip} wrapperClassName="shrink-0">
        <span
          className={cn(
            ...
            dimmed
              ? 'bg-accent text-muted-foreground/75 ring-border'
      ...
      <div className="min-w-0 flex-1 flex flex-col gap-0.5">
        ...
        {lane.namespace && (
          <span className="text-[11px] text-muted-foreground/75 truncate">{lane.namespace}</span>
        )}
      </div>
    </div>
  )
...
function LaneBackdrop({ gridXs }: { gridXs: number[] }) {
  return (
    <div className="absolute inset-0 z-0 pointer-events-none">
      {gridXs.map((x, i) => (
        <div key={i} className="absolute top-0 bottom-0 w-px bg-border/40" style={{ left: `${x}%` }} />
      ))}
    ...
    <div className="absolute inset-0 z-0">
      {spans.map((span, i) => {
        ...
        return (
          ...
          >
            <div className={cn('relative h-full w-full', getHealthStripColor(span.health))}>
              {showCreatedBefore && (
                <span className="absolute bottom-[9px] left-0.5 text-[9px] text-muted-foreground/75 whitespace-nowrap pointer-events-none">
...
function AggregateHealthTrack({ lane, startTime, windowMs, now, ghost }: {
  ...
  if (segments.length === 0) {
    return <div className={cn('absolute top-1/2 -translate-y-1/2 left-0 right-0 z-0 rounded-[2px] bg-gray-400/25', ghost ? 'h-[4px]' : 'h-[7px]')} />
  }

  return (
    <div className={cn('absolute inset-0 z-0', ghost && 'opacity-40')}>
      {segments.map((seg, i) => {
        ...
        return (
          ...
          >
            <div
              className={cn('h-full w-full', !seg.mixed && getHealthStripColor(seg.health))}
    ...
    <Tooltip
      content={
        <div className="space-y-0.5">
          ...
          {more > 0 && <div className="text-muted-foreground/75">…and {more} more</div>}
          <div className="text-muted-foreground/75">click to browse</div>
        </div>
      }
      position="top"
      delay={100}
      wrapperClassName={cn('absolute top-1/2 -translate-y-1/2 z-20', markerAnchor(cluster.x).anchorClass)}
      wrapperStyle={{ left: markerAnchor(cluster.x).left }}
    >
      <button
        aria-label={`Open ${cluster.count} clustered events`}
        className={cn(
          'inline-flex items-center gap-1 rounded-full border border-border bg-card shadow-sm transition-transform',
      ...
      >
        ...
        {isCriticalIssue(cluster.dominant)
          ? <Ban className={cn('text-destructive dark:text-destructive', small ? 'h-2.5 w-2.5' : 'h-3 w-3')} />
          : <MarkerGlyph shape={eventShape(cluster.dominant)} size={small ? 9 : 11} className={eventColorClass(cluster.dominant)} />}
        <span className="text-[11px] font-semibold tabular-nums text-foreground">×{cluster.count}</span>
      </button>
    </Tooltip>
  )
}
...
function EventMarker({ event, x, selected, onClick, dimmed, small }: EventMarkerProps) {
  ...
  const getMarkerStyle = () => {
    ...
    if (isHistorical) {
      ...
      if (isProblematic) {
        return 'bg-warning/20 border-2 border-dashed border-warning/60'
      }
      if (isChange) {
        switch (event.eventType) {
          case 'add':
            return 'bg-success/20 border-2 border-dashed border-success/60'
          case 'delete':
            return 'bg-destructive/20 border-2 border-dashed border-destructive/60'
          case 'update':
            return 'bg-primary/20 border-2 border-dashed border-primary/60'
        }
      }
      return 'bg-accent/30 border-2 border-dashed border-border/60'
    }
    ...
    if (isProblematic) {
      return dimmed ? 'bg-warning/50' : 'bg-warning'
    }
    if (isChange) {
      switch (event.eventType) {
        case 'add':
          return dimmed ? 'bg-success/50' : 'bg-success'
        case 'delete':
          return dimmed ? 'bg-destructive/50' : 'bg-destructive'
        case 'update':
          return dimmed ? 'bg-blue-500/50' : 'bg-blue-500'
      }
    }
    return dimmed ? 'bg-muted-foreground/50' : 'bg-muted-foreground/75'
  }
  ...
  if (isCritical && IssueIcon && !small) {
    return (
      <Tooltip
        ...
        delay={100}
        className="max-w-sm!"
        wrapperClassName={cn('absolute top-1/2 -translate-y-1/2 z-20', markerAnchor(x).anchorClass)}
      ...
      >
        <button
          ...
          className={cn(
            ...
            'w-5 h-5',
            markerClasses,
            selected ? 'ring-2 ring-white ring-offset-2 ring-offset-background scale-125' : 'hover:scale-110',
            'shadow-sm'
          )}
          onClick={(e) => {
            e.stopPropagation()
  ...
  return (
    ...
    >
      ...
      >
        <MarkerGlyph shape={shape} size={size} className={cn(eventColorClass(event), selected && 'drop-shadow')} />
      </button>
    </Tooltip>
  )
}
    ...
    <li>
      ...
      >
        ...
        <span className="flex-1 min-w-0">
          <span className={cn('block text-xs font-medium truncate', isProblematic ? 'text-warning dark:text-warning' : 'text-foreground')}>
            {event.reason || event.eventType}
          </span>
          <span className="block text-[11px] text-muted-foreground/75 truncate font-mono">{event.name}</span>
        </span>
        <span className="text-[11px] text-muted-foreground/75 tabular-nums shrink-0">
...
export function EventDetailPanel({ events, selectedId, onSelectId, onClose, onResourceClick, allEvents }: EventDetailPanelProps) {
  ...
  return (
    ...
    >
      ...
      >
        <span className="h-1 w-10 rounded-full bg-border opacity-0 transition-opacity group-hover:opacity-100" />
      </div>
      ...
      <div className="flex shrink-0 items-center justify-between gap-3 border-b border-border px-4 py-2">
        <div className="flex min-w-0 items-center gap-2 text-sm">
          ...
          <span className={cn('shrink-0 font-medium', isProblematic ? 'text-warning dark:text-warning' : 'text-foreground')}>
            {selected.reason || selected.eventType}
          </span>
          <span className="badge-sm shrink-0 bg-popover text-muted-foreground">{displayKind(selected.kind) || 'Event'}</span>
          <span className="min-w-0 truncate font-medium text-foreground">{selected.name}</span>
          {selected.namespace && <span className="shrink-0 text-xs text-muted-foreground/75">in {selected.namespace}</span>}
          <span className="shrink-0 text-xs tabular-nums text-muted-foreground/75">{formatFullTime(new Date(selected.timestamp))}</span>
          {events.length > 1 && (
            <span className="badge-sm shrink-0 bg-popover text-muted-foreground">{events.length} events</span>
          )}
        </div>
        <div className="flex shrink-0 items-center gap-1">
          ...
          <span className="text-xs tabular-nums text-muted-foreground/75">{`${selIdx + 1} of ${railEvents.length}`}</span>
          <button
            onClick={() => step(1)}
          ...
          <Tooltip content="Close (Esc)">
            <button
              onClick={onClose}
              aria-label="Close"
              className="ml-1 rounded p-1.5 text-muted-foreground hover:bg-popover hover:text-foreground"
            >
              <X className="h-4 w-4" />
            </button>
          </Tooltip>
      ...
      <div className="flex min-h-0 flex-1">
        ...
        <ul className="w-72 shrink-0 overflow-auto border-r border-border py-1" aria-label="Drawer events">
          {railEvents.map((ev, i) => (
            <Fragment key={ev.id}>
              {i === neighborsFrom && (
                <li aria-hidden className="mt-1 border-t border-border px-3 pb-1 pt-2 text-[10px] font-bold uppercase tracking-[0.07em] text-muted-foreground/75">
                  Within ±15 min
                </li>
              )}
        ...
        <div className="min-w-0 flex-1 overflow-auto p-4">
          <dl className="grid grid-cols-[96px_1fr] items-baseline gap-x-3 gap-y-2 text-sm">
            <dt className="text-xs text-muted-foreground/75">Reason</dt>
            <dd className="flex flex-wrap items-center gap-2">
              <span className={cn(
                'font-medium',
                isProblematic ? 'text-warning dark:text-warning' : 'text-foreground',
              )}>
                {selected.reason || selected.eventType}
              </span>
              {!isChangeEvent(selected) && selected.eventType && (
                <span className={cn('badge-sm', getEventTypeColor(selected.eventType))}>{selected.eventType}</span>
              )}
              {selected.count && selected.count > 1 && (
                <span className="text-xs text-muted-foreground/75">×{selected.count}</span>
              )}
            </dd>
            <dt className="text-xs text-muted-foreground/75">Resource</dt>
            <dd className="flex min-w-0 flex-wrap items-center gap-1.5">
              {/* The ONE click target for the resource — the same badge used by
                  every Opsia drawer, so it reads as "this navigates". */}
              <ResourceRefBadge
              ...
              />
              {selected.namespace && <span className="text-xs text-muted-foreground/75">in {selected.namespace}</span>}
            </dd>
            <dt className="text-xs text-muted-foreground/75">Time</dt>
            ...
            {selected.healthState && selected.healthState !== 'unknown' && (
              <>
                <dt className="text-xs text-muted-foreground/75">Health</dt>
                <dd><span className={cn('badge-sm', getHealthBadgeColor(selected.healthState))}>{selected.healthState}</span></dd>
              </>
            )}
          </dl>
          {selected.message && (
            <p className={cn('mt-3 text-sm', isProblematic ? 'text-warning dark:text-warning' : 'text-muted-foreground')}>
              {selected.message}
            </p>
          )}
          {selected.diff && (
