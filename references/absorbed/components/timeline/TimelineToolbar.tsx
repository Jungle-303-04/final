...
export interface TimelineToolbarProps {
  ...
  // Manual refresh button. Opsia's own timeline omits it (the view is live);
...
export function TimelineToolbar({
  ...
  return (
    ...
    <div className="@container/toolbar border-b border-border bg-card/50">
      <div className="flex items-center gap-3 px-4 py-2">
        ...
        <div className="-m-1 flex min-w-0 items-center gap-2 overflow-x-auto p-1">
          ...
          {showRange && (
            <select
              ...
              className="shrink-0 appearance-none bg-popover text-foreground text-sm rounded-lg px-3 py-2 border border-border/60 focus:outline-none focus:ring-2 focus:ring-primary"
            >
        ...
        <div className="ml-auto flex min-w-0 shrink-0 items-center gap-2">
          ...
          {onRefresh && (
            <Tooltip content="Refresh" position="bottom" wrapperClassName="shrink-0">
              <button
                ...
                aria-label="Refresh"
                className="p-2 text-muted-foreground hover:text-foreground hover:bg-popover rounded-lg disabled:opacity-50"
              >
                <RefreshCw className={cn('w-4 h-4', isRefreshAnimating && 'animate-spin')} />
              </button>
            </Tooltip>
          )}
            ...
            <button
              ...
              className={cn(
                ...
                legend.shown
                  ...
                  : 'border-border/60 text-muted-foreground hover:bg-popover hover:text-foreground',
              )}
            >
              Legend
            </button>
          ...
          {/* View toggle — labeled, and LAST so it hugs the toolbar's right
              ...
              re-aiming. */}
          {onViewChange && (
            <div className="flex shrink-0 items-center gap-1 bg-popover rounded-lg p-1">
              <button
                type="button"
              ...
              <button
                ...
                className={cn(
                  'flex items-center gap-1.5 px-2.5 py-1.5 rounded-md text-xs font-medium transition-colors',
                  view === 'swimlane' ? 'bg-accent text-foreground' : 'text-muted-foreground hover:text-foreground'
                )}
              >
    ...
    <Tooltip content={tooltip} disabled={!tooltip || dead} position="bottom" wrapperClassName="shrink-0">
      <button
        ...
        className={cn(
          ...
          active
            ? 'border-border/60 bg-accent font-semibold text-foreground'
            : 'border-border text-muted-foreground',
          dead ? 'cursor-default opacity-45' : !active && 'hover:bg-accent hover:text-foreground',
        )}
      >
        <span>{label}</span>
        {count !== undefined && (
          <span className="tabular-nums text-xs text-muted-foreground/75">{count.toLocaleString()}</span>
        )}
      </button>
    </Tooltip>
  )
    ...
    <Tooltip content={tooltip} position="bottom" disabled={dead} wrapperClassName="shrink-0">
      <button
        ...
        className={cn(
          'flex items-center gap-1.5 whitespace-nowrap rounded-full border px-2.5 py-1.5 text-sm transition-colors',
          active
            ? 'border-warning/60 bg-warning font-semibold text-warning dark:border-warning/40 dark:bg-warning/50 dark:text-warning'
            : 'border-border text-muted-foreground',
          dead ? 'cursor-default opacity-45' : !active && 'hover:bg-accent hover:text-foreground',
        )}
      >
        ...
        <span>Problems</span>
        <span className={cn('tabular-nums text-xs', active ? 'text-warning/80 dark:text-warning/80' : 'text-muted-foreground/75')}>
  ...
  return (
    <div ref={rootRef} className="relative shrink-0">
      <button
        ...
        onClick={() => setOpen((o) => !o)}
        className={cn(
          'flex items-center gap-1.5 rounded-lg border border-border/60 px-2.5 py-1.5 text-sm font-medium transition-colors',
          open
      ...
      {open && (
        <div
          ...
          className="absolute right-0 top-full z-50 mt-1 min-w-[14rem] rounded-lg border border-border bg-popover p-2 shadow-lg"
        >
          <span className="block px-2 pb-1 text-[10px] font-bold uppercase tracking-[0.08em] text-muted-foreground/75">Sort</span>
          <SegmentedRadioGroup
            label="Lane sort"
          ...
          />
          <span aria-hidden className="my-2 block h-px bg-border" />
          <span className="block px-2 pb-1 text-[10px] font-bold uppercase tracking-[0.08em] text-muted-foreground/75">Group by</span>
          <SegmentedRadioGroup
            label="Lane grouping"
    ...
    >
      <button
        ...
        className={cn(
          'flex items-center gap-1.5 whitespace-nowrap rounded-full border px-2.5 py-1.5 text-sm transition-colors',
          showDeleted
            ? 'border-dashed border-border/60 text-muted-foreground hover:bg-accent hover:text-foreground'
            : 'border-accent bg-accent-muted text-accent-text',
        )}
      >
