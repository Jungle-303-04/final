...
export function LabelSelectorDisplay({
  ...
  if (!selector) {
    return <span className="text-xs text-muted-foreground/75">{emptyText}</span>
  }
  ...
  return (
    <Wrapper className={inline ? 'inline-flex flex-wrap gap-1 align-middle' : 'flex flex-wrap gap-1'}>
      ...
      {matchExpressions.map((expr, i) => (
        <span key={`e-${i}`} className="badge bg-popover text-muted-foreground">
...
export function Section({ title, icon: Icon, children, defaultExpanded = true }: SectionProps) {
  ...
  return (
    <div className="border-b-subtle pb-4 last:border-0">
      ...
      >
        <ChevronRight className={cn('w-4 h-4 text-muted-foreground/75 transition-transform duration-200', expanded && 'rotate-90')} />
...
export function Property({ label, value, copyable, onCopy, copied }: PropertyProps) {
  ...
  return (
    <div className="flex items-start gap-2 text-sm">
      <span className="text-muted-foreground/75 w-40 shrink-0">{label}</span>
      <span className={cn('text-foreground flex-1 min-w-0', isReactElement(value) ? 'break-normal' : 'break-all')}>{displayValue}</span>
      {copyable && onCopy && !isReactElement(value) && (
        <button
          onClick={() => onCopy(strValue, labelKey)}
          className="p-0.5 text-muted-foreground/75 hover:text-foreground shrink-0"
        >
          ...
          {sorted.map((cond: any) => {
            ...
            return (
              <div key={cond.type} className={cn(
                ...
                isFail && 'border-l-2 border-destructive/60 dark:border-destructive/40',
                isWarning && 'border-l-2 border-warning/60 dark:border-warning/40'
              )}>
                {/* Timestamp column — fixed width on the left */}
                <div className="w-[48px] shrink-0 text-[10px] text-muted-foreground/75 text-right pr-2 pt-0.5">
                  {cond.lastTransitionTime ? formatDuration(Date.now() - new Date(cond.lastTransitionTime).getTime(), true) : ''}
                </div>
                {/* Timeline dot */}
                <span className={cn(
                  'w-3 h-3 rounded-full flex items-center justify-center shrink-0 mt-1 z-10 ring-2 ring-card',
                  isOk ? 'bg-success/20 text-success dark:bg-success/30'
                    : tone === 'unknown' ? 'bg-gray-400/20 text-gray-400 dark:bg-gray-400/30'
                    : isWarning ? 'bg-warning/25 text-warning dark:text-warning dark:bg-warning/35'
                    : 'bg-destructive/25 text-destructive dark:bg-destructive/35'
                )}>
                ...
                <div className="min-w-0 flex-1 pl-2">
                  <span className={cn(
                    ...
                    isOk ? 'text-foreground'
                      : tone === 'unknown' ? 'text-muted-foreground'
                        : isWarning ? 'text-warning dark:text-warning'
                          : 'text-destructive dark:text-destructive'
                  )}>{cond.type}</span>
                  {cond.reason && cond.reason !== cond.type && (
                    <div className="text-[10px] text-muted-foreground">{cond.reason}</div>
                  )}
                  {cond.message && <div className="text-[10.5px] text-muted-foreground/75 break-words leading-relaxed">{cond.message}</div>}
                </div>
              </div>
            )
          })}
...
const ALERT_COLORS = {
  error:   { bg: 'bg-destructive dark:bg-destructive/30', border: 'border-destructive dark:border-destructive/30', title: 'text-destructive dark:text-destructive', message: 'text-destructive/80 dark:text-destructive/70', list: 'text-destructive dark:text-destructive', bullet: 'text-destructive/60 dark:text-destructive/50' },
  warning: { bg: 'bg-warning dark:bg-warning/30', border: 'border-warning dark:border-warning/30', title: 'text-warning dark:text-warning', message: 'text-warning/80 dark:text-warning/70', list: 'text-warning dark:text-warning', bullet: 'text-warning/60 dark:text-warning/50' },
  ...
  success: { bg: 'bg-success dark:bg-success/30', border: 'border-success dark:border-success/30', title: 'text-success dark:text-success', message: 'text-success/80 dark:text-success/70', list: 'text-success dark:text-success', bullet: 'text-success/60 dark:text-success/50' },
} as const
...
export function AlertBanner({ variant, icon, title, message, items, children }: AlertBannerProps) {
  ...
  return (
    <div className={cn('mb-4 p-3 border rounded-lg', colors.bg, colors.border)}>
      <div className={cn('flex gap-2', hasBody ? 'items-start' : 'items-center')}>
        <Icon className={cn('w-4 h-4 shrink-0', colors.title, hasBody && 'mt-0.5')} />
        {hasBody ? (
          <div className="flex-1 min-w-0">
            <div className={cn('text-sm font-medium', colors.title, items && 'mb-1')}>{title}</div>
            {message && <div className={cn('text-xs mt-1 break-words', colors.message)}>{message}</div>}
            {items && items.length > 0 && (
              <ul className={cn('text-xs space-y-1', colors.list)}>
                {items.map((item, i) => (
                  <li key={i} className="flex items-start gap-1.5">
                    <span className={cn(colors.bullet, 'mt-0.5')}>•</span>
                    <span>{item}</span>
                  </li>
                ))}
              </ul>
      ...
      <PropertyList>
        ...
        <Property label="Created" value={meta.creationTimestamp ? (
          <Tooltip content={new Date(meta.creationTimestamp).toLocaleString()}>
            <span className="border-b border-dotted border-muted-foreground/75 cursor-help">{formatAge(meta.creationTimestamp)}</span>
          </Tooltip>
        ) : '-'} />
      </PropertyList>
    </Section>
  ...
  return (
    <div className="space-y-2">
      {initContainers.length > 0 && (
        <>
          <div className="text-xs text-muted-foreground/75 font-medium uppercase tracking-wide">Init Containers</div>
          {initContainers.map((c: any) => (
            <div key={c.name} className="card-inner text-sm border-l-2 border-warning/40">
              <div className="font-medium text-foreground">{c.name}</div>
              <Tooltip content={c.image} delay={300} wrapperClassName="block min-w-0">
                <div className="text-xs text-muted-foreground truncate">{c.image}</div>
              </Tooltip>
              {(c.command || c.args) && (
                <Tooltip content={[...(c.command || []), ...(c.args || [])].join(' ')} delay={300} wrapperClassName="block min-w-0">
                  <div className="text-xs text-muted-foreground/75 font-mono mt-1 truncate">
          ...
          <div className="text-xs text-muted-foreground/75 font-medium uppercase tracking-wide mt-3">Containers</div>
        </>
      )}
...
function RelationshipGroup({ label, refs, onNavigate }: RelationshipGroupProps) {
  ...
  return (
    <div>
      <div className="text-xs text-muted-foreground/75 mb-1">{label}{refs.length > 1 ? ` (${refs.length})` : ''}</div>
      <div className="flex flex-wrap gap-1">
        ...
        {truncated && (
          <button
            ...
            className="px-2 py-0.5 text-xs rounded border border-border text-muted-foreground hover:text-foreground hover:bg-popover transition-colors"
          >
...
export function ResourceRefBadge({ resourceRef, onClick, wrapAtSeparator }: ResourceRefBadgeProps) {
  ...
  return (
    <Tooltip content={tooltip} delay={300}>
      <span className={cn(layoutClass, kindClass)}>
        {content}
      </span>
    </Tooltip>
  )
...
export function EventsSection({ events, updates = [], isLoading, eventsError, updatesError, hint }: EventsSectionProps) {
  ...
  if (isLoading) {
    return (
      <Section title="Recent Events" defaultExpanded>
        <div className="text-sm text-muted-foreground/75">Loading events…</div>
      </Section>
    )
  }
  ...
  const errors = (
    <>
      {eventsError && (
        <div className="text-xs text-destructive mt-2">Failed to load K8s events: {eventsError.message}</div>
      )}
      {updatesError && (
        <div className="text-xs text-destructive mt-2">Failed to load resource changes: {updatesError.message}</div>
      )}
    </>
  )
  ...
  if (visible.length === 0) {
    return (
      <Section title="Recent Events" defaultExpanded={!!(eventsError || updatesError)}>
        <div className="text-sm text-muted-foreground/75">No recent events</div>
        {errors}
