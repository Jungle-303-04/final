...
export function BoardTableSkeleton({ rows = 10 }: { rows?: number }) {
  return (
    <div aria-live="polite" aria-label="Loading…" className="divide-y divide-border/60">
      {Array.from({ length: rows }, (_, i) => (
        <div key={i} className="flex items-center gap-6 px-4 py-3" style={{ opacity: 1 - i * 0.09 }}>
          ...
          <div className="hidden h-4 flex-1 animate-pulse rounded bg-accent md:block" />
          <div className="h-5 w-20 animate-pulse rounded-full bg-accent" />
          <div className="h-5 w-20 animate-pulse rounded-full bg-accent" />
          <div className="hidden h-4 flex-[1.5] animate-pulse rounded bg-accent lg:block" />
        </div>
      ))}
    </div>
  )
  ...
  return (
    <div aria-hidden>
      {sections.map(([title, rows]) => (
        <div key={title} className="border-b border-border/60 px-3 py-3">
          <div className="mb-2 h-3 w-24 animate-pulse rounded bg-accent" />
          <div className="space-y-1.5">
            {Array.from({ length: rows }, (_, i) => (
              <div key={i} className="flex items-center justify-between px-1.5 py-1">
                <div className="h-3.5 animate-pulse rounded bg-accent" style={{ width: `${52 - i * 6}%` }} />
                <div className="h-3.5 w-6 animate-pulse rounded bg-accent" />
              </div>
            ))}
          </div>
        </div>
