...
export function HealthRing({ segments, size = 48, strokeWidth = 5, label }: HealthRingProps) {
  ...
  if (total === 0) {
    return (
      <svg width={size} height={size} viewBox="0 0 100 100" className="shrink-0">
        ...
        />
        {label && (
          <text x="50" y="48" textAnchor="middle" dominantBaseline="central" fill="currentColor" className="text-muted-foreground/75 text-[22px] font-semibold font-mono">
            0
          </text>
        )}
      </svg>
  ...
  return (
    <svg width={size} height={size} viewBox="0 0 100 100" className="shrink-0">
      ...
      {label && (
        <text x="50" y="48" textAnchor="middle" dominantBaseline="central" fill="currentColor" className="text-foreground text-[22px] font-semibold font-mono">
          {label}
        </text>
      )}
    </svg>
