...
export function FreshnessControl({
  ...
  const labelNode = label ? (
    <span className="flex items-center gap-1 text-xs text-muted-foreground/75">
      {(live || (mode === 'auto' && paused)) && !degraded && (
        <span
          className={cn('w-1.5 h-1.5 rounded-full', paused ? 'bg-warning' : 'bg-success animate-pulse')}
          aria-hidden
        />
      )}
      <span className="tabular-nums">{label}</span>
      {ageSuffix && <span className="tabular-nums text-muted-foreground/55">· updated {ageSuffix}</span>}
    </span>
  ) : null
      ...
      {showRefresh && (
        <Tooltip content="Refresh now" delay={100} position="bottom">
          <button
            ...
            className="p-1.5 rounded-lg text-muted-foreground/75 hover:text-muted-foreground hover:bg-accent transition-colors disabled:opacity-50"
          >
            {phase === 'success' ? (
