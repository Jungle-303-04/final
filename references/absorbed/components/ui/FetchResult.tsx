...
// @opsia-io/k8s-ui without importing ApiError from either web/ (OSS)
// or opsia-hub-web.
...
export function FetchResult({
  ...
  if (error === undefined || error === null) {
    ...
    return (
      <div className={cn('flex items-center justify-center text-muted-foreground/75', className)}>
        {notFoundMessage}
      </div>
    )
  }
...
function ErrorSurface({ error, notFoundMessage, className }: ErrorSurfaceProps) {
  ...
  return (
    ...
    >
      <Icon className="h-5 w-5 text-muted-foreground/75" aria-hidden />
      <div className="text-sm font-medium text-muted-foreground">{classified.headline}</div>
      {classified.detail && (
        <div className="flex items-center gap-2 max-w-md">
          <span className="text-xs text-muted-foreground/75 break-words">{classified.detail}</span>
...
function CopyErrorButton({ text }: { text: string }) {
  ...
  return (
    <button
      type="button"
      onClick={onCopy}
      className="flex-shrink-0 p-1 rounded text-muted-foreground/75 hover:text-muted-foreground hover:bg-accent"
