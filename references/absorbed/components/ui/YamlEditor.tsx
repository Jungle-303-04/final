...
export function YamlEditor({
  ...
  useEffect(() => {
    return () => { delete (window as any).__opsiaMonacoEditor }
  }, [])

  const handleEditorMount: OnMount = useCallback((editor, monaco) => {
    editorRef.current = editor
    monacoRef.current = monaco
    ;(window as any).__opsiaMonacoEditor = editor
...
export function YamlDiffEditor({
  ...
  return (
    <div className={bleed ? 'overflow-hidden' : 'rounded-lg overflow-hidden border border-border'} style={{ height }}>
      <DiffEditor
        original={original}
        modified={modified}
        language="yaml"
