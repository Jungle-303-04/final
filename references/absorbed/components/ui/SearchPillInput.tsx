...
// query back via onChange. Shared so Opsia (standalone) and Opsia Cloud can both
...
function highlightPartial(text: string, partial: string): React.ReactNode {
  ...
  return (
    <>
      {text.slice(0, i)}
      <span className="font-semibold text-[var(--primary)]">{text.slice(i, i + partial.length)}</span>
...
export function SearchPillInput({
  ...
  return (
    <div ref={containerRef} className={cn('flex items-center gap-1.5', pillsExpanded && 'flex-wrap', className)} onClick={() => inputRef.current?.focus()}>
      {leftSlot}
      {shownPills.map((p, i) => (
        <span key={`${p.key}:${p.value}:${i}`} className="inline-flex items-center gap-1 shrink-0 rounded-md bg-popover border border-border/60 pl-1.5 pr-1 py-0.5 text-xs whitespace-nowrap">
          <span className="text-muted-foreground/75">{p.key}:</span>
          <span className="max-w-[16ch] truncate font-medium text-foreground" title={p.value}>{p.value}</span>
          <button
            type="button"
            tabIndex={-1}
      ...
      {collapsePills && hiddenPillCount > 0 && (
        <button
          ...
          className="shrink-0 rounded-md bg-popover border border-border/60 px-1.5 py-0.5 text-xs font-medium text-muted-foreground hover:text-foreground whitespace-nowrap"
      ...
      {pillsExpanded && pills.length > MAX_VISIBLE_PILLS + 1 && (
        <button
          ...
          className="shrink-0 rounded-md px-1.5 py-0.5 text-xs text-muted-foreground/75 hover:text-foreground whitespace-nowrap"
        >
          less
        </button>
      )}
      <Input
        ...
        className={cn(
          'flex-1 min-w-[80px] bg-transparent text-foreground placeholder-muted-foreground/75 outline-none',
      ...
      {suggesting && anchor && mod && createPortal(
        <div
          ...
          className="z-[130] dialog shadow-lg overflow-hidden"
        >
          <div className="px-2.5 py-1 text-[10px] font-semibold uppercase tracking-wider text-muted-foreground/75">{mod.canon}</div>
          <div className="max-h-56 overflow-y-auto pb-1">
            {filtered.map((opt, i) => (
              <button
                ...
                className={cn('w-full text-left px-2.5 py-1 text-sm truncate', i === sel ? 'selection' : 'hover:bg-popover/40 text-foreground')}
              >
