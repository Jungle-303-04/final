  ...
  return (
    <DialogPortal open={open} onClose={handleClose} closable={!isApplying} className="w-[700px] max-h-[85vh] flex flex-col">
      ...
      <div className="flex items-center justify-between px-5 py-3.5 border-b border-border shrink-0">
        <h2 className="text-sm font-semibold text-foreground">{dialogTitle}</h2>
      ...
      {success && (
        <div className="mx-5 mb-2 px-3 py-2 rounded-md bg-success/10 border border-success/30 text-success text-xs flex items-center gap-2">
      ...
      <div className="flex items-center justify-between px-5 py-3 border-t border-border shrink-0">
        <div className="flex items-center gap-3">
          ...
          <Tooltip content="Apply: create or update (idempotent). Create: fail if exists." position="bottom">
            <div className="flex items-center rounded-md bg-background border border-border p-0.5" role="radiogroup" aria-label="Apply mode">
              <button
          ...
          <Tooltip
            content={
              <div className="space-y-1.5 max-w-[19rem]">
                ...
                <p>
                  <span className="font-medium text-foreground">Off (default):</span> if a field is owned by Helm/Flux/Argo/kubectl, the whole apply is rejected with a conflict instead of overwriting.
                </p>
                <p>
                  <span className="font-medium text-foreground">On:</span> your manifest overwrites those fields — but an active controller may reconcile them back on its next sync.
                </p>
              </div>
            }
            position="bottom"
          >
            <label className={`flex items-center gap-1.5 text-xs cursor-pointer ${mode === 'apply' ? 'text-muted-foreground' : 'text-muted-foreground/75 cursor-not-allowed'}`}>
            <input
              type="checkbox"
        ...
        <div className="flex items-center gap-2">
          <button
            onClick={handleClose}
            className="px-3 py-1.5 text-xs rounded-lg hover:bg-accent text-muted-foreground transition-colors"
          >
            Cancel
          </button>
          <button
...
function ApplyErrorBanner({ error }: { error: string }) {
  ...
  return (
    <div className="mx-5 mb-2 rounded-md bg-destructive/10 border border-destructive/30 text-xs">
      <div className="px-3 py-2 flex items-start gap-2 text-destructive">
        <AlertTriangle className="w-3.5 h-3.5 mt-0.5 shrink-0" />
        <div className="min-w-0 flex-1">
          <span className="font-medium">{parsed.summary}</span>
          {parsed.suggestion && (
            <p className="mt-1 text-destructive/80">{parsed.suggestion}</p>
          )}
        </div>
      </div>
      {hasFriendly && (
        <button
          ...
          className="flex items-center gap-1 px-3 pb-2 text-destructive/60 hover:text-destructive/80 transition-colors"
        >
      ...
      {(expanded || !hasFriendly) && hasFriendly && (
        <div className="px-3 pb-2 text-destructive/60 break-all font-mono leading-relaxed">
          {parsed.raw}
        </div>
      )}
    </div>
