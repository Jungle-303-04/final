  ...
  return (
    <div className="absolute top-0 left-0 right-0 z-50 pointer-events-none">
      <div className="flex justify-center animate-fade-in-out">
        <div className="mt-2 px-4 py-2 bg-success/90 dark:bg-success/90 backdrop-blur-sm rounded-lg shadow-lg flex items-center gap-2">
...
export function EditableYamlView({ resource, data, onCopy, copied, readOnly = false, onSaved, onSave, isSaving, saveError, onDuplicate, onDownload }: EditableYamlViewProps) {
  const draftKey = `opsia_yaml_draft:${resource.kind}/${resource.namespace}/${resource.name}`
  ...
  if (isEditing) {
    return (
      <div className="flex flex-col h-full">
        <div className="flex items-center justify-between px-4 py-2 border-b border-border bg-popover/50">
          <div className="flex items-center gap-2">
            <Pencil className="w-4 h-4 text-blue-400" />
            <span className="text-sm font-medium text-foreground">Editing Resource</span>
          </div>
          <div className="flex items-center gap-2">
            <button
              onClick={handleCancelEdit}
              disabled={isPending}
              className="flex items-center gap-1 px-3 py-1.5 text-xs text-muted-foreground hover:text-foreground hover:bg-card rounded border border-border disabled:opacity-50"
            >
            ...
            <Tooltip
              content={
                <div className="space-y-1.5 max-w-[19rem]">
                  ...
                  <p>
                    <span className="font-medium text-foreground">On (default):</span> your edits overwrite fields owned by Helm/Flux/Argo/kubectl — but an active controller may reconcile them back on its next sync.
                  </p>
                  <p>
                    <span className="font-medium text-foreground">Off:</span> if another manager owns a field you changed, the whole save is rejected with a conflict (nothing is applied).
                  </p>
                </div>
              }
              position="bottom"
        ...
        {editWarning && (
          <div className="px-4 py-2.5 bg-warning/10 dark:bg-warning/10 border-b border-warning dark:border-warning/30">
            <div className="flex items-start gap-2">
              <AlertTriangle className="w-4 h-4 text-warning dark:text-warning mt-0.5 shrink-0" />
              <div className="text-xs">
                <span className="font-medium text-warning dark:text-warning">{editWarning.message}</span>
                <span className="text-warning dark:text-warning/80 ml-1">{editWarning.tip}</span>
        ...
        {yamlErrors.length > 0 && (
          <div className="px-4 py-2 bg-destructive/10 border-b border-destructive dark:border-destructive/30">
            <div className="flex items-start gap-2">
              <AlertTriangle className="w-4 h-4 text-destructive dark:text-destructive mt-0.5 shrink-0" />
        ...
        {formattedError && (
          <div className="px-4 py-2 bg-destructive/10 border-b border-destructive dark:border-destructive/30">
            <div className="flex items-start gap-2">
              <AlertTriangle className="w-4 h-4 text-destructive dark:text-destructive mt-0.5 shrink-0" />
              <div className="text-xs text-destructive dark:text-destructive flex-1">
                <div className="font-medium">Save failed</div>
                <div className="mt-1">{formattedError.summary}</div>
                {formattedError.details && (
                  <button
                    onClick={() => setShowErrorDetails(!showErrorDetails)}
                    className="mt-1 text-destructive dark:text-destructive hover:text-destructive dark:hover:text-destructive underline"
                  >
                    {showErrorDetails ? 'Hide details' : 'Show details'}
                  </button>
                )}
                {showErrorDetails && formattedError.details && (
                  <pre className="mt-2 p-2 bg-destructive/10 rounded text-[10px] whitespace-pre-wrap break-all max-h-40 overflow-auto">
                    {formattedError.details}
                  </pre>
                )}
              </div>
  ...
  return (
    <div className="p-4">
      <div className="flex items-center justify-between mb-2">
        <span className="text-sm font-medium text-muted-foreground">YAML</span>
        <div className="flex items-center gap-2">
          {!readOnly && (
            <button
              onClick={handleStartEdit}
              className="flex items-center gap-1 px-2 py-1 text-xs text-blue-400 hover:text-blue-300 hover:bg-popover rounded"
            >
          ...
          <button
            ...
            className="flex items-center gap-1 px-2 py-1 text-xs text-muted-foreground hover:text-foreground hover:bg-popover rounded"
          >
