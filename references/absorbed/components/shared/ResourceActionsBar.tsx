...
interface ResourceActionsBarProps {
   ...
   * Only embedded hosts (Opsia Hub) wire this; standalone Opsia leaves it
          ...
          {canViewLogs && !hideLogs && openLogs && (
            ...
            >
              ...
              {showLogsMenu && containers.length > 1 && (
                <div className="absolute top-full left-0 mt-1 min-w-[160px] py-1 bg-card border border-border rounded-lg shadow-lg z-50">
                  {containers.map((container: string) => (
                    ...
                    >
                      <Box className="w-3 h-3 text-muted-foreground/75 shrink-0" />
            ...
            <>
              {data?.spec?.unschedulable ? (
                onUncordonNode && (
                  <button
                    ...
                    className="flex items-center gap-1.5 px-3 py-1.5 text-xs font-medium text-white bg-success hover:bg-success rounded-lg transition-colors disabled:opacity-50"
                  >
                ...
                onCordonNode && (
                  <button
                    ...
                    className="flex items-center gap-1.5 px-3 py-1.5 text-xs font-medium text-white bg-warning hover:bg-warning rounded-lg transition-colors disabled:opacity-50"
                  >
              ...
              {onDrainNode && (
                <button
                  ...
                  className="flex items-center gap-1.5 px-3 py-1.5 text-xs font-medium text-white bg-destructive hover:bg-destructive rounded-lg transition-colors disabled:opacity-50"
                >
              ...
              <button
                ...
                className={cn(
                  ...
                  hasMultipleRevisions
                    ...
                    : "text-muted-foreground/45 bg-popover"
                )}
              >
        ...
        <Tooltip content="Toggle YAML view" delay={150}>
          <button
            ...
            className={cn(
              ...
              showYaml
                ? 'btn-brand'
                : 'text-muted-foreground hover:text-foreground border border-border/60 hover:bg-popover'
            )}
          >
        ...
        >
          ...
          >
            <button
              ...
              className="p-1.5 text-muted-foreground border border-border/60 rounded-lg hover:text-foreground hover:bg-popover transition-colors flex items-center"
            >
          ...
          {showCompareMenu && onCompareTo && onCompareAcrossClusters && (
            <div className="absolute top-full right-0 mt-1 min-w-[220px] py-1 bg-card border border-border rounded-lg shadow-lg z-50">
              <button
                onClick={() => { onCompareTo(); setShowCompareMenu(false) }}
                className="flex items-center gap-2 w-full px-3 py-1.5 text-xs text-foreground hover:bg-accent transition-colors text-left"
              >
                <GitCompare className="w-3 h-3 text-muted-foreground/75 shrink-0" />
      ...
      {onDelete && (
        <Tooltip content="Delete resource">
          <button
            ...
            className="p-1.5 text-muted-foreground border border-border/60 rounded-lg hover:text-destructive hover:border-destructive/50 hover:bg-destructive/10 transition-colors"
          >
  ...
  return (
    ...
    >
      ...
      <div className="flex-1 min-h-0 flex flex-col overflow-hidden">
        <div className={cn("p-4 overflow-y-auto", diffRevision ? "max-h-48 shrink-0" : "max-h-80")}>
          {isLoading && (
              ...
              <tbody>
                {revisions.map((rev: WorkloadRevision) => (
                  ...
                  >
                    ...
                    <td className="py-2 pr-3 text-muted-foreground font-mono max-w-[180px]">
                        ...
                        {rev.isCurrent ? (
                          ...
                          <>
                            <button
                              ...
                              disabled={isRollingBack}
                              className="px-2 py-0.5 text-xs font-medium text-white bg-warning hover:bg-warning rounded transition-colors disabled:opacity-50"
                            >
                            ...
                            <button
                              ...
                              disabled={isRollingBack}
                              className="px-2 py-0.5 text-xs font-medium text-muted-foreground hover:text-foreground rounded transition-colors disabled:opacity-50"
                            >
                              Cancel
                            </button>
                          </>
                          ...
                          <button
                            ...
                            className="px-2 py-0.5 text-xs font-medium text-warning hover:text-white hover:bg-warning border border-warning/50 hover:border-warning rounded transition-colors"
                          >
                            Rollback
                          </button>
                        )}
...
function RevisionDiffView({ currentTemplate, selectedTemplate, currentRevision, selectedRevision }: {
  ...
  return (
    <div className="border-t border-border flex flex-col shrink-0">
      <div className="flex items-center justify-between px-4 py-2 bg-popover/50 text-xs text-muted-foreground shrink-0">
        <div className="flex items-center gap-4">
          <span className="flex items-center gap-1">
            <span className="w-3 h-3 bg-destructive/20 border border-destructive/50 rounded" /> Revision #{currentRevision} (current)
          </span>
          <span className="flex items-center gap-1">
            <span className="w-3 h-3 bg-success/20 border border-success/50 rounded" /> Revision #{selectedRevision}
          </span>
        </div>
        {hasChanges && (
          <button
            onClick={() => setExpanded(!expanded)}
            className="flex items-center gap-1 px-2 py-0.5 text-xs text-muted-foreground hover:text-foreground hover:bg-popover rounded transition-colors"
          >
      ...
      <div className="overflow-auto max-h-[400px]">
        {hasChanges ? (
            ...
            {diffLines.map((line, index) => {
              ...
              return (
                <div
                  ...
                  className={cn(
                    'flex',
                    isAddition && 'bg-success/10',
                    isDeletion && 'bg-destructive/10',
                    isHeader && 'bg-blue-500/10'
                  )}
                >
                  <span className="w-10 shrink-0 text-right pr-2 py-0.5 text-muted-foreground/45 select-none border-r border-border/50">
                    {index + 1}
                  </span>
                  <span
                    className={cn(
          ...
          <div className="flex flex-col items-center justify-center py-12 text-muted-foreground/75">
            <GitCompare className="w-8 h-8 mb-2 text-muted-foreground/45" />
