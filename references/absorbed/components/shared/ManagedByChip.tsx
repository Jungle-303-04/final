    ...
    <Tooltip content={tooltipContent} delay={500} position="bottom" wrapperClassName={variant === 'block' ? 'mt-1' : undefined}>
      <Wrapper
        {...(interactive
          ...
          : {})}
        className={cn(
          'inline-flex min-w-0 items-center gap-1 rounded border border-border bg-popover px-1.5 py-0.5 text-[11px] text-muted-foreground',
          interactive && 'hover:border-primary/60 hover:text-primary transition-colors',
        )}
      >
        <GitBranch className="h-3 w-3 shrink-0" />
        <span className="shrink-0 text-muted-foreground/75">{prefix}</span>
        <span className="truncate max-w-[180px]">{label}</span>
        {statusLabel && (
          <>
            <span className="mx-0.5 h-3 w-px shrink-0 bg-border" aria-hidden />
            <span className="inline-flex shrink-0 items-center gap-1 text-muted-foreground/75">
  ...
  return (
    <Tooltip content={tooltipContent} delay={500} position="bottom" wrapperClassName={variant === 'block' ? 'mt-1' : undefined}>
      ...
      >
        ...
        <span className="shrink-0 text-muted-foreground/75">Managed by Helm</span>
        <span className="truncate max-w-[180px]">{label}</span>
      </Wrapper>
    </Tooltip>
  )
