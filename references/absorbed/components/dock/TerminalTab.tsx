...
export function TerminalTab({
      ...
      .then(({ wsUrl }) => {
        ...
        ws.onmessage = (event) => {
          ...
          if (msg.type === 'output' && msg.data) {
            ...
            setError(msg.data)
            // Support both camelCase (opsia) and snake_case (conduit) error type field
  ...
  return (
    <div className="relative h-full w-full bg-background overflow-hidden">
      {/* Mini toolbar */}
      <div className="h-8 flex items-center gap-2 px-2 bg-card/50 border-b border-border/50">
        <span
        ...
        />
        <span className="text-xs text-muted-foreground/75">{podName}</span>
        ...
        {containers.length > 1 && (
          <div className="relative">
            <select
              ...
              className="appearance-none bg-popover text-xs text-foreground px-2 py-0.5 pr-5 rounded border border-border/60 focus:outline-none focus:ring-1 focus:ring-blue-500"
            >
              {containers.map((c) => (
                <option key={c} value={c}>{c}</option>
              ))}
            </select>
            <ChevronDown className="absolute right-1 top-1/2 -translate-y-1/2 w-3 h-3 text-muted-foreground/75 pointer-events-none" />
          </div>
        )}
        ...
        {!isConnected && !isConnecting && (
          <button
            onClick={connect}
            className="flex items-center gap-1 px-2 py-0.5 text-xs text-muted-foreground/75 hover:text-foreground hover:bg-popover rounded"
          >
      ...
      {error ? (
        <div key="error" className="absolute top-8 left-0 right-0 bottom-0 flex flex-col items-center justify-center p-4 text-center bg-slate-900">
          {errorType === 'shell_not_found' ? (
            <>
              <div className="text-warning mb-2 text-sm">Shell not available</div>
              <div className="text-xs text-muted-foreground/75 mb-4 max-w-md">
                Container <span className="font-mono text-muted-foreground">{selectedContainer}</span> doesn&apos;t
              ...
              <div className="flex gap-2">
                ...
                <button
                  onClick={connect}
                  className="flex items-center gap-2 px-3 py-1.5 bg-popover text-foreground text-xs rounded hover:bg-accent"
                >
                  <RefreshCw className="w-3 h-3" />
                  Retry
                </button>
            ...
            <>
              <div className="text-destructive mb-2 text-sm">Failed to connect</div>
              <div className="text-xs text-muted-foreground/45 mb-3">{error}</div>
              <button
                onClick={connect}
                className="flex items-center gap-2 px-3 py-1.5 btn-brand text-xs rounded"
              >
