...
export function NodeTerminalTab({
    ...
    return (
      <div className="h-full w-full flex flex-col items-center justify-center gap-3 bg-background">
        ...
        <div className="text-sm text-muted-foreground">
          Creating debug pod on <span className="text-foreground font-medium">{nodeName}</span>...
        </div>
  ...
  if (error) {
    return (
      <div className="h-full w-full flex flex-col items-center justify-center gap-3 bg-background p-4">
        <AlertCircle className="w-6 h-6 text-destructive" />
        <div className="text-sm text-destructive">Failed to create debug shell</div>
        <div className="text-xs text-muted-foreground/75 text-center max-w-md break-all">{error}</div>
        <button
          onClick={createPod}
