 ...
 * control, used by both OSS Opsia's header and Opsia Hub's cluster top bar so
...
export function ScopePill({ children, className = '' }: ScopePillProps) {
  return (
    <div
      className={`flex items-stretch shrink-0 rounded-lg border border-border bg-card divide-x divide-border [&>*:first-child>button]:rounded-l-[7px] [&>*:last-child>button]:rounded-r-[7px] ${className}`}
    >
      {children}
    </div>
  )
