...
// onConnect callback. opsia/web injects its global-apiBase hooks; the hub injects
...
export const WORKLOAD_METRIC_CATEGORIES: MetricCategoryDef[] = [
  ...
  { key: 'network_rx', label: 'Net RX', color: 'text-success', chartColor: '#34d399', fillColor: '#34d39922' },
  { key: 'network_tx', label: 'Net TX', color: 'text-orange-400', chartColor: '#fb923c', fillColor: '#fb923c22' },
  { key: 'filesystem', label: 'Disk I/O', color: 'text-warning', chartColor: '#fbbf24', fillColor: '#fbbf2422' },
]

export const NODE_METRIC_CATEGORIES: MetricCategoryDef[] = [
  { key: 'cpu', label: 'CPU', color: 'text-blue-400', chartColor: '#60a5fa', fillColor: '#60a5fa22' },
  { key: 'memory', label: 'Memory', color: 'text-purple-400', chartColor: '#c084fc', fillColor: '#c084fc22' },
  { key: 'filesystem', label: 'Disk', color: 'text-warning', chartColor: '#fbbf24', fillColor: '#fbbf2422' },
]
  ...
  if (!isConnected) {
    return (
      <div className="flex flex-col items-center justify-center gap-4 py-12">
        <WifiOff className="h-10 w-10 text-muted-foreground/75" />
        <div className="text-center">
          <p className="mb-1 text-sm text-muted-foreground">Prometheus not connected</p>
    ...
    <div className="flex h-full flex-col">
      ...
      <div className="flex shrink-0 items-center justify-between border-b border-border bg-card/50 px-4 py-2.5">
        <div className="flex items-center gap-1">
          <BarChart3 className="mr-2 h-4 w-4 text-muted-foreground/75" />
          {categories.map((cat) => (
            <button
              key={cat.key}
              type="button"
              ...
              className={cn(
                ...
                category === cat.key
                  ...
                  : 'text-muted-foreground/75 hover:bg-popover/50 hover:text-muted-foreground',
              )}
            >
              {cat.label}
            </button>
        ...
        <select
          ...
          className="rounded-md border border-border bg-popover px-2 py-1 text-xs text-muted-foreground focus:outline-none focus:ring-1 focus:ring-blue-500/50"
        >
      ...
      <div className="min-h-[280px] flex-1 p-4">
        {metricsLoading ? (
          ...
          <div className="flex h-full flex-col items-center justify-center text-muted-foreground/75">
            ...
            {metrics?.hint && (
              <p className="mt-3 w-full max-w-lg rounded border border-warning/30 bg-warning/10 px-3 py-2 text-xs text-warning dark:text-warning">
                {metrics.hint}
              </p>
            )}
            {metrics?.query && (
              <details className="mt-3 w-full max-w-lg text-left">
                <summary className="cursor-pointer text-xs text-muted-foreground/55 hover:text-muted-foreground/75">
                  Diagnostics: show PromQL query
                </summary>
                <div className="mt-2 break-all rounded border border-border bg-background p-2 font-mono text-xs text-muted-foreground">
                  {metrics.query}
                </div>
              </details>
            )}
