...
export function VersionTooltip({ workloads }: { workloads: Pick<AppWorkload, 'name' | 'version'>[] }) {
  ...
  return (
    <div className="max-w-xs space-y-1">
      ...
      <div className="text-[10px] font-medium uppercase tracking-wide text-muted-foreground/75">Image tag by workload</div>
...
export function CategoryTooltip({
  ...
  return (
    <div className="max-w-xs space-y-1.5">
      ...
      <div className="text-[11px] leading-snug text-muted-foreground">{lead}</div>
      {items.length > 0 && (
        <div className="space-y-1 border-t border-border pt-1.5">
          <div className="text-[10px] font-medium uppercase tracking-wide text-muted-foreground/75">Evidence</div>
            ...
            {items.map((it, i) => {
              ...
              return (
                <li key={i} className="flex items-baseline gap-1.5">
                  ...
                  {where && <span className="text-[10px] text-muted-foreground/75">{where}</span>}
                </li>
              )
            })}
          </ul>
...
export function AppIdentityTooltip({
  ...
  return (
    <div className="max-w-xs space-y-1.5">
      {source && <div className="text-xs font-medium text-foreground">Grouped by {appSourceLabel(source)}.</div>}
      {evidences.map((e, i) => (
      ...
      {fleet && folds && (
        <div className="text-[10px] uppercase tracking-wide text-muted-foreground/75">folds across clusters</div>
      )}
...
export function EnvHint({ unlabeled }: { unlabeled?: boolean }) {
  return (
    <div className="max-w-xs space-y-1">
      {unlabeled ? (
        <div className="text-xs text-foreground">No environment detected for this app.</div>
      ) : (
        <div className="text-xs text-foreground">Detected from labels, GitOps overlay paths, or name patterns (shown <span className="italic">~inferred</span>).</div>
      )}
      <div className="text-[11px] leading-snug text-muted-foreground">
