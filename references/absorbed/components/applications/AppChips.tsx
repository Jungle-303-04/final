  ...
  const tooltip =
    workloadClass === 'mixed' && composition && composition.length > 0 ? (
      <div className="space-y-1">
        ...
        <div className="text-[11px] leading-snug text-muted-foreground">Class filters match any of the contained classes.</div>
      </div>
    ) : (
      meta.tooltip
    )
...
export function VersionInfo({ app, variant }: { app: AppRow; variant: 'cell' | 'fact' }) {
  ...
  const mono = variant === 'fact' ? 'font-mono' : 'font-mono text-xs text-muted-foreground'
  if (app.appVersion) {
    return (
  ...
  if (versions.length === 0) {
    return variant === 'cell' ? <span className="text-muted-foreground/75">—</span> : null
  }
