...
function ApplicationRuntimeCell({ apps, workloadClass, ready, desired }: { apps: AppRow[]; workloadClass: AppWorkloadClass; ready: number; desired: number }) {
  ...
  if (!worst) return <span className="text-xs text-muted-foreground/75">Idle</span>
  ...
  return (
    <Tooltip content={runtimes.map((runtime, index) => `${apps[index]?.name ?? 'app'}: ${runtime.label} — ${runtime.detail}`).join('\n')} delay={150}>
      <span className="inline-flex items-center gap-1.5 text-xs text-muted-foreground"><StatusDot tone={mapHealthToTone(worst.health)} />{label}</span>
    </Tooltip>
  )
}
...
export function ApplicationsView({ entries: allEntries, variant, onSelect, title = 'Applications', description, emptySlot, headerActions, loading }: ApplicationsViewProps) {
  ...
  return (
    <div className="flex h-full w-full min-w-0 flex-1 flex-col overflow-hidden bg-background">
      ...
      <div className="shrink-0 border-b border-border px-4 py-4">
        ...
        />
        {initialLoading ? (
          <div className="mt-3 h-1.5 w-full animate-pulse rounded-full bg-accent" aria-hidden />
        ) : (
          <DistributionBar
            className="mt-3"
      ...
      <div className="flex min-w-0 flex-1 overflow-hidden max-[899px]:flex-col">
        ...
        <aside className="flex w-52 shrink-0 flex-col overflow-hidden border-r border-border bg-card/90 max-[899px]:max-h-72 max-[899px]:w-full max-[899px]:border-b max-[899px]:border-r-0">
          ...
          <div className="flex-1 overflow-y-auto">
            ...
            {systemCount > 0 && (
              <label className="flex cursor-pointer items-center gap-2 border-b border-border px-3 py-2 text-[11px] text-muted-foreground hover:bg-accent">
                <input type="checkbox" checked={showSystem} onChange={(e) => filters.setBoolean('system', e.target.checked)} className="accent-primary" />
                <span>Show system namespaces</span>
                <span className="ml-auto tabular-nums text-muted-foreground/75">{systemCount}</span>
              </label>
            )}
              </>
            )}
                  ...
                  {visibleRows.map((r, idx) => r.kind === 'group' ? (
                    <tr
                      ...
                      className={cn(
                        ...
                        idx === highlightedIndex ? 'selection selection-ring' : 'hover:bg-accent',
                      )}
                    ...
                    >
                      <td className="py-2.5 pl-3 pr-2">
                        <span className="flex items-center gap-2">
                          ...
                          <Tooltip content={applicationRuntimeLabel(r.members.map((member) => member.row), r.workloadClass, r.health)} delay={150}>
                            <span className={cn('h-8 w-1 shrink-0 rounded-full', HEALTH_META[r.health].bar)} />
                          </Tooltip>
                          <ChevronRight className={cn('h-3.5 w-3.5 shrink-0 text-muted-foreground/75 transition-transform', r.expanded && 'rotate-90')} aria-hidden />
                      ...
                      <td className="px-2 py-2.5">
                        <span className="text-xs text-muted-foreground/75">{pluralize(r.members.length, 'instance')}</span>
                      </td>
                      <td className="px-2 py-2.5">
                          ...
                          {r.cells.slice(0, 4).map((c) => (
                            <Tooltip key={c.env} content={`${c.env}${c.version ? ` · ${c.version}` : ''}${c.count > 1 ? ` · ${c.count} instances — expand to choose` : ' — open'}`} delay={150}>
                              <button
                                ...
                                className={`${CHIP} ${CHIP_TONE.neutral} gap-1 hover:bg-accent`}
                              >
                          ...
                          {r.cells.length > 4 && (
                            <Tooltip content={`${r.cells.length - 4} more environments — expand to see all instances`} delay={150}>
                              <button
                                ...
                                className={`${CHIP} ${CHIP_TONE.muted} hover:bg-accent`}
                              >
                                +{r.cells.length - 4}
                              </button>
                            </Tooltip>
                      ...
                      <td className="px-2 py-2.5">
                        <span className="text-xs text-muted-foreground">{Object.entries(r.kinds).map(([k, n]) => pluralize(n, k)).join(' · ')}</span>
                      </td>
                    ...
                    >
                      <td className={cn('py-2.5 pr-2', r.child ? 'pl-10' : 'pl-3')}>
                        <span className="flex items-center gap-2">
                          <Tooltip content={applicationRuntimeLabel([e.row], e.workloadClass, e.health)} delay={150}>
                            <span className={cn('h-8 w-1 shrink-0 rounded-full', HEALTH_META[e.health].bar)} />
                          </Tooltip>
                          <span className="truncate font-medium text-foreground">{e.row.name}</span>
                      ...
                      {e.variant === 'single' ? (
                        <>
                          <td className="px-2 py-2.5">
                            {e.namespace ? (
                              <span className="truncate font-mono text-xs text-muted-foreground">{e.namespace}</span>
                            ) : e.namespaces.length > 1 ? (
                              <Tooltip content={e.namespaces.join(', ')} delay={150}>
                                <span className="text-xs text-muted-foreground">{e.namespaces.length} namespaces</span>
                              </Tooltip>
                            ) : (
                              <span className="text-muted-foreground/75">—</span>
                            )}
                          ...
                          <td className="px-2 py-2.5">
                            {e.env ? (
                              ...
                              <Tooltip content={<EnvHint unlabeled />} delay={300}>
                                <span className="cursor-default text-muted-foreground/75">—</span>
                              </Tooltip>
                            )}
                          </td>
                        </>
                        ...
                        <>
                          <td className="px-2 py-2.5">
                            {e.clusters.length === 1 ? (
                              <span className="truncate text-xs text-muted-foreground">{e.clusters[0].name}</span>
                            ) : e.clusters.length > 1 ? (
                              <Tooltip content={e.clusters.map((c) => c.name).join(', ')} delay={150}>
                                <span className="text-xs text-muted-foreground">{e.clusters.length} clusters</span>
                              </Tooltip>
                            ) : (
                              <span className="text-muted-foreground/75">—</span>
                            )}
                      ...
                      <td className="px-2 py-2.5">
                        {e.variant === 'fleet' ? (
                          e.versionSkew ? (
                            ...
                            <span className="font-mono text-xs text-muted-foreground">{e.versions[0]}</span>
                          ) : e.versions.length > 1 ? (
                      ...
                      <td className="px-2 py-2.5">
                        {Object.keys(e.kinds).length === 0 ? (
                          ...
                          <span className="text-xs text-muted-foreground">{Object.entries(e.kinds).map(([k, n]) => pluralize(n, k)).join(' · ')}</span>
                        )}
                      </td>
                      <td className="pr-2 text-right"><ChevronRight className="inline h-4 w-4 text-muted-foreground/75" /></td>
                    </tr>
                  ))(r.entry))}
                </tbody>
              </table>
