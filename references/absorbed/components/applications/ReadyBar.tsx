...
export function ReadyBar({ ready, desired, width = 'w-12' }: { ready: number; desired: number; width?: string }) {
  if (desired <= 0) {
    return <span className="font-mono text-xs tabular-nums text-muted-foreground/75">—</span>
  }
  ...
  return (
    <span className="inline-flex items-center gap-1.5">
      <span className={`inline-block h-1.5 ${width} rounded-full bg-accent`}>
        <span className={`block h-1.5 rounded-full ${tier.bar}`} style={{ width: `${pct}%` }} />
      </span>
      <span className={`font-mono text-xs tabular-nums ${ok ? 'text-muted-foreground' : tier.text}`}>{ready}/{desired || '—'}</span>
    </span>
  )
}
