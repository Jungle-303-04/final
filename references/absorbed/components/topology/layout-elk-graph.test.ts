...
describe('buildHierarchicalElkGraph — collapse predicate consistency', () => {
  ...
  // (opsia-gateway) with no groupLevels entry. Node placement hid its members
  ...
  it('redirects edges into a late-arriving (no-level) group so no edge dangles', () => {
    const nodes: TopologyNode[] = [
      deployment('app1', 'web'),
      deployment('opsia-gateway', 'opsia-frpc'),
    ]
    const edges: TopologyEdge[] = [
      { id: 'e1', source: 'deployment/app1/web', target: 'deployment/opsia-gateway/opsia-frpc', type: 'routes-to' },
    ]

    // Smart default chipped app1 only; opsia-gateway appeared later → no entry.
    ...
    // late-arriving opsia-gateway defaults to collapsed.
    ...
    expect(endpoints).not.toContain('deployment/opsia-gateway/opsia-frpc')
    expect(endpoints).toContain('group-namespace-opsia-gateway')
  })
