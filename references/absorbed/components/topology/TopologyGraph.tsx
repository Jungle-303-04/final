    ...
    return (
      <div className="absolute inset-0 flex items-center justify-center text-muted-foreground">
        <div className="text-center max-w-md">
          <p className="text-lg text-warning">Layout Error</p>
          <p className="text-sm mt-2">
            Failed to compute topology layout. The graph has {topology.nodes.length} nodes.
          </p>
          <p className="text-xs mt-2 text-muted-foreground/75 font-mono bg-muted p-2 rounded">
            {layoutError}
          </p>
          <button
            onClick={() => {
            ...
            className="mt-4 inline-flex items-center gap-2 px-3 py-1.5 text-sm bg-card hover:bg-popover border border-border rounded-lg transition-colors"
          >
    ...
    <ReactFlowProvider>
      ...
      {namespaceBreadcrumb && (
        <div className="absolute top-3 left-3 z-10 flex items-center gap-1.5">
          ...
          <span className="text-xs font-medium text-muted-foreground bg-card/80 backdrop-blur-sm border border-border/50 rounded-md px-2 py-0.5">
            {namespaceBreadcrumb}
          </span>
        </div>
      )}
      ...
      {topology?.warnings && topology.warnings.length > 0 && (() => {
        ...
        return (
          <div className={`absolute top-2 z-10 ${isViewLimited ? 'left-2 max-w-xl' : 'left-2 right-2'} ${isAllRbac ? 'bg-warning/10 border-warning/20' : 'bg-warning/10 border-warning/30'} border rounded-lg p-2 backdrop-blur-sm`}>
            <div className="flex items-start gap-2">
              ...
              <div className="text-sm">
                ...
                <details className="mt-1">
                  <summary className="text-xs text-warning/80 hover:text-warning">
                    Show details ({topology.warnings.length})
                  </summary>
                  <ul className="mt-1 text-xs text-muted-foreground/75 space-y-0.5">
                    {rbacWarnings.length > 0 && otherWarnings.length > 0 && (
                      <li className="text-warning/60 font-medium mt-1">RBAC restrictions:</li>
                    )}
                    ...
                    {viewWarnings.length > 0 && (rbacWarnings.length > 0 || otherWarnings.length > 0) && (
                      <li className="text-warning/60 font-medium mt-1">View limits:</li>
                    )}
                    ...
                    {otherWarnings.length > 0 && (rbacWarnings.length > 0 || viewWarnings.length > 0) && (
                      <li className="text-warning/60 font-medium mt-1">Other warnings:</li>
                    )}
      ...
      {layoutError && nodes.length > 0 && (
        <div className="absolute top-2 left-2 right-2 z-10 bg-destructive/10 border border-destructive/30 rounded-lg p-2 backdrop-blur-sm">
          <div className="flex items-start gap-2">
            <AlertTriangle className="w-4 h-4 text-destructive shrink-0 mt-0.5" />
            <div className="text-sm">
              <span className="font-medium text-destructive">Layout Error:</span>
              <span className="text-muted-foreground ml-1">
                Failed to update layout. Showing previous view.
              </span>
              <p className="mt-1 text-xs text-muted-foreground/75 font-mono">{layoutError}</p>
            </div>
          </div>
        </div>
      )}
      ...
      >
        ...
        <Panel position="bottom-left" className="flex flex-col items-start gap-2">
          ...
          {!isTrafficView && (
            <>
              {showLegend && (
                <div className="rounded-md border border-border bg-card/95 px-3 py-2 shadow-md backdrop-blur">
                  <div className="mb-1.5 text-[10px] font-semibold uppercase tracking-wide text-muted-foreground/75">Edge colors</div>
                  <div className="flex flex-col gap-1">
                    {EDGE_LEGEND.map((e) => (
                      <div key={e.label} className="flex items-center gap-2 text-[11px] text-muted-foreground">
...
function ExportImageButton({ onExportingChange }: { onExportingChange: (v: boolean) => void }) {
  ...
  return (
    <>
      ...
      {showDialog && (
        <div
          className="absolute bottom-12 left-0 z-50 bg-card border border-border rounded-lg shadow-2xl p-3 w-72"
          onClick={(e) => e.stopPropagation()}
        >
          <div className="text-sm font-medium text-foreground mb-3">Export topology</div>
          <label className="block text-xs text-muted-foreground mb-1">Filename</label>
          <Input
            ref={inputRef}
            value={filename}
            onChange={(e) => setFilename(e.target.value)}
            className="w-full px-2 py-1.5 text-sm bg-background border border-border rounded text-foreground outline-none focus:border-blue-500 mb-3"
          />
          <label className="block text-xs text-muted-foreground mb-1">Capture</label>
          <div className="flex gap-1 mb-3">
            {(['full', 'viewport'] as const).map(mode => (
              <button
                key={mode}
                onClick={() => setCaptureMode(mode)}
                className={`flex-1 px-2 py-1.5 text-xs rounded transition-colors ${captureMode === mode ? 'btn-brand' : 'bg-background text-muted-foreground hover:text-foreground border border-border'}`}
              >
              ...
              <div className="flex gap-1">
                {(['image/webp', 'image/png'] as ImageFormat[]).map(f => (
                  <button
                    key={f}
                    onClick={() => setFormat(f)}
                    className={`flex-1 px-2 py-1.5 text-xs rounded transition-colors ${format === f ? 'btn-brand' : 'bg-background text-muted-foreground hover:text-foreground border border-border'}`}
                  >
                    {FORMAT_LABELS[f]}
                  </button>
                ))}
            ...
            {captureMode === 'viewport' && (
              <div className="flex-1">
                ...
                <select
                  ...
                  className="w-full px-2 py-1.5 text-sm bg-background border border-border rounded text-foreground outline-none focus:border-blue-500"
                >
          ...
          {dims && (
            <div className="text-[10px] text-muted-foreground/75 mb-2">
          ...
          <div className="flex gap-2">
            <button
              ...
              className="flex-1 px-3 py-1.5 text-sm text-muted-foreground hover:text-foreground hover:bg-popover rounded transition-colors"
            >
              Cancel
            </button>
            <button
      ...
      {exporting && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/40">
          <div className="bg-card border border-border rounded-lg px-5 py-4 shadow-2xl">
            <div className="text-sm text-foreground animate-pulse">Exporting topology…</div>
          </div>
        </div>
      )}
    </>
    ...
    <>
      ...
      {onTogglePause && (
        <Tooltip content={paused ? 'Resume live updates' : 'Pause live updates'} delay={TIP} position="right">
          <button
            className={`react-flow__controls-button ${paused ? 'text-warning' : ''}`}
