  ...
  return (
    <>
      ...
      <button
        onClick={handleOpen}
        className={cn(
          'absolute z-10 flex items-center gap-2 px-3 py-2 bg-card/90 backdrop-blur border border-border rounded-lg text-muted-foreground hover:text-foreground hover:border-border/60 transition-colors',
          triggerClassName ?? 'top-4 left-4'
        )}
      >
        <Search className="w-4 h-4" />
        <span className="text-sm">Search</span>
        <kbd className="hidden sm:inline-flex items-center px-1.5 py-0.5 text-xs bg-popover rounded border border-border/60">
          /
        </kbd>
      </button>
          ...
          <div className="relative w-full max-w-lg mx-4 bg-card border border-border rounded-xl shadow-2xl overflow-hidden">
            ...
            <div className="flex items-center gap-3 px-4 py-3 border-b border-border">
              ...
              <Input
                ...
                className="flex-1 bg-transparent text-foreground placeholder-muted-foreground/45 outline-none text-sm"
                autoFocus
              />
              {query && (
                <button
              ...
              <kbd className="px-1.5 py-0.5 text-xs text-muted-foreground/75 bg-popover rounded border border-border/60">
                ESC
              </kbd>
            </div>
            ...
            <div ref={resultsRef} className="max-h-[60vh] overflow-y-auto">
              {query && filteredNodes.length === 0 && (
                <div className="px-4 py-8 text-center text-muted-foreground/75">
                ...
                return (
                  <button
                    ...
                    className={cn(
                      ...
                      isSelected ? 'bg-popover/50' : 'hover:bg-popover/30'
                    )}
                  >
                    <div className={cn(
                      'flex items-center justify-center w-8 h-8 rounded-lg',
                      isSelected ? 'bg-blue-500/20' : 'bg-popover/50'
                    )}>
                      <Icon className={cn('w-4 h-4', getKindColor(node.kind))} />
                    </div>
                    ...
                    <ChevronRight className={cn(
                      ...
                      isSelected ? 'text-muted-foreground opacity-100' : 'opacity-0'
                    )} />
                  </button>
                )
              })}
              ...
              <div className="px-4 py-2 border-t border-border bg-card/50">
                <div className="flex items-center gap-4 text-xs text-muted-foreground/75">
                  <span className="flex items-center gap-1">
                    <kbd className="px-1 py-0.5 bg-popover rounded">↑</kbd>
                    <kbd className="px-1 py-0.5 bg-popover rounded">↓</kbd>
                    <span>Navigate</span>
                  </span>
                  <span className="flex items-center gap-1">
                    <kbd className="px-1.5 py-0.5 bg-popover rounded">Enter</kbd>
            ...
            {!query && (
              <div className="px-4 py-6 text-center text-muted-foreground/75">
                <p className="text-sm mb-3">Search for resources in the topology</p>
                <div className="flex flex-wrap justify-center gap-2 text-xs">
                  <span className="px-2 py-1 bg-popover/50 rounded">nginx</span>
                  <span className="px-2 py-1 bg-popover/50 rounded">service/api</span>
                  <span className="px-2 py-1 bg-popover/50 rounded">production</span>
                </div>
              </div>
            )}
          </div>
