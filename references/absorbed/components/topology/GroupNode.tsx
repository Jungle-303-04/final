...
export const GroupNode = memo(function GroupNode({
      ...
      <>
        ...
        >
          ...
          <div className="flex items-center gap-2 min-w-0">
            ...
            <span className="text-[10px] text-muted-foreground shrink-0 bg-card/50 rounded px-1.5 py-0.5">{nodeCount}</span>
            {statusDotColor && (
          ...
          {kindPills.length > 0 && (
            <div className="flex flex-wrap gap-1 mt-1.5">
              {kindPills.map(([kind, count]) => (
                <div key={kind} className="flex items-center gap-1 bg-card/50 rounded px-1.5 py-0.5">
                  {(() => {
                    const KindIcon = getTopologyIcon(kind)
                    return <KindIcon className="h-2.5 w-2.5 shrink-0 text-muted-foreground/75" aria-hidden />
                  })()}
                  <span className="text-[10px] text-muted-foreground">{pluralize(count, kind)}</span>
                </div>
              ))}
              {kindCounts && Object.keys(kindCounts).length > maxPills && (
                <span className="text-[10px] text-muted-foreground/75 self-center">+{Object.keys(kindCounts).length - maxPills}</span>
              )}
            </div>
          )}
        </div>
        ...
        >
          ...
          >
            <div className="flex items-center gap-2 min-w-0 flex-1">
              ...
              <span className="shrink-0 text-xs text-muted-foreground bg-card/50 rounded px-1.5 py-0.5">
                ...
                >
                  <div className="grid-card-header">
                    {(() => {
                      ...
                      return <KindIcon className="h-3.5 w-3.5 shrink-0 text-muted-foreground/75" aria-hidden />
                    })()}
  ...
  return (
    <>
      {handles}
      <div
        className={`absolute left-0 box-border isolate overflow-hidden group-container-adjusted ${hideHeader ? '' : 'rounded-xl bg-card/40'}`}
        style={{
          width: width || '100%',
          height: height || '100%',
