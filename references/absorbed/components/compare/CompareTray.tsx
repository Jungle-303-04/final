...
export function CompareTray({ kind, picks, onRemove, onCompare, onExit }: CompareTrayProps) {
  ...
  return (
    <div
      ...
      className={cn(
        'shrink-0 border-t border-primary/30 bg-card/95 backdrop-blur',
    ...
    >
      ...
      <div className="flex items-center gap-3 pl-4 pr-20 py-2.5">
        <div className="flex items-center gap-2 shrink-0">
          ...
          <span className="text-[10px] text-muted-foreground/75 font-medium px-1.5 py-0.5 rounded bg-popover">
            {picks.length}/2
          </span>
        </div>
        ...
        <button
          ...
          className={cn(
            ...
            ready
              ? 'btn-brand'
              : 'text-muted-foreground/45 bg-popover border border-border/60 cursor-not-allowed',
          )}
        >
...
function PickSlot({
  ...
  if (!pick) {
    return (
      <div className="group flex items-center gap-2 pl-1.5 pr-2.5 py-1 rounded-lg border border-dashed border-border/60 text-xs font-mono min-w-0 max-w-[22rem] text-muted-foreground/75 italic flex-1">
        <span className={cn('inline-flex items-center justify-center w-4 h-4 rounded text-[10px] font-bold leading-none shrink-0 opacity-50', tones.chipBg)}>
          {side === 'a' ? 'A' : 'B'}
        </span>
  ...
  return (
    <div
      ...
      title={full}
    >
      <span className={cn('inline-flex items-center justify-center w-4 h-4 rounded text-[10px] font-bold leading-none shrink-0', tones.chipBg)}>
        {side === 'a' ? 'A' : 'B'}
      </span>
      ...
      <button
        onClick={onRemove}
        className="shrink-0 p-0.5 rounded text-muted-foreground/75 hover:text-foreground hover:bg-popover/70"
