  ...
  return (
    <DialogPortal open={open} onClose={onClose} className="max-w-xl w-full max-h-[70vh] flex flex-col">
      <div className="flex items-center justify-between px-4 py-3 border-b border-border shrink-0">
        ...
        <button
          onClick={onClose}
          className="p-1 text-muted-foreground hover:text-foreground hover:bg-popover rounded"
          aria-label="Close"
        >
          <X className="w-5 h-5" />
        </button>
      ...
      <div className="px-4 py-3 border-b border-border shrink-0 space-y-2">
        <div className="text-xs text-muted-foreground flex items-center gap-1.5 flex-wrap">
          <span className={cn('inline-flex items-center justify-center w-4 h-4 rounded text-[10px] font-bold leading-none', sourceChipBg)}>
            {sourceLabel}
          </span>
        ...
        <div className="relative">
          <Search className="w-4 h-4 absolute left-3 top-1/2 -translate-y-1/2 text-muted-foreground/75 pointer-events-none" />
          <Input
            autoFocus
            value={query}
            ...
            className="w-full pl-9 pr-3 py-2 text-sm bg-popover border border-border rounded-lg text-foreground placeholder:text-muted-foreground/75 focus:outline-none focus:border-primary"
          />
        </div>
      </div>
      ...
      <div className="flex-1 min-h-0 overflow-y-auto">
        ...
        {!loading && error == null && filtered.length === 0 && (
          <div className="flex flex-col items-center justify-center py-12 text-muted-foreground/75 text-sm gap-1">
            {candidates.length <= 1
                    ...
                    {c.namespace && (
                      <span
                        className={cn(
                          ...
                          sameNs ? 'text-primary' : 'text-muted-foreground/75',
                        )}
