      ...
      <div className="flex flex-col gap-3.5 rounded-2xl border border-border bg-card p-4 shadow-sm">
        <div className="flex flex-wrap items-end justify-between gap-3">
          <div className="flex items-baseline gap-2">
            <span className="text-2xl font-semibold tabular-nums text-foreground">{fleetChecks.length}</span>
            <span className="text-sm text-muted-foreground">
              {fleetChecks.length === 1 ? 'check' : 'checks'}
              {totalFindings > fleetChecks.length && <span className="text-muted-foreground/75"> · {totalFindings} findings</span>}
              {clusterOptions.length > 1 && clusterCount > 0 && (
                <span className="text-muted-foreground/75"> · {clusterCount} {clusterCount === 1 ? 'cluster' : 'clusters'}</span>
              )}
            </span>
          </div>
          <div className="relative">
            ...
            <Input
              ...
              className="w-64 rounded-lg border border-border/60 bg-background py-1.5 pl-9 pr-8 text-sm text-foreground placeholder-muted-foreground/45 focus:outline-none focus:ring-2 focus:ring-[var(--primary)]"
            />
            {search && (
              <button
                type="button"
                ...
                className="absolute right-2 top-1/2 -translate-y-1/2 rounded p-0.5 text-muted-foreground/75 hover:text-foreground"
              >
        ...
        <div className="flex flex-wrap items-center gap-1.5">
          ...
          {clusterFilter.size > 0 && (
            <>
              {!multiCluster && <span className="mx-1.5 h-5 w-px bg-border" />}
              {[...clusterFilter].map((id) => {
                const label = clusterLabelById?.(id) ?? clusterOptions.find((o) => o.id === id)?.label ?? id
                return (
                  <span
                    key={id}
                    className="inline-flex items-center gap-1 rounded-full border border-[var(--primary)]/30 bg-[var(--primary)]/10 py-1 pl-2.5 pr-1 text-xs text-foreground"
                  >
                    ...
                    <button
                      ...
                      className="rounded-full p-0.5 text-muted-foreground/75 transition-colors hover:bg-accent hover:text-foreground"
                    >
                      <X className="h-3 w-3" />
                    </button>
                  </span>
          ...
          <EmptyState
            ...
            action={
              <button
                type="button"
                onClick={clearAll}
                className="badge badge-sm border border-border bg-popover text-foreground transition-colors hover:bg-accent"
              >
                Clear all filters
              </button>
            }
...
export function CheckRemediationBlock({ description, remediation, references, layout = 'columns' }: CheckRemediationBlockProps) {
  ...
  if (layout === 'stack') {
    return (
      <div className="flex flex-col gap-2">
        {description && <p className="text-[13px] leading-relaxed text-muted-foreground">{renderProse(description)}</p>}
        {remediation && (
          <div>
            <div className="text-[11px] uppercase tracking-wider text-muted-foreground/75">How to fix</div>
            <p className="mt-0.5 text-[13px] leading-relaxed text-muted-foreground">{renderProse(remediation)}</p>
          </div>
        )}
  ...
  return (
    <div className="flex flex-col divide-y divide-border/70 [&>*]:py-4 [&>*:first-child]:pt-0 [&>*:last-child]:pb-0">
      {description && (
  ...
  return (
    <Container
      className={[
        ...
        open ? 'border-[var(--border-emphasis)] ring-1 ring-[var(--border-emphasis)] shadow-md' : 'border-border shadow-sm',
        dimmed ? 'opacity-60' : '',
        className ?? '',
      ]
        .filter(Boolean)
    ...
    >
      ...
      <div
        ...
        className={`group flex cursor-pointer items-center gap-3 border-l-[3px] py-3 pl-3 pr-4 transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--primary)]/40 ${open ? SEVERITY_HEADER_BAND_CLASS[sev] : SEVERITY_RAIL_CLASS[sev]}`}
      >
        ...
        {renderActions?.()}
        <ChevronRight className={`h-4 w-4 shrink-0 text-muted-foreground/75 transition-transform duration-200 ${open ? 'rotate-90' : ''}`} />
      </div>
      ...
      <div className="grid transition-[grid-template-rows] duration-200 ease-out" style={{ gridTemplateRows: open ? '1fr' : '0fr' }}>
        <div className="overflow-hidden" inert={!open || undefined}>
          <div className="flex flex-col divide-y divide-border/70 border-t border-border bg-card py-4 pl-6 pr-4 [&>*]:py-4 [&>*:first-child]:pt-0 [&>*:last-child]:pb-0">{children}</div>
        </div>
      </div>
    </Container>
  )
...
export function CheckClusterBreakdownShell<T extends CheckClusterBreakdownGroup>({
  ...
  return (
    <section className="flex flex-col gap-1.5">
      {heading ? <h4 className="text-[11px] font-semibold uppercase tracking-[0.06em] text-muted-foreground/75">{heading}</h4> : null}
      <ul className="flex flex-col gap-1">
        {shown.map((group) => {
          const isOpen = openClusters.has(group.id)
          return (
            <li key={group.id} className="rounded-lg border border-border/70 bg-card">
              <button
                type="button"
              ...
              >
                <ChevronDown className={`h-3.5 w-3.5 shrink-0 text-muted-foreground/75 transition-transform duration-200 ${isOpen ? '' : '-rotate-90'}`} />
                <span className="min-w-0 max-w-[260px] truncate text-sm font-medium text-foreground">{group.label}</span>
                {group.environment && (
                  <span className="shrink-0 rounded bg-popover px-1.5 py-0.5 text-[10px] font-medium uppercase tracking-wide text-muted-foreground ring-1 ring-border">
                    {group.environment}
                  </span>
                )}
                <span className="flex-1" />
                <span className="shrink-0 text-xs tabular-nums text-muted-foreground">{group.count}</span>
              </button>
      ...
      {hiddenClusters > 0 && (
        <button
          ...
          className="mt-0.5 inline-flex w-fit items-center gap-1 rounded px-2 py-1 text-xs font-medium text-[var(--primary)] hover:underline"
        >
...
function ResourceList({
  ...
  return (
    <section className="flex flex-col gap-1.5">
      {label && <h4 className="text-[11px] font-semibold uppercase tracking-wide text-muted-foreground/75">{label}</h4>}
...
function FindingLine({
  ...
  const body = (
    <>
      <span className="shrink-0 font-mono text-[11px] uppercase tracking-wide text-muted-foreground/75">{r.kind}</span>
      <span className={`shrink-0 font-medium ${linkable ? 'text-[var(--primary)]' : 'text-foreground'}`}>
        {r.namespace ? `${r.namespace} / ` : ''}
        {r.name}
      </span>
      {linkable && <ExternalLink className="h-3 w-3 shrink-0 text-muted-foreground/75 opacity-0 transition-opacity group-hover/f:opacity-100" />}
      {showMessage && <span className="ml-1 truncate text-xs text-muted-foreground/75">{finding.message}</span>}
    </>
  )
  // items-baseline so the smaller mono kind label shares a baseline with the
  // larger resource name (their line-heights differ).
  const cls = 'group/f flex w-full items-baseline gap-2 rounded-md px-2 py-1 text-left text-sm transition-colors hover:bg-accent/60'
  return (
    <li>
      {onResourceClick ? (
    ...
    <>
      <button
        ...
        className={[
          ...
          active ? 'border-border bg-popover text-foreground' : 'border-border/60 text-muted-foreground hover:bg-accent/60',
        ].join(' ')}
      >
        Clusters
      ...
      {menu &&
        createPortal(
          <div
            ...
            className="fixed z-[60] max-h-80 w-64 overflow-auto rounded-lg border border-border bg-card py-1 shadow-xl"
          ...
          >
            {active && (
              <button
                type="button"
                onClick={onClear}
                className="flex w-full items-center px-3 py-1.5 text-left text-xs text-muted-foreground/75 hover:bg-accent hover:text-foreground"
              >
                Clear selection
              </button>
            )}
            {options.map((o) => {
              ...
              return (
                ...
                >
                  <span className={`flex h-3.5 w-3.5 shrink-0 items-center justify-center rounded-sm border ${on ? 'border-[var(--primary)] bg-[var(--primary)] text-white' : 'border-border'}`}>
  ...
  return (
    <>
      <button
        ...
        className="shrink-0 rounded p-1 text-muted-foreground/75 opacity-0 transition-opacity hover:bg-accent hover:text-muted-foreground group-hover:opacity-100"
      >
      ...
      {menu &&
        createPortal(
          <div
            role="menu"
            className="fixed z-[60] min-w-48 rounded-lg border border-border bg-card py-1 shadow-xl"
