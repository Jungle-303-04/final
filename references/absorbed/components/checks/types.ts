...
// host-agnostic: Opsia Hub feeds it fleet-resolved data, and OSS Opsia can feed
// a single-cluster ("fleet of one") resolve. Hosts map their wire payloads onto
// these types; the components render against them.
//
// Mirrors Opsia OSS's resource-key convention (opsia/pkg/audit.ResourceKey).
...
/** Raw detector severity Opsia emits. */
export type OpsiaSeverity = 'danger' | 'warning';
...
/** mapOpsiaSeverity maps a raw detector severity to the Checks ladder
 *  (danger→high, warning→medium). critical/low are only reachable via an org
 *  severity override; the detector never emits them. */
export function mapOpsiaSeverity(raw: OpsiaSeverity | string): CheckSeverity {
  switch (raw) {
    case 'danger':
      return 'high';
    case 'warning':
...
/** Built-in (Opsia-detected) finding source. The only V1 source. */
export const SOURCE_OPSIA_BUILTIN = 'opsia_builtin';
...
export interface EffectiveCheckFinding {
  source: 'opsia_builtin';
  resource: CheckResourceRef;
  checkID: string;
  category: string;
  originalSeverity: OpsiaSeverity;
