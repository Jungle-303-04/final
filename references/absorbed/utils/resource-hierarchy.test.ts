...
describe('app membership outranks topology attachment (grouping=app)', () => {
  const crashloop: AppMembership = { appKey: 'opsia-netdiag/Deployment/crashloop', appName: 'crashloop' }

  const exposesTopo = {
    nodes: [
      { id: 'service/opsia-netdiag/crashloop', kind: 'Service', name: 'crashloop', data: { namespace: 'opsia-netdiag' } },
      { id: 'deployment/opsia-netdiag/crashloop', kind: 'Deployment', name: 'crashloop', data: { namespace: 'opsia-netdiag' } },
    ],
    edges: [{ source: 'service/opsia-netdiag/crashloop', target: 'deployment/opsia-netdiag/crashloop', type: 'exposes' }],
  } as unknown as Topology

  it('keeps two same-app members SIBLINGS under the app header despite an exposes edge', () => {
    const events = [
      changeEvent('Service', 'opsia-netdiag', 'crashloop'),
      changeEvent('Deployment', 'opsia-netdiag', 'crashloop'),
    ]
    const appIndex = index({
      'Service/opsia-netdiag/crashloop': crashloop,
      'Deployment/opsia-netdiag/crashloop': crashloop,
    })
  ...
  it('still parents the Deployment under the Service when they belong to DIFFERENT apps', () => {
    const other: AppMembership = { appKey: 'opsia-netdiag/Service/other', appName: 'other' }
    const events = [
      changeEvent('Service', 'opsia-netdiag', 'crashloop'),
      changeEvent('Deployment', 'opsia-netdiag', 'crashloop'),
    ]
    const appIndex = index({
      'Service/opsia-netdiag/crashloop': other,
      'Deployment/opsia-netdiag/crashloop': crashloop,
    })
    const lanes = buildResourceHierarchy({ events, topology: exposesTopo, grouping: 'app', appIndex })
...
describe('structural app members survive the window filter', () => {
  it('keeps a server-declared member visible with no events in the window', () => {
    const member: ResourceLane = {
      id: 'Service/opsia-netdiag/crashloop', kind: 'Service', namespace: 'opsia-netdiag',
  ...
  it('still filters an event-less lane that is NOT a structural member', () => {
    const stray: ResourceLane = {
      id: 'Pod/opsia-netdiag/old-pod', kind: 'Pod', namespace: 'opsia-netdiag',
