...
describe("ProductRouter unified filter cutover", () => {
  it("does not rewrite canonical filters or detail while mounting and resolving the cluster catalog", async () => {
    ...
    const { router } = renderProductRouter(
      `/product/resources${FILTER_SEARCH}#detail`,
      clusterScope,
    );

    expect(currentLocation(router)).toBe(`/product/resources${FILTER_SEARCH}#detail`);
  ...
  it("preserves every filter and drops detail when the sidebar changes surfaces", async () => {
    ...
    expect(currentLocation(router)).toBe(`/product/issues${FILTER_ONLY_SEARCH}`);
  ...
  it("preserves every filter and drops detail when a route shortcut changes surfaces", async () => {
    ...
    await waitFor(() => {
      expect(currentLocation(router)).toBe(`/product${FILTER_ONLY_SEARCH}`);
    });
    expect(router.state.historyAction).toBe("PUSH");
  });
  ...
  it("drops detail when a route shortcut targets the already active surface", async () => {
    ...
    await waitFor(() => {
      expect(currentLocation(router)).toBe(`/product/resources${FILTER_ONLY_SEARCH}`);
    });
    expect(router.state.historyAction).toBe("PUSH");
  });
  ...
  it("replaces an unknown path with the fallback while preserving filters and dropping detail", async () => {
    const { router } = renderProductRouter(
      `/product/not-released${FILTER_SEARCH}#detail`,
      emptyClusterScope,
    );
