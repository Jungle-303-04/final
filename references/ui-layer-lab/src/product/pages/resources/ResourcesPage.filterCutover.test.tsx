...
describe("ResourcesPage unified-filter cutover", () => {
  ...
  it.each([
    ["no Cluster filter", "/product/resources?resources.types=pod"],
    [
      "multiple Cluster filters",
      "/product/resources?clusters=cluster-1,kubernetes-ops&resources.types=pod",
    ],
  ...
  it.each([
    ...
    renderEnglishResources(
      port,
      `/product/resources?clusters=cluster-1&resources.types=pod${suffix}`,
    );
  ...
  it("loads a self-contained detail even when the partial catalog omits its type", async () => {
    ...
    renderEnglishResources(
      port,
      "/product/resources?clusters=cluster-1&resources.types=node" +
  ...
  it("uses resources.types as authority when a legacy path disagrees", async () => {
    ...
    renderEnglishResources(
      port,
      "/product/resources/node?cluster=cluster-1&clusters=cluster-1&resources.types=pod",
    );
...
function canonicalDetailEntry(): string {
  return "/product/resources/pod?cluster=cluster-1&clusters=cluster-1" +
