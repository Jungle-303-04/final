...
- [ ] 새 event payload 또는 변경된 event payload가 `src/packages/contracts/event_bus/bodies/`에 반영됨
- [ ] 새 worker 구독 또는 변경된 worker 구독이 서비스 `app.py`의 `App.on(...)` 흐름을 따름
- [ ] typed handler는 body 객체를 받고, `@app.on_any` projector만 `EventEnvelope`를 받음
- [ ] `ack/nak/DLQ`를 직접 처리하지 않고 runtime에 맡김
- [ ] 새 API field 또는 변경된 API field가 Pydantic schema에 반영됨
- [ ] Worker code가 `App.on(...)`/`yield Body(...)` 패턴을 사용함
