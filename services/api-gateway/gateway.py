...
from packages.contracts.gateway.requests import AgentConnectRequest, AgentEvidenceRequest, CommandRequest, CommandResultRequest, GitHubWebhookRequest, OAuthCallbackRequest
...
class ApiGateway:
    def __init__(self) -> None:
        ...
        self.app = FastAPI(title=Settings.APP_TITLE, version=Settings.APP_VERSION, lifespan=self.lifespan)
    ...
    def configure_routes(self) -> None:
        ...
        async def session(request: Request) -> dict[str, Any]:
            ...
            return {Gateway.AUTHENTICATED: True, Gateway.USER_ID: current.user_id, Gateway.ROLES: current.roles}

        @app.get(gateway_routes.OAUTH_START_PATH)
        async def oauth_start(provider: str, user_id: str = Auth.LOCAL_USER_ID, scopes: str = Settings.DEFAULT_SCOPES) -> dict[str, Any]:
            ...
            await self.events.accept(EventSubject.OAUTH_START_REQUESTED, {Gateway.PROVIDER: provider, Gateway.USER_ID: user_id, Gateway.SCOPES: scope_list, Gateway.STATE: response[Gateway.STATE]})
            return response
        ...
        async def oauth_callback(provider: str, payload: OAuthCallbackRequest) -> dict[str, Any]:
            ...
            return {Gateway.ACCEPTED: True, Gateway.EVENT_ID: accepted.event.event_id, Gateway.TOKEN_REF: account[Gateway.TOKEN_REF], Gateway.SESSION: result[Gateway.SESSION]}

        @app.post(gateway_routes.GITHUB_WEBHOOK_PATH)
        async def github_webhook(payload: GitHubWebhookRequest) -> dict[str, Any]:
            accepted = await self.events.accept(EventSubject.GIT_WEBHOOK_RECEIVED, payload.model_dump())
        ...
        async def agent_evidence(payload: AgentEvidenceRequest) -> dict[str, Any]:
            ...
            accepted = await self.events.accept(EventSubject.CLUSTER_EVIDENCE_RECEIVED, evidence, payload.correlation_id)
            return accepted.response()
        ...
        async def commands(request: Request, payload: CommandRequest) -> dict[str, Any]:
            ...
            accepted = await self.events.accept(EventSubject.COMMAND_REQUESTED, command, actor=Actor(current.user_id, tuple(current.roles)))
            return accepted.response()

        @app.get(gateway_routes.DEAD_LETTERS_PATH)
        async def dead_letters(request: Request, limit: int = Settings.DEFAULT_DEAD_LETTER_LIMIT) -> dict[str, Any]:
        ...
        async def replay_dead_letter(request: Request, dead_letter_id: int) -> dict[str, Any]:
            ...
            if dead_letter is None:
                raise HTTPException(status_code=Settings.COMMAND_NOT_FOUND_STATUS_CODE, detail=Settings.DEAD_LETTER_NOT_FOUND_MESSAGE)
            if dead_letter[Gateway.STATUS] == Gateway.STATUS_REPLAYED:
                raise HTTPException(status_code=Settings.CONFLICT_STATUS_CODE, detail=Settings.DEAD_LETTER_REPLAYED_MESSAGE)

            accepted = await self.events.accept(dead_letter["original_subject"], dead_letter[Gateway.PAYLOAD], dead_letter[Gateway.CORRELATION_ID], dead_letter["original_event_id"])
            self.db.mark_dead_letter_replayed(dead_letter_id, accepted.event.event_id)
            return {Gateway.ACCEPTED: True, Gateway.DEAD_LETTER_ID: dead_letter_id, Gateway.REPLAY_EVENT: accepted.event}

        @app.get(gateway_routes.AGENT_COMMAND_POLL_PATH)
        async def poll_command(cluster_id: str = Target.DEFAULT_CLUSTER_ID, timeout: int = Settings.DEFAULT_AGENT_COMMAND_POLL_SECONDS) -> dict[str, Any]:
            deadline = time.time() + min(timeout, Settings.MAX_COMMAND_POLL_SECONDS)
            while time.time() < deadline:
                row = await self.db.lease_agent_command(cluster_id, Settings.COMMAND_STATUS_QUEUED, Settings.COMMAND_STATUS_LEASED)
                if row:
        ...
        async def command_result(command_id: str, payload: CommandResultRequest) -> dict[str, Any]:
            ...
            if not correlation_id:
                raise HTTPException(status_code=Settings.COMMAND_NOT_FOUND_STATUS_CODE, detail=Settings.COMMAND_NOT_FOUND_MESSAGE)
            accepted = await self.events.accept(EventSubject.COMMAND_COMPLETED, {Gateway.COMMAND_ID: command_id, Gateway.RESULT: result}, correlation_id)
        ...
        async def unhandled(_request: Request, exc: Exception) -> JSONResponse:
            ...
            return JSONResponse(status_code=Settings.GATEWAY_ERROR_STATUS_CODE, content={Gateway.ERROR: str(exc)})
