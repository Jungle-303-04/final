...
class HttpManagementPlaneClient:
    ...
    async def register_agent(self, cluster_id: str, agent_id: str, capabilities: list[str]) -> None:
        await self.client.post(f"{self.base_url}{gateway_routes.AGENT_CONNECT_PATH}", json={Gateway.CLUSTER_ID: cluster_id, Gateway.AGENT_ID: agent_id, Gateway.CAPABILITIES: capabilities})

    async def ship_evidence(self, evidence: JsonObject) -> int:
        response = await self.client.post(f"{self.base_url}{gateway_routes.AGENT_EVIDENCE_PATH}", json=evidence)
        return response.status_code

    async def poll_command(self, cluster_id: str, timeout_seconds: int) -> CommandRecord | None:
        response = await self.client.get(f"{self.base_url}{gateway_routes.AGENT_COMMAND_POLL_PATH}", params={Gateway.CLUSTER_ID: cluster_id, "timeout": timeout_seconds})
        response.raise_for_status()
        return response.json().get(Gateway.COMMAND)

    async def complete_command(self, command_id: str, result: JsonObject) -> None:
        await self.client.post(f"{self.base_url}{gateway_routes.agent_command_result_path(command_id)}", json=result)


class TargetClusterAgent:
    def __init__(self, client: ManagementPlaneClient | None = None) -> None:
        self.base_url = env(Settings.MANAGEMENT_BASE_URL_ENV, Settings.DEFAULT_MANAGEMENT_BASE_URL).rstrip("/")
        self.cluster_id = env(Settings.TARGET_CLUSTER_ID_ENV, Target.DEFAULT_CLUSTER_ID)
        self.interval = int(env(Settings.EVIDENCE_INTERVAL_ENV, Target.DEFAULT_EVIDENCE_INTERVAL_SECONDS))
        self.client = client
    ...
    async def register(self, client: ManagementPlaneClient) -> None:
        while True:
            try:
                await client.register_agent(self.cluster_id, env(Settings.HOSTNAME_ENV, Settings.DEFAULT_AGENT_ID), Settings.AGENT_CAPABILITIES)
                return
    ...
    async def poll_commands(self, client: ManagementPlaneClient) -> None:
        while True:
            try:
                command = await client.poll_command(self.cluster_id, Settings.COMMAND_POLL_TIMEOUT_SECONDS)
                if command:
                    ...
                    await client.complete_command(command_id, {Gateway.STATUS: Settings.COMMAND_COMPLETED_STATUS, Gateway.CLUSTER_ID: self.cluster_id, Gateway.APPLIED: True, Gateway.MESSAGE: Settings.COMMAND_RESULT_MESSAGE})
            except Exception as exc:
    ...
    def fake_evidence(self) -> JsonObject:
        return {Gateway.CLUSTER_ID: self.cluster_id, "kubernetes": {"pods": [{"name": Settings.CRASHING_POD_NAME, "status": Settings.CRASHING_POD_STATUS, "restarts": Settings.CRASHING_POD_RESTARTS}], "events": [Settings.K8S_READINESS_FAILED_EVENT, Settings.K8S_BACKOFF_EVENT]}, "metrics": {"source": Settings.FAKE_PROMETHEUS_SOURCE, "cpu": Settings.FAKE_NODE_CPU, "memory_mb": Settings.FAKE_NODE_MEMORY_MB, "http_5xx_rate": Settings.FAKE_HTTP_5XX_RATE}, "logs": [{"source": Settings.FAKE_LOKI_SOURCE, "line": Settings.LOKI_ERROR_LINE}, {"source": Settings.FAKE_LOKI_SOURCE, "line": Settings.LOKI_WARNING_LINE}], "traces": {"source": Settings.FAKE_OTEL_SOURCE, "slow_span": Settings.OTEL_SLOW_SPAN}}
...
def create_fake_telemetry_app(kind: str) -> FastAPI:
    ...
    async def catch_all(path: str) -> dict[str, Any]:
        if kind == "prometheus":
            return {"status": "success", "data": {"resultType": "vector", "result": [{"metric": {"pod": Settings.CHECKOUT_APP_NAME}, "value": [time.time(), Settings.PROMETHEUS_VECTOR_VALUE]}]}}
        if kind == "loki":
            return {"status": "success", "data": {"result": [{"stream": {"pod": Settings.CHECKOUT_APP_NAME}, "values": [[str(int(time.time() * 1e9)), Settings.K8S_READINESS_FAILED_EVENT]]}]}}
...
async def run_fake_telemetry(kind: str) -> None:
    await Server(Config(create_fake_telemetry_app(kind), host=Settings.SERVICE_HOST, port=int(env(Settings.SERVICE_PORT_ENV, Settings.DEFAULT_SERVICE_PORT)), log_level=Settings.LOG_LEVEL)).serve()
