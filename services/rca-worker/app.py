...
from packages.contracts.event_bus.bodies import ClusterEvidenceReceived, EventBody, Evidence, EvidenceBuiltBody, RcaCompletedBody, SafePrRequestedBody
...
async def on_cluster_evidence(evt: ClusterEvidenceReceived, ctx: EventContext) -> AsyncIterator[EventBody]:
    ...
    evidence = Evidence(cluster_id=evt.cluster_id, kubernetes=evt.kubernetes, metrics=evt.metrics, logs=evt.logs, traces=evt.traces, object_ref=evidence_ref)
    report = RcaCompletedBody(root_cause=ROOT_CAUSE, action=RECOMMENDED_ACTION, evidence_ref=evidence.object_ref)
    ...
    yield report
    yield SafePrRequestedBody(title=PR_TITLE, body=f"RCA: {ROOT_CAUSE}\n\nAction: {RECOMMENDED_ACTION}", provider=GitHub.PROVIDER)
