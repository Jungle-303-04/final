...
| # | 상태 | 코드 강제 | 회귀 테스트 |
|---|---|---|---|
| 1. evidence Kubernetes 권한 read-only | 구현됨 | [`agent-rbac.yaml`](../charts/opsia/templates/agent-rbac.yaml)은 `get/list/watch`만 부여한다. [`manifest-check.sh`](../scripts/manifest-check.sh)는 mutation verb, wildcard, exec/attach/port-forward/proxy RBAC를 거부한다. 현재 `src/services/target/cluster-agent/providers` module은 없다. | `make manifest-check` |
