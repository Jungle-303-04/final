...
현 구현 메모: `manifest-render-worker`는 rendered manifest의 canonical `artifact_digest`를 만들고 artifact store payload와 diff basis에 연결한다. `diff-analyze-worker`는 `desired_manifest`가 있으면 `SafePrFilePatch`를 생성하고, GitHub provider는 repository-relative path 검증 뒤 GitHub Contents API로 manifest patch file을 커밋한다. command-worker는 approval이 필요한 write action에 `approval_ref`/`policy_decision_ref`를 요구하고, cluster-agent도 실행 직전 같은 ref를 검사한다. cluster-agent command result는 `retryable`, `resources`, sanitized `stdout`/`stderr`를 포함한다. GitHub provider는 `TokenVaultPort`를 통해 `GITHUB_TOKEN_REF` 또는 기존 `GITHUB_TOKEN` env secret ref를 해석한다. 아직 남은 P0는 repo checkout/cache, rollback patch 생성, last-approved snapshot, route policy, reviewer checklist, approval ref 만료/권한 검증, feature flag, token scope/rotation 검증, repo allowlist, 외부 SecretVault adapter다.
