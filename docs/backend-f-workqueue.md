...
| BQ-016 | in_progress | OSS 프로파일 ([D-017]) | controller+PostgreSQL+agent 3컴포넌트의 로컬 Helm/Kind 설치는 `7bb74d71f`에서 실증. 공개 OCI `ghcr.io/opsia/charts/opsia`는 403이고, 데모 후반은 여전히 mock PR 생성과 직접 `kubectl set image` 정상화라 완료로 보지 않는다 | 기존 조립 `f0c3b4e42`, 로컬 Helm 기반 `7bb74d71f` | 동일 artifact OCI 공개 설치, NATS/in-process 동일 시나리오 결과, 실제 safe-pr 생성·정상화, lifecycle 통합 증명 후에만 landed 복귀 |
