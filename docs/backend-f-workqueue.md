...
| BQ-016 | in_progress | OSS 프로파일 ([D-017]) | controller+PostgreSQL+agent 로컬 Helm/Kind 설치는 `7bb74d71f`, actual JetStream과 in-process의 clean-run 전송·재전달 결과 동등성은 `82a7f29f2`에서 실증. 공개 OCI는 403이고 데모 후반은 여전히 mock PR와 직접 정상화다 | 기존 조립 `f0c3b4e42`, 로컬 Helm `7bb74d71f`, 버스 실측 `82a7f29f2` | 동일 artifact OCI 공개 설치, 실제 safe-pr 생성·정상화, lifecycle 통합 증명 후에만 landed 복귀. 프로세스 crash 내구성은 두 모드가 동등하지 않음을 명시 |
