...
| P0-3 | in_progress | dev gate → digest build/ECR → migration → 단계적 rollout. 명시 context·immutable digest만 허용하고 DB downgrade 없이 이전 이미지를 복원하는 경계 `3cf0b694a` 착륙 | `AWS_DEV_DEPLOY_ENABLED=1` exact opt-in, P0-1/P0-2 선행. 이전 digest capture·migration Job·단계적 rollout·strict smoke workflow 배선은 잔여 |
