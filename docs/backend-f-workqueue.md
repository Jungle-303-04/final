...
| BQ-014 | requested | Argo observer 어댑터 ([D-017], BQ-013 이후) | Application repo/revision/path·sync/health, Rollouts stable revision 읽기 전용. Argo 리소스 변경 금지. 사후 검증 연계.<br>전면 branch 정리로 lane 회수; 복구 hash `f5461aa80` | (읽기 전용 어댑터; gateway 계약 무접촉) | 재개 시 신규 lane + 최신 canonical 재검증, 쓰기 호출 0 + 전체 그린 |
