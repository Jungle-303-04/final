...
| BQ-014 | in_progress | Argo observer 어댑터 ([D-017], BQ-013 이후) | Application repo/revision/path·sync/health, Rollouts stable revision 읽기 전용. Argo 리소스 변경 금지. 사후 검증 연계.<br>담당: Codex 백엔드 세션<br>브랜치: `codex/f-argocd-observer`<br>최종 복구 hash: `8127cc973` | (읽기 전용 어댑터; gateway 계약 무접촉) | 복구 후 최신 canonical 재검증, 쓰기 호출 0 + 전체 그린 |
