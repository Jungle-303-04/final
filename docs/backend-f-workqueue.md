...
| BQ-004 | in_progress | F2 | correlation 타임라인 조회: C0의 신뢰 `workspace_id` 귀속을 기준으로 audit_log를 correlation_id 시간순 반환 (subject, source, created_at, causation_id, payload 요약)<br>담당: Codex 백엔드 세션<br>브랜치: `codex/f-audit-timeline` | `AUDIT_TIMELINE_PATH` `GET /api/audit/timeline?correlation_id=` → `AuditTimelineResponse` | 커서 페이지네이션 |
