...
| BQ-023 | in_progress | GAP-005 | Issues common 3축 + severity/status/environment, facet payload, stable detail ID, cursor/total/completeness. 기존 GAP-005 canonical 계약을 중복 구현하지 않고 새 번호에 연결한다.<br>담당: Codex 백엔드 세션<br>브랜치: `codex/frontend-contract-issues` | Issues 필터 |
