...
| BQ-007 | done-pending-merge | F5 | RolloutDiagnosed(next_action≠observe) → 직전 정상 이미지 patch 생성 → SafePrRequested 발행 배선. **`RECOVERY_ENABLE_AUTO_REVERT_PR` flag(기본 false) 필수**<br>담당: Codex 백엔드 세션<br>브랜치: `codex/f-auto-revert-pr`<br>rebase HEAD: `87eb0eb88` | (신규 route 없음, 이벤트 배선) | flag off에서 무발화 테스트 / on에서 sandbox E2E |
...
| BQ-009 | done-pending-merge | 권위 patch 엔진 ([D-010] 1항, [D-012] read port 승인, [D-019] 배선 예외 승인) | `GitOpsAuthorityReadPort`를 dispatcher에 주입하고 patch 시점에 workflow/diff/active binding/repository/provenance를 재검증. exact base 원문의 scalar span만 바꾸는 `GitOpsScalarPatch` 6종과 exact inverse rollback, 권위 부재/불일치/미지원 fail-closed 구현.<br>브랜치: `codex/f-auto-revert-pr`<br>코드: `a5f7c9a3d`, scorer: `73f4b8b51` | (gateway 계약 변경 없음; read-only port는 계약 lock 비대상) | scorer 6/6 PASS, 전체 `1820 passed, 3 skipped`; H3 origin 착륙 전이라 DoD 완료 아님 |
| BQ-010 | done-pending-merge | 카탈로그 정비 ([D-010] 2항) | `oom_memory`/`replica_scale`/`image_rollback`/`image_tag_fix`/`probe_fix`/`selector_fix` 선언 파라미터화. `gitops_recovery_review`만 문서 action으로 격리하고 실제 patch보다 score 하향.<br>코드: `15d28020b` | (gateway 계약 변경 없음) | 카탈로그 계약 + 전체 `1820 passed, 3 skipped`; H3 origin 착륙 전이라 DoD 완료 아님 |
...
| BQ-014 | in_progress | Argo observer 어댑터 ([D-017], BQ-013 이후) | Application repo/revision/path·sync/health, Rollouts stable revision 읽기 전용. Argo 리소스 변경 금지. 사후 검증 연계<br>담당: Codex 백엔드 세션<br>브랜치: `codex/f-argocd-observer` | (읽기 전용 어댑터; gateway 계약 무접촉) | 읽기 전용 증명(쓰기 호출 0) + 전체 그린 |
