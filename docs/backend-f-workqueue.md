...
| BQ-015 | in_progress | `.remediation.yaml` 소스 계약 ([D-017], P 이후 같은 lane) | 저장소 소유자 선언 필드만 patch(helm-values/kustomize/raw/replica/probe). 추측 금지, 미선언 = unsupported<br>담당: Codex 백엔드 세션<br>브랜치: `codex/remediation-source-contract` | (신규 계약 파일 파서) | 선언 외 필드 수정 불가 테스트 + 전체 그린 |
