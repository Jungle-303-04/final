...
| GAP-010 | in_progress | Resources 단일 cluster 그래프 | Resources 필터와 같은 권한·snapshot revision에서 stable node·검증된 relation edge·drill-down identity를 additive route로 제공.<br>담당: Codex 백엔드 세션<br>lane: `codex/resource-graph-contract` | gateway 계약 lock 보유. single-cluster만 허용, raw payload·cross-cluster edge 금지, relation 근거와 completeness 노출 | 기존 Resources list 무변경, relation 추측 0건, budget 초과 partial, Bruno·전체 게이트·실 PostgreSQL projection 검증 |
