...
| BQ-024 | in_progress | GAP-006 | Applications/GitOps/Checks provider-neutral canonical list DTO + common/surface axes + cursor/facet/completeness.<br>현재 안전 착륙 단위: Applications strict list/facet. GitOps/Checks는 route 미노출로 다음 사이클 처리.<br>담당: Codex 백엔드 세션<br>브랜치: `codex/surface-contracts-apps` | 세 화면 필터 — Applications부터 개별 착륙, legacy `/applications` 유지 |
