...
| BQ-019 | in_progress | VP-003 이벤트 여정 계약 | `AuditTimelineItem`에 자기 `event_id`와 서버 권위의 subject→journey stage 분류를 additive로 노출한다.<br>담당: Codex 백엔드 세션<br>브랜치: `codex/audit-journey-contract` | 기존 `AUDIT_TIMELINE_PATH` 응답 확장(additive) | 부모 `causation_id`와 자기 `event_id` 구분, 전체 subject 분류 완전성, 기존 소비자 회귀, Bruno 응답 계약 |
