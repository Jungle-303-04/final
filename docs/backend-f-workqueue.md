...
| BQ-004 | done-pending-merge | F2 | correlation 타임라인 조회: C0의 신뢰 `workspace_id` 귀속을 기준으로 audit_log를 correlation_id 시간순 반환 (subject, source, created_at, causation_id, payload 요약)<br>담당: Codex 백엔드 세션<br>브랜치: `codex/f-audit-timeline`<br>코드: `d6ee319e5` | `AUDIT_TIMELINE_PATH` `GET /api/audit/timeline?correlation_id=` → `AuditTimelineResponse` | 커서 페이지네이션 |
| BQ-005 | in_progress | F4 | 변경↔장애 상관 projection: gitops 배포 이벤트를 워크로드 키 `(workspace, cluster, ns, kind, name)`로 인덱싱하는 projection 워커(신규 구독자) + 인시던트의 워크로드 키로 직전 N개 변경 조회 route<br>담당: Codex 백엔드 세션<br>브랜치: `codex/f-audit-timeline` | `RCA_RECENT_CHANGES_PATH` `GET /api/rca/incidents/{incident_id}/recent-changes` → `RecentChangeListResponse` | 기존 워커 수정 0건 (신규 구독자만) |
...
| BQ-009 | requested | 권위 patch 엔진 ([D-010] 1항, [D-012] read port 승인, [D-019] 배선 예외 승인) | `contracts` read-only GitOps 권위 컨텍스트 port를 dispatcher에 주입하고 `source_patch.py`의 exact base SHA·SCM provenance·원문 byte 보존을 재사용. patch 생성 시점에 승인 snapshot·binding·repository를 조회해 `safe_pr_patches()`의 markdown fallback을 action_type별 실제 patch 생성기(`oom_memory`, `image_rollback`/`image_tag_fix`, `replica_scale`, `probe_fix`, `selector_fix`)로 교체. 확보 불가 시 `unsupported`, rollback patch 동반.<br>허용 범위: `src/packages/contracts/` 신규 read port + `src/services/ai/dispatch-worker/app.py` authority 주입 최소 배선 + `src/services/ai/agent/recovery/**` ([D-019])<br>담당: Codex 백엔드 세션<br>브랜치: `codex/f-auto-revert-pr` | (gateway 계약 변경 없음; read-only port는 계약 lock 비대상) | 권위 부재 `unsupported`, 6종 patch+rollback, 주입 배선·전체 게이트 검증 |
...
| BQ-012 | requested | **AI fallback 근거 검증 구멍 수정 ([D-016] — P급 우선순위)** | LLM 후보가 자기 작성 expected_evidence + source 존재만으로 rca.completed 진입 가능한 경로 차단(`ai_fallback.py:119`). 내용 기반 signal predicate 최소 1개 검증 전 완료 진입 금지. 구현은 [D-016] 2항의 (a)/(b)/(c) 중 택1·조합, 설계 근거 night-log 기록. frozen 예외: `src/services/ai/agent/pipeline/ai_fallback.py`·관련 평가 경로 한정 | (gateway 계약 변경 없음) | 조작 시나리오 테스트(가짜 후보가 completed 도달 불가 증명) + `test_ai_fallback_worker.py:159` 계약 강화 갱신 + 전체 그린 |
| BQ-013 | requested | single-writer invariant ([D-017]) | `reconciler_mode=builtin`(기본)/`argocd`(observer). argocd 모드에서 자체 apply 경로 차단 + 무발화 테스트 | (설정+가드, additive) | argocd 모드 apply 0건 증명 + 전체 그린 |
...
| BQ-016 | requested | OSS 프로파일 ([D-017]) | PR-only 기본(agent read-only, direct command off), controller+PG+agent 축소 설치, `make demo` 원커맨드(Kind→bad rollout→PR→검증). F0 잔여 증명(단일 composition root 기동, 모드 동등성 비교) 포함 | (프로파일+조립) | make demo 원커맨드 성공 + 모드 동등성 테스트 |
