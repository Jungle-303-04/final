...
|---|---|---|---|---|
| BQ-022 | in_progress | GAP-010 | Resources filter와 동일 scope/revision의 single-cluster graph snapshot, 노드·검증된 edge, partial/restricted/completeness, drill-down identity. 기존 GAP-010 canonical 계약을 중복 구현하지 않고 새 번호에 연결한다.<br>담당: Codex 백엔드 세션<br>브랜치: `codex/frontend-contract-graph` | 표/그래프 모드 |
