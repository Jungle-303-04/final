...
| BQ-005 | done-pending-merge | F4 | 변경↔장애 상관 projection: 성공 배포만 워크로드 키 `(workspace, cluster, ns, kind, name)`로 인덱싱하는 신규 projection 워커 + 인시던트 event-time 이전 최근 변경 조회 route<br>담당: Codex 백엔드 세션<br>브랜치: `codex/f-audit-timeline`<br>코드: `744c12f34`, 회귀 정합: `24998dfaf` | `RCA_RECENT_CHANGES_PATH` `GET /api/rca/incidents/{incident_id}/recent-changes` → `RecentChangeListResponse` | 기존 워커 수정 0건 (신규 구독자만) |
