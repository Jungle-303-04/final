...
- release dispatch guard snapshot: live 실행 시 readiness impact, warning action, diagnostics/alert gate 통과 근거를 run step details와 audit event details에 기록
...
- operator handoff: 선택한 release run의 headline, severity, 다음 조치, mode/health/attention/rollback/verification 체크를 한 번에 표시해 교대/온콜 인계를 빠르게 수행
