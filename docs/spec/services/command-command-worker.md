...
1. **정적 정책** `evaluate_command_policy`:
   ...
   - 액션 카탈로그(`src/domains/command/builtin_actions.py`: `rollout_restart`, `apply_manifest`, `k8s.apps.v1.deployments.scale` — 전부 `allowed_namespaces=("sandbox",)`; `requires_approval` 은 `rollout_restart`=False(비파괴 자동 실행 허용), `apply_manifest`/`deployment scale`=True): `spec.allows_namespace(namespace)` 실패 → "namespace not allowed for this command action"; `requires_approval` 이고 `approval_exempt_for_environment` 아님인데 `approval_ref` 없음 → "write command requires approval_ref"; `policy_decision_ref` 없음 → "write command requires policy_decision_ref".
