...
snapshot 필드: `workload{kind,namespace,name}`, `deployment_labels`, `pod_template_labels`, `managed_fields_managers`, `containers[{name,image,readiness_probe,liveness_probe,startup_probe}]`, `replicaset_revisions[{name,revision}]`. probe 요약은 `path`, `port`, `timeout_seconds`, `period_seconds`, `failure_threshold` 중 존재하는 값만 남긴다. collect fallback 또는 k8s API 미구성 시 `{"change_context": {"current_workload_snapshots": []}}`를 반환한다.
