...
| 순서 | 이벤트 | 라우팅 키 | 조건 |
|---|---|---|---|
| 1 | `IncidentDetectedBody` | `incident.detected` | 항상. `detected = IncidentDetector.has_signal(evidence)` |
| 2a | `EvidenceBundleBuiltBody` | `evidence.bundle.built` | `detected=True` 이고 evidence/incident 존재 |
| 2b | `RcaActionRequiredBody` | `rca.action_required` | 컨텍스트 누락(reason=`"incident context is missing"`) |

body 스키마는 [rca 도메인](../domains/rca.md) 참조. `detected=True` 인
`IncidentDetectedBody` 에는 `severity`(kubernetes.severity, 기본 `"medium"`),
`affected`(단일 리소스 요약 리스트), `evidence`, `incident`(=`IncidentRecord`,
`incident_id=ctx.correlation_id`) 가 실린다. `detected=False` 정상 샘플은
`severity=None`, `affected=[]`, `incident=None` 이다.

## 동작 (Behavior)

1. `bodies = pipeline.build_bodies(evt.evidence, ctx.correlation_id)`
   - `IncidentDetector.detect_body` → 장애 판정·분류(`detected=True`일 때 `IncidentRecord` 생성, correlation_id = incident_id).
...
2. `yield bodies.detected_body` → `detected=True` 일 때만 `yield bodies.next_body`.

## 불변식·오류 (Invariants & Errors)

- `incident.detected` 는 항상 발행된다(미탐지여도 사실 기록).
- 미탐지는 `incident.detected(detected=false)`만 남기고 downstream RCA 이벤트를 내지 않는다.
- 컨텍스트 누락은 예외가 아니라 `rca.action_required` 로 수렴 —
