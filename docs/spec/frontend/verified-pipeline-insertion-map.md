...
| VP-005 | 승격 게이트 표시 — release flow에서 네 가지 판정 조건과 현재 승격 가능 여부 표시 | BQ-006 | workflow run의 optional `promotion_gate: PromotionGateResponse \| null` · 앵커 `8cd0b18e96f1266873d1632486472d0d22c18477` (`origin/dev` ancestor exit 0) | 직결 | Applications release flow + `UI-020 StatusMark`. `eligible`은 승격 실행 완료가 아니라 현재 조건 판정이다 | `APIQ-032 listApplicationRuns` in_progress |
