...
| VP-004 | 인시던트 상세 "최근 변경" 섹션 — 성공 배포의 절대 시각·workload·image·commit·workflow·허용 PR 링크 표시 | BQ-005 완료 | `RCA_RECENT_CHANGES_PATH` = `GET /api/rca/incidents/{incident_id}/recent-changes?limit=` · `RecentChangeListResponse` · 앵커 `81969f23e46cb40743af08ffbc1affe556bd5c5e` (`origin/dev` ancestor exit 0, progress·route·Bruno 실물 확인) | 직결 | `RCA-002 IncidentWorkspace`에 독립 섹션 추가. 서버가 incident event-time 이전 성공 배포만 반환하므로 클라이언트 상관 필터·재정렬 금지. 변경 없으면 섹션 미렌더(빈 카드 금지). 응답에 incident 발생 시각·구조화 PR 번호가 없으므로 "N분 전"·"PR #x"를 추측하지 않는다 | `API 완성: getIncidentRecentChanges (4f602cc86660a7f8a12583cffc44a53e220d9dbf)` · UI GREEN 대기 |
