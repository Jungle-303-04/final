...
| VP-004 | 인시던트 상세 "최근 변경" 섹션 — "이 장애 N분 전 PR #x로 image y 배포됨" | BQ-005 완료 | `RCA_RECENT_CHANGES_PATH` = `GET /api/rca/incidents/{incident_id}/recent-changes?limit=` · `RecentChangeListResponse` · 앵커 `81969f23e46cb40743af08ffbc1affe556bd5c5e` (`origin/dev` ancestor exit 0, progress·route·Bruno 실물 확인) | 직결 | `RCA-002 IncidentWorkspace`에 독립 섹션 추가. 서버가 incident event-time 이전 성공 배포만 반환하므로 클라이언트 상관 필터·재정렬 금지. 변경 없으면 섹션 미렌더(빈 카드 금지) | `APIQ-031 getIncidentRecentChanges` in_progress |
