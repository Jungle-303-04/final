...
| VP-007 | 클러스터-최상위 IA — 전역 Cluster selector가 URL 단일 권위로 전 화면 스코프 결정 + 연결된 Cluster 목록(이름·environment·connection) | provider-free slice 완료 / ProviderIcon만 BQ-017 | 기존 `CLUSTERS_PATH` 착륙 계약 소비 완료 + BQ-017 optional `provider` 필드(착륙 대기) | 목록·selector: 어댑터 완료 / ProviderIcon: backend 선행 | 셸 단일 `ClusterScopeProvider` + `UI-004 ScopePicker`; 페이지별 목록 요청·selector 제거. BQ-017 전에는 일반 Server 아이콘만 사용하며 provider·health를 추측하지 않는다 | provider-free selector 완료: `2c4487d7b`; ProviderIcon 미요청 |
