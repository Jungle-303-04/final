...
| `REF-API-029` | `GET /applications` | `APPLICATIONS_PATH` `GET /api/applications` → `ApplicationProductListResponse` | `직결` | strict provider-neutral card. VP-010 권한 필터, health/deployment/drift/resource count/open incident completeness를 서버가 계산 | `listApplications` BQ-039 |
...
| `REF-API-031` | `GET /changes` | `CHANGES_PATH` → `ChangeTimelineResponse` | `직결` | BQ-057 strict contract. immutable inventory version·RCA incident projection·workflow run의 allowlist evidence를 시간축으로 병합하고 observation 부재를 gap으로 명시. `[from,to)`·stable `(occurredMs,kind,id)` 정렬·24h/1,000 event 상한·초과 fail-closed | `list_changes` S11 착륙 |
...
| `REF-API-118` | `GET /api-resources`; `GET /resource-counts`; `GET /resources/{kind}` | `APPLICATIONS_PATH` → `ApplicationProductListResponse`; `APPLICATION_DEPLOYMENTS_PATH` → `ApplicationDeploymentHistoryResponse`; `APPLICATION_DRIFT_PATH` → `ApplicationDriftResponse` | `어댑터` | canonical application card·배포 이력·semantic drift를 GitOps 목록의 application 요약으로만 사용. Kubernetes discovery 자체는 대체하지 않음 | `listApplications`, `listApplicationDeployments`, `getApplicationDrift` BQ-039~042 |
