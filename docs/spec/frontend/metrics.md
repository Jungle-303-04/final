...
| 방향 | 대상 | 스펙 링크 | 용도 |
|---|---|---|---|
| import | `@/shared/lib/api`(`get`, `post`, `del`), `@/shared/lib/live`(`liveStore`), `@/shared/lib/types`(`MetricQueryPreset`, `MetricWidget`), `@/shared/lib/ui-store`, `@/shared/ui`, `@/shared/ui/charts`(`TimeSeriesChart`, `Series`), `@/shared/motion` | [shared](shared.md) | 실시간·차트·저장형 query/widget |
