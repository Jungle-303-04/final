---
title: VP-019 — 기준 원본 전면 이식 + 우리 기획 덧입히기 (정본)
status: spec-approved
date: 2026-07-14
owner: 우녕 (확정) / 조율 세션 (실측·기록)
reference: references/provenance/source.json `10461f40bcfaf6dd578b24262c8f8fb84ae20766` (Apache-2.0) — `references/upstream/`
...
> "기존 기획을 엎으라는 게 아니야. 기존 기획에서 기준 원본에서 가져올 수 있는 모든 항목과 내용을
...
| 층 | 정본 | 무엇을 결정하나 |
|---|---|---|
| **기능·구조·라벨** | **기준 원본** (`references/upstream/`) | 어떤 화면·컴포넌트·컬럼·kind 렌더러·감사 체크·라벨 텍스트가 있는가. **가져올 수 있는 건 전부 가져온다.** |
| **디자인** | **shadcn/ui** (`ui.shadcn.com`) | 색·간격·타이포·radius·primitive·문법(`cn`/`cva`/`data-slot`). **기준 원본의 시각 언어는 버린다.** |
...
**금지 (모순의 근원):**
- 기준 원본의 **IA(정보구조)를 그대로 베끼는 것** — 우리는 필터=줌이고 기준 원본는 메뉴 나열이다
- 기준 원본의 **색·간격·클래스를 그대로 쓰는 것** — 디자인은 shadcn이다
- 기준 원본에 있다고 **우리 기획에 없는 화면을 만드는 것** — Cost, Local Terminal 등
- 기준 원본에 없다고 **우리 기획의 화면을 빼는 것** — AI 채팅, 3-way 경고, 고스트 파드
...
### 1.1 기준 원본 메뉴 (`web/src/components/nav`, 실측)
...
### 1.2 기준 원본 화면별 자산 (`web/src/components/`, 파일 수)
...
### 1.3 기준 원본 재사용 UI 라이브러리 (`packages/k8s-ui/src/`, 492파일)
...
| 기준 원본 화면 | 판정 | 우리 것 |
|---|---|---|
| **Home** (13) | **가져옴 + 확장** | 위젯 조합 대시보드 (VP-011). 기준 원본 Home 카드를 **위젯으로** 편입 |
...
| **GitOps** (6 + k8s-ui 23) | **가져옴 + 2탭** | `[변경]` 우리 것(PR·승인·**3-way 경고**) + `[동기화 상태]` 기준 원본 것 |
...
| **— (기준 원본에 없음)** | **우리만** | **AI 채팅** · **3-way 경고** · **고스트 파드 진행** · **클러스터 연결 위자드** · **필터=줌** · **SLG 모션** |
...
기준 원본의 `theme-*` 는 **19개짜리 얇은 별칭 레이어**다 (`theme/tailwind-theme.css` 실측).
```css
...
`shared/ui/reference/reference-theme.css` — **이 파일 하나가 전부다:**
...
2. **브랜드 색** — `원본-제공자-*` · `--color-reference-accent` · `--color-brand-*` → `var(--primary)`
...
| 위치 | 문법 |
|---|---|
| **`shared/ui/reference/**`** (이식본) | 기준 원본 구조·`theme-*` 별칭 허용. **팔레트 색·`clsx`·브랜드 색은 금지** |
| **그 외 전부** | **shadcn 문법 필수** (VP-018 §10.1-b): `cn()` · `cva` · `data-slot` · 시맨틱 토큰 |

**경계가 폴더 하나로 명확하다.** 회귀 가드: `theme-*` 가 `shared/ui/reference/` **밖**에 나오면 FAIL.
...
## 5. 라벨·용어 — 기준 원본 것을 정본으로 쓴다

**새로 지어내지 않는다.** kind 표시명·컬럼명·감사 체크명·상태 문구는 **기준 원본 것을 그대로 번역**한다.
...
- `Node` → **서버** (그래프에서만. 표의 컬럼명은 기준 원본 것)
...
| **기준 원본의 왼쪽 필터 사이드바** | **안 쓴다.** 우리는 1층 통합 태그 검색. **컴포넌트는 가져오되 IA는 우리 것** |
...
| **기준 원본 `MetricsChart`/`charts/`** | 가져오되 **shadcn `ChartContainer` 를 통과**시킨다. recharts 직접 사용 금지 |
| **기준 원본의 `theme-*`** | `shared/ui/reference/` **안에서만** 허용. 밖에 나오면 FAIL |

---

## 7. 실시간 — **지금 아예 안 된다.** 기준 원본 방식을 그대로 가져온다 ★
...
### 7.1 기준 원본의 실시간 (`web/src/hooks/useEventSource.ts` 실측)
...
| 제외 사유 | 해당 항목 |
|---|---|
| **① 중복** — 우리 기획에 이미 있다 | 기준 원본 `NamespaceSwitcher` → 우리 1층 필터 칩 / 기준 원본 왼쪽 필터 사이드바 → 우리 1층 태그 검색 |
...
### 8.1 기준 원본 기능 전수 체크리스트 (세션이 이 표를 채운다)

`docs/spec/frontend/reference-feature-inventory.md`에 **기준 원본의 모든
화면·컴포넌트·단축키·액션**을 행으로 넣는다.
각 행의 상태는 **`이식됨` / `흡수됨(→어디)` / `중복(→우리 것)` / `제외(사유)` / `미착수`** 중 하나다.

**`미착수`가 하나라도 남아 있으면 이식은 끝난 게 아니다.**

포함해야 할 것 (빠뜨리기 쉬운 것들):
- **키보드 단축키 전부** (`useKeyboardShortcuts`) — 기준 원본의 단축키 표를 그대로
...
기준 원본 = **Apache-2.0**. 우리 저장소는 **public**이다.
1. `references/upstream/` 고정 커밋 스냅샷 — **492파일 완결**
2. 루트 `NOTICE` — 원저작자(원본 제공자) · Apache-2.0 · 출처 URL · 수정 사실
