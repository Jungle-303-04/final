...
async def unsent_events(self, limit: int, source: str) -> list[EventEnvelope]
    # lease claim: sent_at IS NULL AND source=:source AND (lease_id IS NULL OR leased_until < now())
...
인덱스: `Index("ix_outbox_claim", "source", "sent_at", "leased_until", "id")` (ORM 정의 + 호환 DDL 동일).
