...
| `ClusterUnregisterResponse` | `cluster_id: str`, `status: "uninstalling"\|"cleanup_required"\|"disconnected"\|"purged"`, `stage: "agent_cleanup_queued"\|"manual_cleanup_required"\|"registration_revoked"\|"purged"`, `command_id/command_status_path/uninstall_command/failure_reason: str \| None = None`, `cleanup_verified: bool = False`, `resources/residual_resources: list[str] = []` |
...
| `ApplicationResponse` | POST application/connect 호환 응답 `application: JsonMap`; 제품 GET에는 사용하지 않음 |
| `ApplicationProductCard` / `ApplicationProductListResponse` | strict application card: identity/name/environments/lifecycle/repository refs + `ApplicationHealthSummary` + nullable current deployment/drift/open incidents + completeness가 붙은 resource kind counts / `applications: list[ApplicationProductCard]` |
| `ApplicationProductDetail` / `ApplicationProductDetailResponse` | card + completeness가 붙은 endpoint allowlist + 최근 incident/activity 각 최대 3개 / `application: ApplicationProductDetail` |
| `ApplicationDeploymentHistoryItem` / `ApplicationDeploymentHistoryResponse` | strict workflow deployment projection: id/environment/cluster/git SHA/version/time/actor/closed status/GitOps change ID / `deployments: list[...]` |
| `ApplicationDriftDifference` / `ApplicationDriftResponse` | semantic drift `status: in_sync\|drifted\|unknown`, summary, scalar-only differences, observed_at. 민감·복합 값은 `null` + `value_redacted=true` |
| `DeploymentBindingResponse` / `DeploymentBindingListResponse` | POST binding 호환 응답 `deployment: JsonMap` / 기존 내부 목록 `deployments: list[JsonMap]`; 제품 GET `/deployments`에는 사용하지 않음 |
