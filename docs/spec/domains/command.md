...
| 메서드 | 시그니처 | 쿼리 의미 |
|---|---|---|
| `queue_agent_command` | `(self, correlation_id: str, plan: JsonObject, status: str) -> None` | `INSERT INTO agent_commands ... ON CONFLICT (command_id) DO NOTHING` (멱등). `payload=plan` 전문 저장, lease 필드 NULL, `result={}` |
...
| `debug_query_plan` | `def debug_query_plan(payload: AgentDebugQueryRequest, *, workspace_id, requested_by, correlation_id) -> JsonObject` — 아래 동작 참조 | `src/domains/command/router.py :: debug_query_plan` |
| `lease_next_command` | `async def lease_next_command(db, cluster_id, workspace_id, agent_id, timeout) -> JsonObject \| None` — `min(timeout, MAX_POLL_SECONDS)` 데드라인까지 `db.lease_agent_command(...)` 반복, 빈손이면 `asyncio.sleep(POLL_SLEEP_SECONDS)` | `src/domains/command/router.py :: lease_next_command` |
