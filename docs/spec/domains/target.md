...
| `DELETE /clusters/{cluster_id}` | `src/domains/target/router.py :: unregister_cluster` | path: `cluster_id`, query: `purge: bool = false` | 204 / 400 / 403 / 404 | `require_admin_session`; 기본은 soft-delete. `purge=true`는 `APP_ENV=test`이면서 registration `environment=test`인 target role fixture만 물리 삭제 |
