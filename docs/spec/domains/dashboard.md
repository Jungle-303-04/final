...
| `POST /metrics/validate` (`gateway_routes.METRICS_VALIDATE_PATH`) | `src/domains/dashboard/router.py :: queue_metrics_validation` | `AgentDebugQueryRequest`(`cluster_id`, nested strict `PrometheusQueryDefinition`) | `AgentDebugQueryResponse` | `require_session` + cluster `Permission.EVIDENCE_READ`; destination override를 거부하고 `telemetry.query.run` 명령을 capability 보유 cluster-agent에 큐잉하고 OperationEvent 발행 |
