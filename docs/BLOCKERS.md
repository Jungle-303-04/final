...
2026-07-19 · G4 D-4 · 라이브 홈 카드 수치가 `—`이고 W2~W8 보드가 부재 · 홈 구현 ed2de67b1과 증거 push d48f3652a에서 플릿 요약·카드·W2~W8·실데이터 연결을 코드 해소, 최종 후보 배포와 동일 SHA demo-freeze-v3 라이브 대조 전 · 코드 해소/라이브 미검증
...
2026-07-19 · G4/Home data · 라이브 `/api/activity/overview`는 epoch-ms `from=2147483648`부터 HTTP 500, 직전 값은 200 · `src/domains/activity/repository.py`의 bucket 산식에서 raw Python 정수가 SQLAlchemy `Integer`로 추론돼 PostgreSQL int4 범위를 넘는 원인 확정; 타임라인 b4797c2446의 `literal(..., BigInteger())` 패턴과 현재 epoch 회귀 계약을 다음 백엔드 배치에 적용 · 원인 확정/수정 대기
