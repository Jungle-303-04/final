...
| `services/node-collector/node_collector.py` | `/snapshot`, `/metrics`, structured stdout log | 이미 Prometheus text format 비슷한 metric을 제공하므로 real Prometheus scrape 첫 대상으로 쓰기 좋다. |
...
- `services/node-collector/node_collector.py`는 이미 `GET /metrics`를 제공한다.
- metric 예시:
