...
### [D-048] 2026-07-14 — UI 기획 확정 + 코덱스 목표모드 전환 (작성: 우녕 확정 / 조율 세션 기록)

**우녕이 취침 전 확정한 사항이다. 이전 UI 기획보다 우선한다.**

**정본 문서 (읽는 순서):**
1. `docs/auto/codex-goal-directive-20260714.md`  ← **이 지시서 하나만 따르면 된다**
...
| # | 이전 | 확정 |
|---|---|---|
| 1 | 상세 패널 3단계 (480 → 드래그 → 전체) | **2상태. 열면 곧바로 전체화면으로 목록을 덮는다.** 드래그 없음 |
| 2 | AI 열기 = 2단계 애니메이션 | **1단계.** 상세가 이미 전체화면이므로 |
| 3 | 필터=줌 (정적 전환) | **카메라 이동. SLG 게임 상태창 수준의 다이나믹.** FLIP 모프 + 스태거 |
| 4 | 진행 = 프로그레스바 | **고스트 파드 + 단계 레일 + 트레이스 워터폴.** 시간 기반 막대 전면 금지 |
| 5 | 상세 열림이 URL에 없음 | **`?detail=<kind>/<ns>/<name>`.** 새로고침·링크 공유·뒤로가기=닫기 |

**모드: GOAL MODE.** 승인을 기다리며 멈추지 않는다. 예외는 셋뿐이다 —
(a) 비밀 값을 다뤄야 할 때, (b) 파괴적 작업, (c) VP-015와 모순되는 요구 발견.
그 경우 night-log에 선택지를 적고 **다른 슬라이스를 계속 진행한다.**
...
**팀원 확인 경로:** 슬라이스를 닫을 때마다 `docs/auto/deploy-status.md` 갱신.
갱신하지 않은 슬라이스는 **완료가 아니다.**

### [D-049] 2026-07-14 — 뷰도 필터에서 파생된다 (우녕 확정)

물리 뷰 / 관계 뷰를 사람이 고르지 않는다. **필터가 뷰를 정한다.**
앱 칩이 붙으면 관계 뷰, 클러스터/네임스페이스만이면 물리 뷰, Issues 인시던트 선택이면 영향 범위.
전환 시 파드는 FLIP 모프로 날아간다 (화면이 갈리지 않는다).

**단, 사용자 의도가 자동 규칙을 항상 이긴다:**
- 조용히 바꾸지 않는다 — 1회성 힌트 + 되돌리기 버튼
- 사람이 토글하면 핀 고정. 필터를 지우면 핀이 풀린다
- 자동 뷰는 URL에 `view=`를 쓰지 않는다. 사람이 고르면 박힌다. 명시가 자동을 이긴다

정본: VP-015 §4.4 / VP-016 S9

### [D-050] 2026-07-14 — VP-018 셸 정정 (우녕 라이브 실물 검토)

**VP-015 §5.2 "상세는 열리면 곧바로 전체화면(2상태)" 철회.** 조율 세션의 과잉 일반화였다.
정본: `docs/spec/frontend/vp-018-shell-corrections.md`
지시: `docs/auto/codex-directive-vp018-corrections.md`

**C1~C9 정정을 S11 이후 신규 슬라이스보다 우선한다.**
- C1 상세 3상태(닫힘/살짝480/전체화면) 복원 + **사이드바 자동 접힘 전면 금지**
- C2 사이드바 순서(홈→클러스터→리소스→인시던트→앱→GitOps→설정) + 프로필/워크스페이스
- C3 검색 칩을 입력창 **안**으로, 두 줄 금지, 모든 목록 화면 동일 위치
- C4 그래프 640px(2배), 잘림 금지(fitView), 오버레이가 캔버스 안 덮음
- C5 AI = 폼이 아니라 **대화**(스레드/스트리밍/이력/FAB 숨김). BQ-080 스트리밍 신설
- C6 다크/라이트 실제 적용(토큰 :root/.dark 짝 가드)
- C7 클러스터 실제 provider 로고 + 실시간 폴링
- C8 아이콘 lucide 단일화(tabler 제거)
