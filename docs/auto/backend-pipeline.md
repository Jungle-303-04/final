...
| B. 큐 정합 | in_progress | A done | `docs/backend-f-workqueue.md` 갱신: BQ-001/002/003 행 상태를 `landed`로(행 제거는 프론트 소비 확인 후 규칙 유지), BQ-008을 `done-pending-merge` + lane명 기입. `docs/backend-f-progress.md`에 BQ-008 예비 항목 추가는 하지 않는다(앵커는 GO 후) | workqueue 표 ↔ origin/dev·lane 실물 1:1 일치. 대조 결과를 night-log에 표로 기록 |
