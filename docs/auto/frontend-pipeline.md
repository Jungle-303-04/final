...
| E. VP-002/003 | in_progress | **BQ-004가 origin/dev 착륙 + progress 앵커 확인** (§1.2 명령, 출력 기록). 그 전 착수 금지 | §3 절차로 VP-002(타임라인 소비: 커서 페이지네이션, causation_id null 허용) → VP-003(여정 뷰: UI-056 TimelineSwimlane 선례 custom, causation_id 있으면 인과 트리·없으면 시간순 강등, 접근성: 키보드 이벤트 목록 병행 제공) | 진입조건 검증 기록 + 행별 full gate + 앵커 |
