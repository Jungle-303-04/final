...
| E1. VP-002 감사 타임라인 | done | BQ-004 앵커 `66cbe8dec7cb478f5b0774bb5e7bbaab5f616894`가 `origin/dev`에 착륙 | 서버 순서·불투명 cursor·nullable `causation_id`를 보존하는 Issues 독립 감사 섹션 | API `4c2598c4a`, 화면 `fdc921c3d`, 책임 분리·전체 게이트 `9bedcdfa5` |
| E2. VP-003 이벤트 여정 | blocked | `AuditTimelineItem.event_id`와 canonical subject→journey-stage 분류 계약·앵커가 `origin/dev`에 착륙해야 재개 | additive 계약 전 합성 ID·subject prefix 추측·가짜 인과선을 만들지 않는다. 계약 착륙 뒤 인과 트리/시간순 강등과 키보드 이벤트 목록을 RED→GREEN으로 구현 | blocker는 verified insertion map §1 VP-003과 일치. 재개 후 full gate + 화면 앵커 |
