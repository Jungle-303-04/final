...
|---|---|---|---|---|
| A. 착륙 재검증 | pending | 없음(최초) | [D-002]가 보류시킨 판정 2건을 origin 기준으로 해소: ① `git merge-base --is-ancestor 44f35234e origin/dev` ② `git cat-file -e origin/dev:docs/backend-f-progress.md` ③ `git cat-file -e origin/dev:src/domains/rca_bundle/router.py`. 3건 모두 성공 시: VP-001의 "[착륙 대기 — 앵커 미유효]" 표기와 1b 인용구의 착륙 대기 문구를 제거하고 판정을 `직결`(확정)로, APIQ-029 동결 해제, APIQ-012 claim 가능으로 갱신 | 명령 3건의 출력 전문이 night-log에 기록됨 + VP-001 행·1b 갱신 완료. 하나라도 실패하면 갱신 없이 BLOCKED |
| A2. dev→lab 동기화 | pending | A done ([D-009] 2항) | origin/dev를 woonyong/ui-layer-lab에 단방향 merge해 465커밋 merge 부채를 해소. **첫 동기화는 🔒 사람 GO 필수** — GO-REQUEST [FE-A2] 블록(충돌 예측: 프론트 소유 경로 안/밖 구분, merge 명령, abort 명령) 작성 후 대기. GO 후: 프론트 소유 경로 밖 충돌은 dev 채택, 안쪽 충돌은 수동 해소, merge 후 full gate. 이후 반복 동기화는 같은 조건으로 Codex 자동 수행 허용 | merge 완료 + full gate PASS + night-log에 충돌 해소 내역 기록. 실패 시 merge abort + BLOCKED |
| B. APIQ-029 완료 | pending | A done (A2와 병행 가능 — A2 대기가 B를 막지 않음) | 동결 중 작성한 `rca-bundle-schemas.ts`를 착륙본과 대조: origin/dev의 `docs/spec/remediation-bundle.schema.json`(경로가 다르면 `git ls-tree origin/dev docs/spec/ docs/`로 탐색)과 progress의 BQ-003 인계 절을 읽어 필드·타입·null 규칙을 diff. 불일치 시 zod 수정(§1.6 유지). 이후 기존 완료 절차: full gate → `API 완성: getRemediationBundle (<hash>)` 앵커 → VP-001 행 기록 | diff 대조 결과(불일치 목록 또는 "일치") night-log 기록 + full gate PASS + 앵커 |
| C. APIQ-012 | pending | B done (큐 lock 순서: 029 → 012 → 027) | AcceptedResponse zod에 `command_id: z.string().nullable()` 추가(스키마 주석에 "null = 승인 필요 경로: 제출 시점 파생 불가" 명시). adapter 분기: non-null → `getCommandStatus` 폴링으로 직접 추적(CommandStatus 5종 리터럴 축소 금지), null → 기존 30s 갱신 유지. optimistic 완료 표시 금지 불변. REF-API-076 어댑터 판정·Cross-Gap-001 비고를 최신 상태로 갱신 | 신규 테스트(null/non-null 분기) + full gate PASS + 앵커 |
| D. 잔여 큐 소화 | pending | C done | api-needs.md의 requested 행을 고정 순서대로 1행씩(APIQ-027부터). 각 행: claim 조율 커밋 → 구현 → full gate → 앵커 → 행 정리. 실패 행은 blocked 표기 후 다음 행 진행 가능(단 lock은 1행 유지) | 각 행별 앵커 + full gate. 남은 requested가 0이거나 전부 blocked 사유 기록 |
...
| F. 전체 게이트 | pending | D done (E는 진입조건 충족 시 포함) | `npm run check` 전체 + `npm run visual-product`(격리 시나리오, unexpected network 0 기준) 최종 실행 | 두 명령 PASS 전문 기록. 실패 시 자가수정 루프(3회 규칙) |
