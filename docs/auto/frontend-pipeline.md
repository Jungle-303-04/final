...
| D. 잔여 큐 소화 | in_progress | C done | api-needs.md의 requested 행을 고정 순서대로 1행씩 처리한다. APIQ-027은 `b92d081eb` 앵커로 완료했고 다음 행은 APIQ-005다. 각 행: claim 조율 커밋 → 구현 → full gate → 앵커 → 행 정리. 실패 행은 blocked 표기 후 다음 행 진행 가능(단 lock은 1행 유지) | 각 행별 앵커 + full gate. 남은 requested가 0이거나 전부 blocked 사유 기록 |
