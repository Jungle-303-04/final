...
| 6. 거대한 dashboard | 격리 | frontend dashboard, HTTP route, dashboard worker와 fleet/home projection을 제거. `RcaTimeline` model/repository는 RCA·change·retention query의 기존 영속 테이블이라 유지 |
| 7. 개인 AWS 및 외부 터널 배포 경로 | 삭제 | Terraform, 개인 workflow/script/launcher, 외부 터널 manifest, AWS deploy catalog와 Secrets Manager adapter를 제거. EKS 같은 provider 문자열은 수집 증거의 cluster metadata 분류에만 존재 |
