...
- fleet 건강 점수 그리드 / 업그레이드 현황 패널 (기존 대시보드 mock 뷰) — 사용자 결정으로 폐기. 해당 UI용 백엔드 집계 API는 만들지 않는다.
...
- 완료 기준: mock LLM 단위 테스트(이벤트 in → 후보 out), 실키 opt-in 테스트, 룰 미매칭 인시던트가 backlog가 아닌 LLM 후보 경로로 RCA 완료되는 골든패스 테스트.
