...
- 가인: [Project Map](./PROJECT-MAP.md)에서 현재 target 제거 상태, route surface, worker composition, provider 경계를 먼저 확인한다.
...
- command: `domains.command`, command worker, command API router는 아직 코드에 남아 있으며 [Cleanup Matrix](./CLEANUP-MATRIX.md)의 삭제 후보로 다룬다. target-agent command adapter는 현재 코드에 없다.
- target: `src/domains/target`과 `src/services/target` service 표면은 현재 코드에서 삭제된 상태이며, 남은 target 표현은 계약·read model naming으로만 확인한다.
...
- provider: target-agent `@telemetry.source(...)` module은 현재 코드에 없다. provider 경계는 Helm/GitOps/Inventory/AI 계약별로 분리해 보고, Kubernetes read-only 근거는 chart RBAC와 `make manifest-check`를 기준으로 한다.
