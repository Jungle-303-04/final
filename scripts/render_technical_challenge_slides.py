...
class Canvas:
    ...
    def center_text(
        ...
        self.text(
            x, y, lines, size=size, weight=weight, fill=fill, anchor="middle", line_gap=line_gap
        )

    def pill(
        self, x: int, y: int, w: int, h: int, label: str, fill: str, fg: str = "white"
    ) -> None:
    ...
    def card(
        ...
        self.add(
            f'<line x1="{x}" y1="{y + 46}" x2="{x + w}" y2="{y + 46}" stroke="{stroke}" stroke-width="2"/>'
        )
        ...
        if len(title_lines) == 1:
            ...
            self.center_text(
                x + w // 2, y + 20, title_lines, size=17, weight=800, fill=title_fill, line_gap=18
            )
...
def slide_desired_state() -> Canvas:
    ...
    c.card(
        ...
        [
            "GitOps 상태를 K8s resource로 반영",
            "SQL과 CRD 기준점 동기화",
            "운영급 controller 확장 준비",
        ],
        fill=GREEN_LIGHT,
        stroke=GREEN,
    )
...
def slide_reconciler() -> Canvas:
    ...
    c.card(
        ...
        122,
        "Workflow Controller",
        ["desired state 저장", "승인 단계와 실행 흐름 관리"],
        fill=BLUE_LIGHT,
    )
    c.card(
        86,
    ...
    c.card(
        ...
        ["desired / actual 비교", "drift 이벤트 생성"],
        fill=BLUE_LIGHT,
    )
    ...
    c.card(
        ...
        ["actual state 조회", "명령 수신 / 정책 guard"],
        fill=GREEN_LIGHT,
        stroke=GREEN,
    )
    c.card(
        ...
        280,
        122,
        "Agent GitOps Executor",
        ["dry-run / server-side apply", "rollout watch / actual snapshot"],
        fill=GREEN_LIGHT,
        stroke=GREEN,
        dashed=True,
    )
    c.card(
        ...
        100,
        "Kubernetes API + RBAC",
        ["sandbox namespace만 write", "pods/events/nodes/services read"],
        fill="white",
        stroke=GREEN,
    )
    ...
    c.text(
        ...
        575,
        [
            "핵심 판단: credential을 Management Plane에 많이 저장하지 않기 위해",
            "실제 apply는 Target Agent 쪽으로 이동한다.",
        ],
        size=18,
        weight=800,
        fill=TEXT,
...
def slide_git_cache() -> Canvas:
    ...
    c.card(
        ...
        290,
        "GitHub Repo",
        ["branch / PR / commit", "Helm chart / values", "Kustomize / raw YAML"],
        fill="white",
        stroke="#374151",
    )
    c.text(
        112,
        505,
        ["문제", "여러 workflow가 매번 clone/render하면", "느리고 실패 지점이 늘어난다."],
        size=20,
        weight=800,
        fill=TEXT,
        line_gap=30,
    ...
    c.card(
        ...
        112,
        "Git Cache\nWorker",
        ["repo mirror/fetch", "commit checkout"],
        fill="white",
    )
    c.card(
        676,
    ...
    c.card(
        ...
        90,
        "Object / Secret\nStores",
        ["artifact bundle", "digest / provenance"],
        fill=PURPLE_LIGHT,
        stroke=PURPLE,
    )
    ...
    c.card(
        ...
        122,
        "Diff Worker",
        ["desired manifest와", "actual snapshot 비교"],
        fill=GREEN_LIGHT,
        stroke=GREEN,
    )
    c.card(
        ...
        122,
        "Target Reconcile Worker",
        ["drift/reconcile 이벤트 생성", "배포 상태 추적"],
        fill=GREEN_LIGHT,
        stroke=GREEN,
    )
    ...
    c.text(
        ...
        579,
        [
            "해결: repo + commit + chart version 기준으로 cache하고,",
            "결과물은 digest와 provenance로 추적한다.",
        ],
        size=19,
        weight=900,
        fill=TEXT,
...
def slide_closed_loop() -> Canvas:
    ...
    c.add(
        f'<polygon points="1152,342 1232,410 1152,478 1072,410" fill="{ORANGE_LIGHT}" stroke="{ORANGE}" stroke-width="2"/>'
    )
    ...
    c.card(
        ...
        108,
        "Approval / Command\nWorker",
        ["사람 승인 후 실행", "auto apply는 정책으로 차단"],
        fill=ORANGE_LIGHT,
        stroke=ORANGE,
    )
