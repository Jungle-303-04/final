...
def test_feature_contract_catalog_route_returns_generated_catalog_for_authenticated_session() -> (
    ...
    assert catalog.features[0].source_key is None
    assert catalog.features[0].legacy_contract_ids == ("reference.feature.001",)
    assert catalog.features[0].identity_status == "legacy-unmapped"
    serialized_feature = catalog.features[0].model_dump(by_alias=True)
    assert serialized_feature["sourceKey"] is None
