...
def test_agent_kubernetes_surface_is_read_only() -> None:
    ...
    manifest_check = source("scripts/manifest-check.sh")
    target_provider_dir = Path(ROOT, "src/services/target/cluster-agent/providers")

    assert not target_provider_dir.exists()
    assert 'verbs: ["get", "list", "watch"]' in rbac
    assert not any(
        token in rbac
        for token in ("pods/exec", "pods/attach", "pods/portforward", "nodes/proxy")
    )
    assert "only get/list/watch RBAC verbs are allowed" in manifest_check
    assert "wildcard RBAC is forbidden" in manifest_check
    assert "forbidden direct-execution RBAC resource" in manifest_check
