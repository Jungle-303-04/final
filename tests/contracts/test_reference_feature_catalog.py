...
def test_generated_feature_contract_catalog_contains_every_ledger_feature() -> None:
    ...
    assert all(
        feature.backend_contract == "packages.contracts.parity" for feature in catalog.features
    )
    assert all(
        feature.frontend_contract.endswith("referenceParity.ts") for feature in catalog.features
    )
