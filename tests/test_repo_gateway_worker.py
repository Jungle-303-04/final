...
def test_repo_gateway_creates_pr_from_request() -> None:
    ...
    db = SpyDb()
    outs = run_handler(repo.on_safe_pr_requested, SafePrRequestedBody(title="t", body="b", provider="github"), db=db)
...
def test_repo_gateway_emits_failed_event_on_provider_error() -> None:
    repo = load_service("gitops/repo-gateway-worker")
    outs = run_handler(repo.on_safe_pr_requested, SafePrRequestedBody(title="t", body="b", provider="github"), db=FailingDb())
