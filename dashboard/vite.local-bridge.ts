...
async function gatewayHealth() { try { const r = await cmd("curl", ["-sS", "-m", "3", `${gatewayTarget}/healthz`], finalRoot, 5000); return { status: r.stdout.includes("ok") ? "정상" as Status : "주의" as Status, detail: `${gatewayTarget} ${r.stdout}` }; } catch (e) { return { status: "오류" as Status, detail: e instanceof Error ? e.message : String(e) }; } }
...
async function saveVersion(yaml: string) { const v = releaseVersion(); await mkdir(path.dirname(manifestPath), { recursive: true }); await writeFile(manifestPath, versioned(yaml, v)); await cmd("kubectl", ["apply", "--dry-run=client", "-f", manifestPath], finalRoot); await cmd("git", ["add", "--", rel(manifestPath)], finalRoot); const commit = await cmd("git", ["commit", "-m", `dashboard: update kubernetes manifest ${v}`, "--", rel(manifestPath)], finalRoot); emit("gitops.versioned", "정상", "버전 커밋 완료", commit.stdout); return { version: v, commit, state: await gitopsConfig() }; }
...
async function fault() { emit("fault.inject", "주의", "Gateway scale 0"); await cmd("kubectl", [...mgmt, "scale", "deploy/api-gateway", "--replicas=0"], finalRoot); return { health: await gatewayHealth(), overview: await overview() }; }
async function recover() { emit("recovery.start", "주의", "Gateway scale 1"); await cmd("kubectl", [...mgmt, "scale", "deploy/api-gateway", "--replicas=1"], finalRoot); await cmd("kubectl", [...mgmt, "rollout", "status", "deploy/api-gateway", "--timeout=180s"], finalRoot); return { health: await gatewayHealth(), overview: await overview() }; }
