...
spec:
  ...
  template:
    ...
    spec:
      containers:
        - name: checkout-api
          image: ghcr.io/project/checkout-api:stable
          ports:
            - containerPort: 8080
`;
const steps = ["Gateway 확인", "로그인", "클러스터 연결", "설정 가져오기", "diff 보기", "버전 커밋", "PR 생성", "머지 반영", "YAML dry-run", "배포 명령", "SSE 조회", "스케일 조정", "장애 주입", "복구 확인"];
const eventNames = ["bridge.connected", "bridge.heartbeat", "repo.fetch.start", "repo.fetch.done", "build.start", "build.done", "yaml.dry_run", "gitops.diff", "gitops.versioned", "gitops.pr.start", "gitops.pr.done", "deploy.start", "deploy.done", "cluster.scale.start", "cluster.scale.done", "fault.inject", "recovery.start"];
...
export function App() {
  ...
  const [prTitle, setPrTitle] = useState("dashboard: update kubernetes manifest");
  ...
  useEffect(() => { void refresh(false); void loadConfig(false); openBridge(); return () => { bridge.current?.close(); stream.current?.close(); }; }, []);
  ...
  return (
    <main className="app">
      ...
      <section className="hero-grid"><article className="primary-status"><p className="eyebrow">현재 상태</p><h2>{health?.status === "정상" ? "클러스터 Gateway가 연결되어 있습니다" : "확인 또는 복구가 필요합니다"}</h2><div className="status-strip"><Badge s={health?.status ?? "대기"} t={`Gateway ${health?.status ?? "수집 전"}`} /><Badge s={overview?.busy ? "주의" : "정상"} t={overview?.busy ? "파이프라인 실행 중" : "파이프라인 대기"} /><Badge s={sse === "연결" ? "정상" : sse === "오류" ? "오류" : "대기"} t={`SSE ${sse}`} /><Badge s={token ? "정상" : "대기"} t={session} /></div></article><article className="scenario"><p className="eyebrow">발표 실행 순서</p><div className="scenario-row">{steps.map((s, i) => <div className={completed.has(s) ? "scenario-step done" : "scenario-step"} key={s}><span>{String(i + 1).padStart(2, "0")}</span><strong>{s}</strong></div>)}</div></article></section>
      <section className="main-grid"><div className="left-stack">
        ...
        <Panel title="Gateway / 배포 / 장애 복구" icon={<PlugZap size={18} />}><div className="button-grid"><button onClick={() => void run("Gateway 확인", async () => { const r = await gateway<{ status: string; service: string }>("/healthz"); return `${r.service}: ${r.status}`; })} disabled={!!busy}><Wifi size={15} />Gateway 확인</button><button onClick={login} disabled={!!busy}><ShieldCheck size={15} />로그인</button><button onClick={() => void run("클러스터 연결", async () => { await gateway("/agent/connect", { method: "POST", body: JSON.stringify({ cluster_id: "target-cluster-01", agent_id: "dashboard-live-agent", capabilities: ["collector", "command_receiver"] }) }); return "target-cluster-01 연결"; })} disabled={!!busy}><Server size={15} />클러스터 연결</button><button onClick={() => void run("Git webhook", async () => { const r = await gateway<{ accepted: boolean }>("/github/webhook", { method: "POST", body: JSON.stringify({ commit_sha: finalRepo?.head ?? "dashboard", image: "ghcr.io/project/checkout-api:stable", replicas: 2 }) }); return `accepted=${r.accepted}`; })} disabled={!!busy}><GitBranch size={15} />Git webhook</button><button onClick={() => void run("배포 명령", async () => { const r = await gateway<{ accepted: boolean; correlation_id?: string }>("/commands", { method: "POST", auth: true, body: JSON.stringify({ cluster_id: "target-cluster-01", action: "rollout_restart", namespace: "sandbox", reason: "dashboard live demo" }) }); return `accepted=${r.accepted}; ${r.correlation_id ?? ""}`; })} disabled={!!busy}><Activity size={15} />배포 명령</button><button onClick={() => void run("SSE 조회", async () => { const r = await gateway<{ cards: unknown[] }>("/dashboard/query", { auth: true }); setCards(r.cards?.length ?? 0); return `cards=${r.cards?.length ?? 0}`; })} disabled={!!busy}><Radio size={15} />read model 조회</button><button className="danger" onClick={() => void run("장애 주입", async () => { const r = await local<{ health: { status: Status; detail: string }; overview: Overview }>("/fault/gateway/down", { method: "POST" }); setOverview(r.overview); return `${r.health.status}; ${r.health.detail}`; })} disabled={!!busy}><ShieldAlert size={15} />Gateway 내리기</button><button className="primary" onClick={() => void run("복구 확인", async () => { const r = await local<{ health: { status: Status; detail: string }; overview: Overview }>("/fault/gateway/recover", { method: "POST" }); setOverview(r.overview); return `${r.health.status}; ${r.health.detail}`; })} disabled={!!busy}><Undo2 size={15} />Gateway 복구</button></div><p className="hint">장애 주입은 kind-management deploy/api-gateway replica를 0으로 만들고, 복구는 1로 되돌립니다.</p></Panel>
