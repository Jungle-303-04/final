...
with sync_playwright() as p:
    ...
    step('드릴다운 URL 복원', s3)
    def s4():
        pg.goto(f'{BASE}/clusters'); pg.wait_for_selector('tbody tr')
    ...
    step('알림 합성 피드', s10)
    def s11():
        pg.goto(f'{BASE}/settings/members'); pg.wait_for_selector('text=teammate@example.com')
    ...
    if errors: bad('콘솔 pageerror 0건', '; '.join(errors[:2]))
    else: ok('콘솔 pageerror 0건')
    b.close()
