...
function Meta({ label, value }: { label: string; value: React.ReactNode }) {
  return <div style={itemStyle}><span style={{ color: UI.ink3, fontSize: TYPE.caption2 }}>{label}</span><span style={{ color: UI.ink, fontFamily: MONO, fontSize: TYPE.caption2, fontWeight: 700, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{value}</span></div>;
}
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 7 }}>
      <div style={{ alignItems: "center", color: UI.ink2, display: "flex", fontSize: TYPE.label2, gap: 7, marginBottom: 2 }}>
        ...
        {data.observed_at && <span style={{ color: UI.ink3, fontSize: TYPE.caption2, marginLeft: "auto" }}>{data.observed_at.replace("T", " ").slice(0, 16)}</span>}
      </div>
      {data.type === "subject" && <>
        ...
        {data.truncated && <span style={{ background: inkA(0.05), borderRadius: 7, color: UI.ink3, fontSize: TYPE.caption2, padding: "6px 9px" }}>수집 한도로 일부 권한만 표시합니다.</span>}
      </>}
      {data.type === "role" && <>
