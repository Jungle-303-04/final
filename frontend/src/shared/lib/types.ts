...
export interface ReleaseRunHandoffAction { action: string; label: string; enabled: boolean }
export interface ReleaseRunHandoffCheck { name: string; status: 'passed'|'warning'|'blocked'|'info'|string; message: string }
export interface ReleaseRunHandoffVerificationJob { job_id: string; application_id: string; kind: string; status: string; target: Record<string, unknown>; result?: Record<string, unknown>; error?: string }
export interface ReleaseRunHandoffVerification { status: 'passed'|'warning'|'blocked'|'info'|string; message: string; evidence: string[]; jobs?: ReleaseRunHandoffVerificationJob[]; job_count?: number; override_reason?: string | null; production_targets: string[] }
...
export interface ReleaseRunHandoff { run_id: string; plan_id: string; plan_name: string; status: string; headline: string; severity: string; current_wave: number; total_waves: number; live_side_effects: boolean; attention_reasons: string[]; verification?: ReleaseRunHandoffVerification; abort_criteria?: ReleaseRunHandoffAbortCriteria; next_actions: ReleaseRunHandoffAction[]; checks: ReleaseRunHandoffCheck[]; last_event?: Record<string, unknown> | null }
export interface ReleaseRunSummary { total_runs: number; status_breakdown: Record<string, number>; plan_breakdown: Record<string, number>; active_runs: number; attention_required_runs: number; failed_runs: number; rollback_requested_runs: number; waiting_for_approval_runs: number; live_runs: number; unhealthy_runs: number; verification_failed_runs: number; stale_runs: number; last_run_status?: string | null; recent_runs: { run_id: string; plan_id: string; status: string; attention_reasons?: string[] }[] }
