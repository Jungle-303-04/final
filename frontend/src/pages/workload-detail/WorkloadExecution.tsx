...
export function WorkloadExecution({ port, request }: { port: WorkloadDetailPort; request: WorkloadDetailRequest }) {
  ...
  if (frame.phase === "loading") return <p className="py-8 text-sm text-muted-foreground" role="status">Loading retained executions…</p>;
  if (frame.phase === "failed") {
    return <p className="rounded-xl border border-dashed p-6 text-sm text-muted-foreground">Execution history is unavailable for this authorized scope.</p>;
  }
  ...
  return (
    <section className="grid min-w-0 gap-4" aria-labelledby="execution-history-title">
      <header className="flex flex-wrap items-start justify-between gap-3">
        <div className="min-w-0"><h2 className="text-base font-semibold" id="execution-history-title">Retained executions</h2><p className="text-sm text-muted-foreground">Server-authorized run identity, outcome, and lifecycle evidence.</p></div>
        <Button disabled={refreshing} onClick={onRefresh} size="sm" type="button" variant="outline"><RefreshCw aria-hidden="true" className={cn(refreshing && "motion-safe:animate-spin motion-reduce:animate-none")} />Refresh</Button>
      </header>
      {!catalog.complete ? <p className="rounded-lg border border-amber-500/30 bg-amber-500/10 px-3 py-2 text-sm" role="status">The retained run catalog is partial.</p> : null}
      {catalog.runs.length === 0 ? <p className="rounded-xl border border-dashed p-6 text-sm text-muted-foreground">No retained execution is available.</p> : (
        <div className="grid min-w-0 gap-4 lg:grid-cols-[minmax(14rem,20rem)_minmax(0,1fr)]">
          <div className="grid content-start gap-2" role="listbox" aria-label="Retained executions">
          ...
          {selected ? <div className="grid min-w-0 gap-4 rounded-xl border bg-card p-4">
            <div className="flex flex-wrap items-start justify-between gap-3"><div className="min-w-0"><h3 className="truncate font-medium">{selected.resource.name}</h3><p className="text-sm text-muted-foreground">Pods {selected.podTotal} · succeeded {selected.podSucceeded} · failed {selected.podFailed} · running {selected.podRunning}</p></div>{selected.nextStep === "logs" ? <Button onClick={() => openLogs(selected)} size="sm" type="button"><Terminal aria-hidden="true" />View logs</Button> : selected.nextStep === "timeline" ? <Badge variant="outline"><Activity aria-hidden="true" className="mr-1 h-3 w-3" />Timeline evidence</Badge> : null}</div>
            <ol className="grid gap-2" aria-label="Execution lifecycle">{events.map((event) => <li className="flex min-w-0 items-start justify-between gap-3 rounded-lg bg-muted/30 px-3 py-2" key={event.eventId}><span className="min-w-0 truncate text-sm">{event.reason}</span><time className="shrink-0 text-xs text-muted-foreground" dateTime={event.occurredAt}>{formatTime(event.occurredAt)}</time></li>)}</ol>
            {selected.nextStep === "logs" ? <p className="flex items-center gap-1 text-xs text-muted-foreground"><ExternalLink aria-hidden="true" className="h-3 w-3" />Logs open in the shared bounded stream dock.</p> : null}
          </div> : null}
        </div>
      )}
    </section>
