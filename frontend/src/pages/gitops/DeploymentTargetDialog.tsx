...
export function DeploymentTargetDialog({
  ...
  return (
    <Dialog onOpenChange={changeOpen} open={open}>
      ...
      <DialogContent className="max-h-[calc(100dvh-2rem)] overflow-y-auto sm:max-w-2xl" closeLabel={t("common.action.close")} showCloseButton={!pending}>
        <DialogHeader>
        ...
        <div className="grid min-w-0 gap-4 sm:grid-cols-2">
          ...
          <FormField label={t("workflows.target.repository")}>
            <Input onChange={(event) => update("repository", event.target.value)} placeholder={t("workflows.target.repositoryPlaceholder")} value={input.repository} />
          </FormField>
          <FormField label={t("workflows.target.branch")}>
            <Input onChange={(event) => update("branch", event.target.value)} value={input.branch} />
          </FormField>
          <FormField label={t("workflows.target.manifestPath")}>
            <Input onChange={(event) => update("manifestPath", event.target.value)} value={input.manifestPath} />
          </FormField>
          <FormField
          ...
          <FormField label={t("workflows.target.namespace")}>
            <Input onChange={(event) => update("namespace", event.target.value)} value={input.namespace} />
          </FormField>
          ...
          <FormField label={t("workflows.target.token")}>
            <Input autoComplete="off" onChange={(event) => update("token", event.target.value)} type="password" value={input.token || ""} />
          </FormField>
        </div>
