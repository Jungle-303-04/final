    ...
    <section aria-labelledby="helm-release-detail-title" className="grid min-w-0 gap-4">
      ...
      <section className="grid gap-2" aria-labelledby="helm-release-integrations-title">
        ...
        <dl className="grid min-w-0 gap-2 sm:grid-cols-2">
          <ResourceHealthFact feature={detail.release.resourceHealth} label={HELM_COPY.resourceHealth} />
          ...
          <OwnedResourcesFact feature={detail.ownedResources} label={HELM_COPY.ownedResources} />
          <CommandsFact feature={detail.commands} label={HELM_COPY.commands} />
        </dl>
      </section>
    </section>
  );
...
function ObservedFact({ label, value }: { label: string; value: string }) {
  return <div className="grid min-w-0 gap-1 rounded-lg border bg-card px-3 py-2"><dt className="text-sm font-medium">{label}</dt><dd className="m-0 break-words text-xs text-muted-foreground">{value}</dd></div>;
}

function ResourceHealthFact({ feature, label }: { feature: HelmResourceHealth; label: string }) {
  if (feature.availability === "unavailable") return <UnavailableFact feature={feature} label={label} />;
  return <ObservedFact label={label} value={`${feature.health} (${feature.resourceCount})`} />;
}

function OwnedResourcesFact({ feature, label }: { feature: HelmOwnedResources; label: string }) {
  if (feature.availability === "unavailable") return <UnavailableFact feature={feature} label={label} />;
  return <ObservedFact label={label} value={String(feature.items.length)} />;
}

function CommandsFact({ feature, label }: { feature: HelmCommands; label: string }) {
  if (feature.availability === "unavailable") return <UnavailableFact feature={feature} label={label} />;
  return <ObservedFact label={label} value={feature.actions.join(", ")} />;
}
