    ...
    <section className="grid min-w-0 gap-4" aria-labelledby="alert-rules-title">
      <header className="flex min-w-0 flex-wrap items-end justify-between gap-3">
        ...
        <div className="flex shrink-0 items-center gap-2">
          ...
          <Button disabled={pendingId !== null} onClick={beginCreate} type="button"><BellPlus aria-hidden="true" />{t("alerts.rules.create")}</Button>
        </div>
      </header>
      ...
      {loading ? (
        ...
        <div className="grid min-h-52 place-items-center rounded-xl border border-dashed p-8 text-center">
          <div className="grid justify-items-center gap-2"><BellPlus className="size-6 text-muted-foreground" /><p className="font-medium">{t("alerts.rules.empty.title")}</p><p className="text-sm text-muted-foreground">{t("alerts.rules.empty.description")}</p><Button onClick={beginCreate} size="sm">{t("alerts.rules.create")}</Button></div>
        </div>
      ) : (
              ...
              >
                ...
                <CardContent className="grid gap-3">
                  ...
                  <div className="flex flex-wrap justify-end gap-2">
                    <Button disabled={pendingId !== null} onClick={() => void toggle(rule)} size="sm" variant="outline">{pendingId === rule.id && pendingOperation === "toggle" ? <Spinner /> : <Power aria-hidden="true" />}{rule.enabled ? t("alerts.rules.disable") : t("alerts.rules.enable")}</Button>
                    <Button disabled={pendingId !== null} onClick={() => beginEdit(rule)} size="sm" variant="outline"><Pencil aria-hidden="true" />{t("alerts.rules.edit")}</Button>
                    <Button disabled={pendingId !== null} onClick={() => setDeleteRule(rule)} size="sm" variant="outline"><Trash2 aria-hidden="true" />{t("alerts.rules.delete")}</Button>
                  </div>
                </CardContent>
              </Card>
            );
