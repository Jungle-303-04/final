...
export const DUMMY_CONVERSATION: AiConversation = {
  ...
  turns: [
    ...
    },
    {
      id: "t1",
      role: "user",
      question: "checkout-api 파드가 왜 죽었어?",
      collapsed: false,
      ...
      parts: [
        {
          ...
          running: false,
          steps: [
            { id: "s1", label: "리소스 조회", detail: "checkout-api 파드 3건", state: "done" },
        ...
        },
        {
          kind: "text",
          markdown:
            "`checkout-api-0` 파드가 **OOMKilled**로 종료됐습니다. 관측된 근거상 메모리 사용률이 한도(512Mi) 대비 **107%**까지 올라 컨테이너가 강제 종료됐고, 직전 5분간 요청량이 3배로 늘었습니다.",
        },
        {
          kind: "result",
          title: "OOMKilled",
        ...
        },
        {
          kind: "evidence",
          items: [
            { type: "event", id: "e1", label: "Event · OOMKilled @ 19:34", link: "/resources?detail=pod/shop/checkout-api-0" },
            { type: "metric", id: "m1", label: "메모리 시계열 · 107%", link: "/resources?detail=pod/shop/checkout-api-0&tab=metrics" },
            { type: "log", id: "l1", label: "로그 · signal: killed", link: "/resources?detail=pod/shop/checkout-api-0&tab=logs" },
          ],
        },
        {
          kind: "links",
          items: [
            { label: "이 파드 상세 열기", href: "/resources?detail=pod/shop/checkout-api-0", icon: "resources" },
...
export const DUMMY_CONVERSATION_LIST = [
  { id: "aic-dummy", title: "checkout-api 장애 분석", updatedAt: "방금" },
