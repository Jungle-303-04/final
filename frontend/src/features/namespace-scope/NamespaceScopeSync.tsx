...
import type { NamespaceScope } from "../../api/shell-state-schemas";
...
export function NamespaceScopeSync() {
  ...
  useEffect(() => {
    ...
    let active = true;
    void getNamespaceScope(clusterId, controller.signal).then((scope) => {
      if (!active) return;
      setRecord(scope);
      hydratedCluster.current = clusterId;
      if (selectedNamespaces.length === 0 && scope.actives.length > 0) {
        filter.updateFilters((current) => ({
          ...
          common: {
            ...
            namespaces: [
              ...
              ...scope.actives.map((namespace) => ({ clusterId, namespace })),
            ],
          },
        }), "legacy-migration");
      }
  ...
  useEffect(() => {
    if (
      clusterId === null ||
      record === null ||
      record.cluster_id !== clusterId ||
      hydratedCluster.current !== clusterId
    ) return;
    const currentKey = [...record.actives].sort().join("\u0000");
    if (currentKey === selectionKey) return;
    const controller = new AbortController();
    let active = true;
    const timer = window.setTimeout(() => {
      void updateNamespaceScope({
        clusterId,
        ...
        if (!active) return;
        setRecord(next);
        dock.invalidateNamespaceScope(clusterId, next.actives);
        publishNamespaceScopeInvalidation({
          clusterId,
          allowedNamespaces: next.actives,
        });
      }, (error: unknown) => {
        if (!active) return;
        if (isApiError(error) && error.status === 409) {
          void getNamespaceScope(clusterId, controller.signal).then((next) => {
  ...
  }, [clusterId, dock, record, selectedNamespaces, selectionKey]);
