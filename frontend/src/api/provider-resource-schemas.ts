...
const containerProjection = z.strictObject({
  name: z.string().min(1),
  ...
  requests: z.array(keyValue).max(100),
  limits: z.array(keyValue).max(100),
});
const coreWorkload = z.strictObject({
  type: z.literal("core-workload"),
  kind: z.string().min(1),
  owner: namedReference.nullable(),
  replicas,
  unavailable: nullableInteger,
  strategy_type: nullableString,
  max_surge: nullableString,
  max_unavailable: nullableString,
  min_ready_seconds: nullableInteger,
  revision_history_count: nullableInteger,
  service_account_name: nullableString,
  selector: z.array(keyValue).max(100),
  init_containers: z.array(containerProjection).max(100),
  containers: z.array(containerProjection).max(100),
  conditions,
});
const corePod = z.strictObject({
  type: z.literal("core-pod"),
  phase: nullableString,
  node_name: nullableString,
  pod_ip: nullableString,
  host_ip: nullableString,
  service_account_name: nullableString,
  owner: namedReference.nullable(),
  init_containers: z.array(containerProjection).max(100),
  containers: z.array(containerProjection).max(100),
  ephemeral_container_names: z.array(z.string()).max(100),
  conditions,
});
const coreService = z.strictObject({
  type: z.literal("core-service"),
  service_type: nullableString,
  cluster_ip: nullableString,
  external_name: nullableString,
  external_ips: z.array(z.string()).max(100),
  load_balancer_addresses: z.array(z.string()).max(100),
  external_traffic_policy: nullableString,
  internal_traffic_policy: nullableString,
  ip_families: z.array(z.string()).max(100),
  ports: z.array(z.string()).max(100),
  selector: z.array(keyValue).max(100),
  conditions,
});
const coreIngress = z.strictObject({
  type: z.literal("core-ingress"),
  ingress_class_name: nullableString,
  addresses: z.array(z.string()).max(100),
  routes: z.array(z.strictObject({
    host: nullableString,
    path: z.string(),
    path_type: nullableString,
    backend_service: z.string().min(1),
    backend_port: nullableString,
  })).max(100),
  tls: z.array(z.strictObject({
    secret_name: nullableString,
    hosts: z.array(z.string()).max(100),
  })).max(100),
  conditions,
});
const argoApplication = z.strictObject({
  type: z.literal("argo-application"),
  sync_status: nullableString,
  health_status: nullableString,
  operation_phase: nullableString,
  repository_url: nullableString,
  source_path: nullableString,
  target_revision: nullableString,
  chart: nullableString,
  destination_server: nullableString,
  destination_namespace: nullableString,
  automated: z.boolean(),
  self_heal: z.boolean(),
  prune: z.boolean(),
  retry_enabled: z.boolean(),
  managed_resource_count: nonnegativeSafeInteger,
  revision_history: z.array(z.string()).max(100),
  conditions,
});
const coreCronJob = z.strictObject({
  type: z.literal("core-cron-job"),
  schedule: nullableString,
  schedule_description: nullableString,
  timezone: nullableString,
  suspended: z.boolean(),
  last_schedule_time: nullableString,
  last_successful_time: nullableString,
  active_jobs: z.array(namedReference).max(100),
...
const coreConfigMap = z.strictObject({
  type: z.literal("core-config-map"),
  immutable: z.boolean(),
  key_count: nonnegativeSafeInteger,
...
const coreHpa = z.strictObject({
  type: z.literal("core-hpa"),
  target: namedReference.nullable(),
  minimum_replicas: nullableInteger,
  maximum_replicas: nullableInteger,
  current_replicas: nullableInteger,
  desired_replicas: nullableInteger,
  last_scale_time: nullableString,
...
const coreNode = z.strictObject({
  type: z.literal("core-node"),
  ready: z.boolean().nullable(),
  ...
  capacity: z.array(keyValue).max(100),
  allocatable: z.array(keyValue).max(100),
  usage: z.array(keyValue).max(100),
  ...
  managed_pod_count: nullableInteger,
  metrics_observed_at: nullableString,
  conditions,
});
const coreNamespace = z.strictObject({
  type: z.literal("core-namespace"),
  phase: nullableString,
  manager: nullableString,
  injection: nullableString,
  quotas: z.array(z.strictObject({
    name: z.string().min(1),
    hard: z.array(keyValue).max(100),
    used: z.array(keyValue).max(100),
  })).max(100),
  service_account_count: nullableInteger,
  role_binding_count: nullableInteger,
  cluster_role_binding_count: nullableInteger,
  conditions,
});
const coreEvent = z.strictObject({
  type: z.literal("core-event"),
  event_type: nullableString,
  reason: nullableString,
  message: nullableString,
  involved_object: reference.nullable(),
  count: nullableInteger,
  first_observed_at: nullableString,
  last_observed_at: nullableString,
  duration_seconds: nullableInteger,
  source_component: nullableString,
  source_host: nullableString,
  reporting_controller: nullableString,
  reporting_instance: nullableString,
  api_version: nullableString,
  resource_version: nullableString,
  conditions,
});
const coreRbac = z.strictObject({
  type: z.literal("core-rbac"),
  kind: z.string().min(1),
  automount_service_account_token: z.boolean().nullable(),
  secret_names: z.array(z.string()).max(100),
  image_pull_secret_names: z.array(z.string()).max(100),
  role_ref: namedReference.nullable(),
  ...
  rules: z.array(z.strictObject({
    verbs: z.array(z.string()).max(100),
    api_groups: z.array(z.string()).max(100),
    resources: z.array(z.string()).max(100),
    resource_names: z.array(z.string()).max(100),
    non_resource_urls: z.array(z.string()).max(100),
    wildcard: z.boolean(),
    escalation: z.boolean(),
  })).max(100),
  wildcard_warning: z.boolean(),
  escalation_warning: z.boolean(),
  conditions,
});
