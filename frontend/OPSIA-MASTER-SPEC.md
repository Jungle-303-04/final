...
| 클러스터 섹션 | 같은 4칸 격자(카드 2칸). '클러스터 연결'은 가로형 컴팩트 카드 2칸 — 거대 공백 금지 |
...
| 배포 워크플로우 | 배포 서피스 4탭: 애플리케이션·저장소·동기화·**워크플로우**·Helm 릴리스. 워크플로우 = GitOps 배포 파이프라인 정의(소스→중간 단계 편집→배포, 원형 archive/codex/gitOpsNode). 데모 견본도 추가·삭제·저장 실동작 |
