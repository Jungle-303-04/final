...
## D22. IA 개정 v3.1 (2026-07-19 오너 결정) — 이 절이 상충하는 이전 결정보다 우선한다
| 항목 | 결정 |
|---|---|
| 리소스 3관점 명칭 | **인프라 \| 쿠버네티스 \| 트래픽** (대상형으로 층위 통일; 의미는 D18과 동일 — 인프라=물리 지도, 쿠버네티스=kind 카탈로그, 트래픽=서비스 호출 흐름·스코프 비적용) |
| kind 탐색 오너 | 쿠버네티스 관점 본문. 인프라 뷰 우측 패널의 '리소스' 탭 폐지(중복 정의 금지). 패널명 "연결 보기" = 서비스·구성·저장소 3탭 |
| 내비 명칭 | "인시던트" → **"이슈"** (전 표면 통일: 내비·위젯 W2·타임라인 분류·알림 분류) |
| 내비 구성 | 주 내비에 **알림**·**AI 대화** 추가 (내역 모아보기 서피스, 벨/AI 패널의 "전체 보기" 목적지). 하단은 설정만 |
| 홈 보드 격자 | **4칸 그리드 + 밀집 배치(dense)**. 기본 스팬: 이슈1·저장소1·활동추이2 / 분포1·장애주의2·비용1 / 최근변경4 = 기본 배치 빈칸 0. 숨김·이동 시 작은 위젯이 위 빈칸으로 상승 |
| 클러스터 섹션 | 같은 4칸 격자(카드 2칸). 홈에는 '클러스터 연결' 카드를 두지 않는다 — 고정 헤더의 "+ 클러스터 연결" 버튼이 유일한 진입(중복 금지). 점선 연결 카드는 지도 클러스터 뷰 전용 |
| 요약 칩 규칙 | 홈·리소스 요약 줄은 **집계 칩까지만**(클러스터·노드·파드·OutOfSync n·장애 n). 개별 리소스 나열 금지 — 개별 접근은 장애 칩 렌즈/드릴·각 서피스가 오너 |
| 검색·필터 | 상단바 검색 = 전역(⌘K, 리소스+화면 점프). 표 필터(종류·네임스페이스·상태)는 쿠버네티스 관점 표 머리가 오너 |
| 배포 워크플로우 | 배포 서피스 4탭: 애플리케이션·**GitOps**(저장소 연결·동기화)·**워크플로우**·Helm 릴리스. 워크플로우 = GitOps 배포 파이프라인 정의(소스→중간 단계 편집→배포, 원형 archive/codex/gitOpsNode). 데모 견본도 추가·삭제·저장 실동작 |
| 차트 엔진 | 제품 차트는 **shadcn charts(Recharts)** + 디자인 토큰 CSS 변수 주입. 데모 SVG 차트는 시각 정답이며 제품으로 코드 이식 금지 |
