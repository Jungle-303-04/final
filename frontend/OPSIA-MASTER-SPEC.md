...
### 5.1 홈 `/home` (= 플릿, 클러스터 목록의 유일한 표면 — D19)
- 구성(위→아래): ① 상태 요약 줄(클러스터/노드/파드/OutOfSync 세그먼트+임계 칩 — 데모 사양) ② **D1 카드 그리드**(2열 max `minmax(0,560px)`, `+ 클러스터 연결` 점선 카드=D7 모달, 카드 `⋯`=연결 해제) ③ 인시던트 레일(상위 3건, 클릭=`/issues` 해당 건) ④ 라이브 밴드(기존 컴포넌트, 토큰 수렴만).
- 카드 클릭=`/resources` 지도 관점 해당 클러스터 드릴(스코프 전달). 홈의 모든 카운트는 `/resources`와 동일 어댑터 파생.
