...
### D13. 차트 (F12 해소)
- **오너**: `shared/ui/charts/`(recharts 래퍼 + 토큰 팔레트). 데모의 수제 SVG `MetricChart`·미니 바·SegRing은 **사양 원본일 뿐** — 그리드/크로스헤어/펄스/pathLength 문법(DESIGN-RULES 6장)을 recharts 래퍼와 공용 미니 컴포넌트(`shared/ui/charts/MiniBar.tsx` 등)로 재현한다.
- **금지**: 페이지 컴포넌트 안 수제 SVG 차트 신설. 두 번째 차트 팔레트.
...
### D16. 우측 탐색 패널
- **오너**: `ResourcesCatalogCore`를 데모 SidePanel 탭 구조(리소스 종류/서비스/설정/배포)로 재작성. `Mobile`/`Parts` 변형 삭제(F5 확정 해소 — 반응형은 CSS 분기).
- **규칙**: 종류 카운트=D18 목록 보기 표 행수와 동일 소스. 배포 탭에 `+ 저장소 연결`(D7).
...
- **확장(D19와 연동)**: 관점은 3개다 — `지도 | 목록 | 흐름`. 흐름 관점=구 `/traffic`(데모 topology 사양) 흡수. 흐름에서 서비스 클릭=D3 시트, 스코프 공유 동일.
...
- **구조**: 홈은 [고정 헤더 = 상태 요약 줄] + [**위젯 보드**]다. 위젯은 카탈로그(Surface Spec §2.2, W1~W8)에서만 추가하고, `Edit layout`(Kargul 레퍼런스)으로 추가·제거·드래그 배치(`@dnd-kit` — 기존 의존성 재사용, 두 번째 DnD 도입 금지). 배치는 `preferences` 피처에 저장.
- **위젯 공용 문법** (`shared/ui/widgets/WidgetFrame.tsx` — 유일한 위젯 껍데기): 제목 `text-bodyStrong` + ⓘ 설명 툴팁(일시 표면 — 안내 문구 금지 규칙의 명시적 예외) + 우측 `>` 딥링크(반드시 실 라우트) + 접기 토글. 본문 시각은 D13 위젯 부품만 사용: `KpiCard`(큰 값+증감 틴트+요약 1줄) / `TickGauge`(틱 게이지) / `RatioBar`(이중 비율 바+범례) / `MiniBars`(미니 막대+현재 강조) / `Donut`(도넛+값 범례) / `RankList`(색점 순위 리스트) / `SlotMatrix`(점유 도트) / `ProgressFill`(배경 채움 진행). **위젯별 자체 시각 신설 금지.**
- **기간 컨텍스트**: 보드 우상단 셀렉터 1개(오늘/7일/30일)가 모든 시계열 위젯에 공통 적용 — 위젯별 개별 기간 토글 금지(통일성).
- **게이지 역할 분담**(중복 차단): 연속 `MiniBar`=카드·개요 스트립, `TickGauge`=위젯 KPI, `RingGauge`=상세 시트 개요의 순간 사용률 **한정**(카드·홈 금지 — D1 결정 유지). 정의 없는 합성 점수 게이지("헬스 스코어" 류)는 금지 — 근거 없는 숫자는 가짜 정보다.
- **금지**: 홈에 새 피드 구현(최근 활동 위젯 W8은 타임라인의 미니 뷰 임베드), 위젯 안 개별 새로고침 버튼(자동 갱신 일원화).
...
- [보드] 기본 배치: **W1 클러스터 카드 그리드**(D1, 보드 제1 위젯·제거 불가) → W2 인시던트 레일 → W3 동기화 비율 → W4 활동 추이 → W5~W8(선택). 위젯 정의·바인딩은 Surface Spec §2.2.
...
**P3 — 홈·클러스터 축(D19 1단계)**: D1 카드 v6(`ClusterCard` 재작성) + 홈을 5.1 구성으로 개편 + `/clusters` 리다이렉트·`ClustersPage` 은퇴 / D2 개요 스트립 + `clusterStats` 어댑터(카드와 단일 계산 공유) / D7 연결 모달(실 API 진행 이벤트 파생 포함) / D12 드릴 맵 재작성 착수.
...
**P5 — IA 병합 마감(D19 2단계)**: D18 3관점(`지도|목록|흐름`) 완성 — 지도 밑 표 구조 폐지, `/traffic` 리다이렉트 / `/deploy` 신설(탭 3, `/applications`·`/gitops`·`/helm` 리다이렉트) / `/issues` 탭 2(`/alerts` 리다이렉트) / D16 패널 재작성+`Mobile`/`Parts` 삭제 / D6 `GitOpsSyncSearch` 제거 / D15 diff 수렴 / 나머지 표 3종 D5 교체 / D9 AI(FAB·도킹·forceFull·action 카드) / 5.10 수렴 / 단축키 재배치·`tw-animate-css` 제거 검토.
